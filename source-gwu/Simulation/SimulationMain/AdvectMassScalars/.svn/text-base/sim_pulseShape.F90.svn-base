!!****if* source/Simulation/SimulationMain/AdvectMassScalars/sim_pulseShape
!!
!! NAME
!!
!!  sim_pulseShape()
...
!! SYNOPSIS
!!
!!      sim_pulseShape (x, y, fctn, phase, val)
!!      sim_pulseShape (real, real, integer, real, real)
!!
!!
!! DESCRIPTION
!!
...
subroutine sim_pulseShape (x, y, fctn, phase, val)
  ...
  real, intent(IN)    :: x, y, phase
  integer, intent(IN) :: fctn
  real, intent(OUT)   :: val

  real    :: dxphase, dyphase, xx, yy
...
!!$     if ( yy < sim_ymin ) yy = yy + (sim_ymax - sim_ymin)
     ...
     if ( abs(xx) <= sim_width .and. abs(sim_multid*yy) <= sim_width ) then
        val = 1.e0
     else
        val = 0.e0
     end if
     ...
     val = exp( -(xx**2 + sim_multid*yy**2)/sim_width**2 )
     ...
     val = (1.e0-0.5e0*sim_multid) * (sin(sim_twoPi*(x+dxphase)) + sim_multid*sin(sim_twoPi*(y+dyphase)))
