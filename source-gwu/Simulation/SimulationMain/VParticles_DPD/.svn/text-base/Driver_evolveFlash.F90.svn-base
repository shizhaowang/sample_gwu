...
subroutine Driver_evolveFlash()
  ...
  use Simulation_data, ONLY: domainsize,sqrt_dt,pt_NumPart,Aslab,local_Aslab,local_surftension
  use Grid_data,ONLY : gr_globalDomain,gr_meshNumProcs,gr_nblockx,gr_nblocky, gr_nblockz
  use VParticles_DPD_interface, ONLY : IO_writeDpd, IO_writeDpd_parallel
  implicit none
...
#include "Flash_mpi.h"
  ...
  real :: dt,surface_tension
  real :: ET

!------ End of local variables -------------
  

  if (gr_meshNumProcs/=(gr_nblockx*gr_nblocky*gr_nblockz)) call Driver_abortFlash(':  Number of blocks not equal the number of procs and this will cause problems with the virtual paritcles')
  ...
  Aslab=domainsize(IAXIS)*domainsize(JAXIS)
  
  !--------------------
  ! write the geomtry to tecplot
  !----------------------
  call Grid_getLocalNumBlks(localNumBlocks)
  !write(*,*) 'localNumBlocks',localNumBlocks
  ...
  open(unit=115,file='bounds.'//index_proc//'.txt',form='formatted')
  ...
  do i=1,blockCount
     ...
     local_Aslab = (gr_ptVPBndBox(HIGH,IAXIS)-gr_ptVPBndBox(LOW,IAXIS))* & 
          (gr_ptVPBndBox(HIGH,JAXIS)-gr_ptVPBndBox(LOW,JAXIS))
     write(115,*) gr_ptVPBndBox(LOW,IAXIS),gr_ptVPBndBox(HIGH,IAXIS) 
     write(115,*) gr_ptVPBndBox(LOW,JAXIS),gr_ptVPBndBox(HIGH,JAXIS)
     write(115,*) gr_ptVPBndBox(LOW,KAXIS),gr_ptVPBndBox(HIGH,KAXIS) 
  end do
  write(114,*) '  '
  do i=1,blockCount
...
#ifdef DEBUD_LOC
     if(dr_globalMe==3) then
     do i=1,pt_numlocal
        if ( (particles(POSX_PART_PROP,i)<gr_ptVPBndBox(LOW,IAXIS)).or.(particles(POSX_PART_PROP,i)>gr_ptVPBndBox(HIGH,IAXIS))) &
             write(*,*) 'Particle outside proc',dr_globalMe,' bounds Tag',particles(TAG_PART_PROP,i),&
             'x',particles(POSX_PART_PROP,i),'bounds=',gr_ptVPBndBox(LOW:HIGH,IAXIS)
        if ( (particles(POSY_PART_PROP,i)<gr_ptVPBndBox(LOW,JAXIS)).or.(particles(POSY_PART_PROP,i)>gr_ptVPBndBox(HIGH,JAXIS))) &
             write(*,*)  'Particle outside proc',dr_globalMe,' bounds Tag',particles(TAG_PART_PROP,i),&
             'y',particles(POSY_PART_PROP,i),'bounds=',gr_ptVPBndBox(LOW:HIGH,JAXIS)
        if ( (particles(POSZ_PART_PROP,i)<gr_ptVPBndBox(LOW,KAXIS)).or.(particles(POSZ_PART_PROP,i)>gr_ptVPBndBox(HIGH,KAXIS))) &
             write(*,*) 'Particle outside proc',dr_globalMe,' bounds Tag',particles(TAG_PART_PROP,i),&
             'z',particles(POSZ_PART_PROP,i),'bounds=',gr_ptVPBndBox(LOW:HIGH,KAXIS)
     end do
  end if
#endif
  ...
  loop_integ: do dr_nstep = dr_nbegin, dr_nend
     ...
     !write(*,*) 'On proc#',dr_globalMe,'after the IO_writeDpd'
     call IO_output( dr_simTime, dr_dt, dr_nstep+1, dr_nbegin, endRun)

      if ( dr_meshNumProcs>1) then
        total_pt_local=0;
        write(*,*)'The local num of particles before reduction ',pt_numLocal,'on proc#',dr_globalMe
        call MPI_ALLREDUCE(pt_numlocal, total_pt_local,CONSTANT_ONE, & 
             MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, ierr)
        call MPI_ALLREDUCE( local_surftension, surface_tension,CONSTANT_ONE, & 
             MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, ierr)
        if (dr_globalMe==MASTER_PE) then
           if (total_pt_local/=pt_NumPart)  then 
              call Driver_abortFlash('Driver_evolveFlash: Total number of particles is not correct!!');
           else
              write(*,*)'Total no. of particles= ',total_pt_local,'is equal to actual total no. of parts ',pt_NumPart
           end if
           surface_tension=surface_tension/Aslab;
           write(*,*) 'Surface tension =',surface_tension
        end if
     else 
        write(*,*) 'Total no. of particles= ',pt_numlocal,' Actual total Parts ',pt_NumPart
        if (pt_numlocal/=pt_NumPart) call Driver_abortFlash('Driver_evolveFlash: Total number of particles is not correct!!');
     end if

     ! write the particles to the output. 
     call IO_writeDpd(particles,pt_numLocal,fileflag)
     call SYSTEM_CLOCK(TA2(1),count_rate)
     call IO_writeDpd_parallel(particles,pt_numLocal,fileflag)
     CALL SYSTEM_CLOCK(TA2(2),count_rate)
     ET=REAL(TA2(2)-TA2(1),8)/count_rate
     if (dr_globalMe.eq. MASTER_PE)  then 
        write(*,*)'!---------------------------------------------'
        write(*,*)'  '
        write(*,'(A,F12.8,A)')'Parallel write =',ET,'seconds.'
        write(*,*)'  '
        write(*,*)'!---------------------------------------------'
     end if
     ...
     if(endRun) exit
     !write(*,*) 'On proc#',dr_globalMe,'after the IO_output'
