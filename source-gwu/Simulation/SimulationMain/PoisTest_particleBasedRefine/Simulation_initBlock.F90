!!****if* source/Simulation/SimulationMain/PoisTest_particleBasedRefine/Simulation_initBlock
!!
!! NAME
!!
...
!!$
!!$     !print *, "Initializing block:", blockID
  ...
  do k = blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)    
     do j = blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
        do i = blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)       
           ...
           call PrintDensityToFile(i,j,k,rho)
...
!!$  if (sim_meshMe == MASTER_PE) then
!!$     print *, "Finished initializing block:", blockID
!!$  end if
...
subroutine PrintDensityToFile(i, j, k, rho)
...
integer, parameter :: FILE_NUMB1 = 15
character (len=*), parameter :: BaseFileName1="densityOnGrid"
character (len=500) :: filename1, uniqueName

write(uniquename, fmt='(i3.3,''.dat'')') sim_meshMe
filename1 = BaseFileName1//uniquename
open(unit=FILE_NUMB1, file=filename1, POSITION='APPEND')
  
55 FORMAT (3i10, e20.8)

write(FILE_NUMB1, FMT=55) i, j, k, rho  

close(FILE_NUMB1)

end subroutine PrintDensityToFile
