!!****if* source/Simulation/SimulationMain/magnetoHD/CloudShock/Grid_bcApplyToRegionSpecialized
!!
!!
!! NAME
              ...
              do i = 1,guard !x-direction
                 do j=1,je   !y-direction
                    if (gridDataStruct==CENTER) then
                       ...
                       regionData(HEAV_SPEC,i,j,k)=sim_smallX
                       regionData(LIGH_SPEC,i,j,k)=1.0-sim_smallX
#endif
                       if (ivar == DENS_VAR) regionData(i,j,1:ke,ivar) = sim_dL
                       if (ivar == VELX_VAR) regionData(i,j,1:ke,ivar) = sim_uL
                       if (ivar == VELY_VAR) regionData(i,j,1:ke,ivar) = sim_vL
                       if (ivar == VELZ_VAR) regionData(i,j,1:ke,ivar) = sim_wL
                       if (ivar == PRES_VAR) regionData(i,j,1:ke,ivar) = sim_pL
                       if ((ivar == ENER_VAR) .or. (ivar==EINT_VAR)) then
                          tmp_vel2= sim_uL**2 + sim_vL**2 + sim_wL**2
                          tmp_pres=sim_pL
                          tmp_dens=sim_dL
...
#if defined(MAGX_VAR)
                       if (ivar == MAGX_VAR) regionData(i,j,1:ke,ivar)= sim_bxL
#endif
#if defined(MAGY_VAR)
                       if (ivar == MAGY_VAR) regionData(i,j,1:ke,ivar)= sim_byL
#endif
#if defined(MAGZ_VAR)
                       if (ivar == MAGZ_VAR) regionData(i,j,1:ke,ivar)= sim_bzL
#endif
#if defined(MAGP_VAR)
#if defined(MAGX_VAR) && defined(MAGY_VAR) && defined(MAGZ_VAR)
                       tmp_B2 = 0.5*(sim_bxL**2 + sim_byL**2+sim_bzL**2)
#else
                       tmp_B2 = 0.
#endif
                       if (ivar == MAGP_VAR) regionData(i,j,1:ke,ivar)=tmp_B2
#endif
                    elseif (gridDataStruct == FACEX) then
                       regionData(i,j,1:ke,ivar) = sim_bxL
                    elseif (gridDataStruct == FACEY) then
                       regionData(i,j,1:ke,ivar) = sim_byL
                    elseif (gridDataStruct == FACEZ) then
                       regionData(i,j,1:ke,ivar) = sim_bzL
                    endif
                 end do
              end do
           case default
