!!****if* source/Simulation/SimulationMain/magnetoHD/KHS/Grid_bcApplyToRegionSpecialized
!!
!!
!! NAME
...
#include "Flash.h"
  ...
  use Simulation_data, ONLY : sim_gCell, sim_gamma,     &
                              ...
                              sim_shearThick,sim_dVx0,  &
                              sim_perturb,sim_smallX
  ...
  real :: scale, tmp_velx2, tmp_pres, tmp_dens, eintZone, enerZone, ekinZone
  real :: ymid,yvsc,omega

  ! Define a perturbation frequency for vely and magy
  omega=4.*PI

  ymid = 0.5*(sim_yMin+sim_yMax)
  yvsc = (sim_xMax-sim_xMin)/sim_shearThick
   ...
   do ivar = 1,varCount
      if(mask(ivar)) then
        ...
        if(face==LOW) then ! apply BC on the bottom boundary at x=0
           ...
           case(USER_DEFINED)
              ! Supersonic inflow boundary  condition on the xl boundary
              k = 2*guard+1
              if(isFace)k=k+1
              do i = 1,guard !x-direction
                 do j=1,je   !y-direction
                    if (gridDataStruct==CENTER) then
                       if (ivar == DENS_VAR) regionData(i,j,1:ke,ivar) = 1.
                       if (ivar == VELX_VAR) regionData(i,j,1:ke,ivar) = &
                            sim_Vx0+sim_dVx0*tanh((yCoord(j+yOffset)-ymid)/yvsc) 
                       if (ivar == VELY_VAR) then
                          if (sim_perturb) then
                             regionData(i,j,1:ke,ivar) = 0.1*sin(omega*dr_simTime)
                          else
                             regionData(i,j,1:ke,ivar) = 0.
                          endif
                       endif
                       if (ivar == VELZ_VAR) regionData(i,j,1:ke,ivar) = 0.
                       if (ivar == PRES_VAR) regionData(i,j,1:ke,ivar) = 1./sim_gamma
                       if (ivar == MAGX_VAR) regionData(i,j,1:ke,ivar) = sim_Bx0
                       if (ivar == MAGY_VAR) then
                          if (sim_perturb) then
                             regionData(i,j,1:ke,ivar) = -0.1*sin(omega*dr_simTime)*sim_By0
                          else
                             regionData(i,j,1:ke,ivar) = sim_By0
                          endif
                       endif
                       if (ivar == MAGZ_VAR) regionData(i,j,1:ke,ivar) = sim_Bz0
                       if (ivar == MAGP_VAR) regionData(i,j,1:ke,ivar) = .5*(sim_Bx0**2+sim_By0**2+sim_Bz0**2)
                       if (ivar == DIVB_VAR) regionData(i,j,1:ke,ivar) = 0.
                       if ((ivar == ENER_VAR) .or. (ivar==EINT_VAR)) then
                          tmp_velx2=(sim_Vx0+sim_dVx0*tanh((yCoord(j+yOffset)-ymid)/yvsc))**2
                          tmp_pres=1./sim_gamma
                          tmp_dens=1.
                          ekinZone = 0.5 * tmp_velx2
                       ...
                       if (ivar == LIGH_SPEC) then
                          if (yCoord(j+yOffset) < ymid) then
                             regionData(i,j,1:ke,ivar) = sim_smallX
                          else
                             regionData(i,j,1:ke,ivar) = 1.-sim_smallX
                          endif
                       endif
                       if (ivar == HEAV_SPEC) then
                          if (yCoord(j+yOffset) < ymid) then
                             regionData(i,j,1:ke,ivar) = 1.-sim_smallX
                          else
                             regionData(i,j,1:ke,ivar) = sim_smallX
...
#if NFACEVARS > 1
                    else !for facevariables
                       if ((gridDataStruct==FACEX)) regionData(i,j,1:ke,ivar)=sim_Bx0
                       if ((gridDataStruct==FACEY)) regionData(i,j,1:ke,ivar)=sim_By0
                       if ((gridDataStruct==FACEZ)) regionData(i,j,1:ke,ivar)=sim_Bz0
#endif
                    endif
                 end do
              end do
