!!****if* source/Simulation/SimulationMain/magnetoHD/Torus/Grid_bcApplyToRegionSpecialized
!!
!!
!! NAME
   ...
   do ivar = 1,varCount
      if(mask(ivar)) then
        ...
        if(face==LOW) then ! apply BC on the bottom boundary at x=0
           ...
           if (axis==IAXIS) then   ! IBEG 
              do i = 1,guard !x-direction
                 if (gridDataStruct==CENTER) then
                    regionData(i,1:je,1:ke,ivar) = regionData(guard+1,1:je,1:ke,ivar)
                    if (ivar == VELX_VAR) regionData(i,1:je,1:ke,ivar) = min(0.0,regionData(guard+1,1:je,1:ke,ivar))
                    if (ivar == MAGX_VAR) regionData(i,1:je,1:ke,ivar)= 0.0
                    if (ivar == MAGY_VAR) regionData(i,1:je,1:ke,ivar)= 0.0
                    if (ivar == MAGZ_VAR) regionData(i,1:je,1:ke,ivar)= 0.0
                    if (ivar == MAGP_VAR) regionData(i,1:je,1:ke,ivar)= 0.0
           ...
           if (axis==JAXIS) then ! JBEG
              do i = 1,guard !y-direction
                 if (gridDataStruct==CENTER) then
                    regionData(i,1:je,1:ke,ivar) = regionData(guard+1,1:je,1:ke,ivar)
                 elseif (gridDataStruct == FACEX) then
                    regionData(i,1:je,1:ke,ivar) = 0.0
