!!****if* source/Simulation/SimulationMain/magnetoHD/SupersonicCloud/Grid_bcApplyToRegionSpecialized
!!
!!
!! NAME
...
#endif
                       if (ivar == DENS_VAR) regionData(i,j,1:ke,ivar) = sim_dAmbient
                       if (ivar == VELX_VAR) regionData(i,j,1:ke,ivar) = sim_uAmbient
                       if (ivar == VELY_VAR) regionData(i,j,1:ke,ivar) = sim_vAmbient
                       if (ivar == VELZ_VAR) regionData(i,j,1:ke,ivar) = sim_wAmbient
                       if (ivar == PRES_VAR) regionData(i,j,1:ke,ivar) = 1./sim_gamma
                       if ((ivar == ENER_VAR) .or. (ivar==EINT_VAR)) then
                          tmp_vel2= sim_uAmbient**2 + sim_vAmbient**2 + sim_wAmbient**2
                          tmp_pres=1./sim_gamma
                          tmp_dens=sim_dAmbient
