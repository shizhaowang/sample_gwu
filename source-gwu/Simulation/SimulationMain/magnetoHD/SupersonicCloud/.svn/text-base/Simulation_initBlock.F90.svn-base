!!****if* source/Simulation/SimulationMain/magnetoHD/SupersonicCloud/Simulation_initBlock
!!
!! NAME
!!
  ...
  do k = blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)
     do j = blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
        do i = blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)
           ...
           solnData(DENS_VAR,i,j,k)=sim_dAmbient
...
!!$              solnData(VELX_VAR,i,j,k)=-sim_uAmbient*.01
!!$           elseif ((radius > r0).and.(radius < sim_cloudRadius)) then
!!$              solnData(DENS_VAR,i,j,k)=sim_dAmbient + (sim_cloudDensity-sim_dAmbient)*taper
           endif

           !! Let the bullet remain stationary during the simulation
           if ((xCoord(i) > sim_cloudXCtr) .and. (abs(yCoord(j)) <= sim_cloudRadius)) then
              solnData(VELX_VAR,i,j,k) = 0.
           endif
