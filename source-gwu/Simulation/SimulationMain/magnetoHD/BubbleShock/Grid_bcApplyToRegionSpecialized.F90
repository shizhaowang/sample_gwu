!!****if* source/Simulation/SimulationMain/magnetoHD/BubbleShock/Grid_bcApplyToRegionSpecialized
!!
!!
!! NAME
...
#include "Flash.h"
  ...
  use Simulation_data, ONLY : sim_gCell, sim_killdivb, &
   ...
   do ivar = 1,varCount
      if(mask(ivar)) then
...
#ifdef VELX_VAR
              if ((ivar==VELX_VAR).and.(axis==IAXIS))sign=-1.
#endif
#ifdef VELY_VAR
              if ((ivar==VELY_VAR).and.(axis==JAXIS))sign=-1.
#endif
#ifdef VELZ_VAR
              if ((ivar==VELZ_VAR).and.(axis==KAXIS))sign=-1.
#endif
           else
              if ((gridDataStruct==FACEX).and.(axis==IAXIS))sign=-1.
              if ((gridDataStruct==FACEY).and.(axis==JAXIS))sign=-1.
              if ((gridDataStruct==FACEZ).and.(axis==KAXIS))sign=-1.
           endif
        ...
        if(face==LOW) then ! apply BC on the bottom boundary at y=ymin
           ...
           case(USER_DEFINED)
              ! We consider applying the conducting BC on the lower x=0 boundary.
              ! That is, the same reflecting BC is being considered as we're treating
              ! the boundary as if the rigid wall (or perfectly conducting wall).
              ! As the wall is "perfect" conductor (e.g., metal), the plasma and gas
              ! are not leaving or coming in across the boundary.
              ! That is, for velocity V=(u,v,w), n_y \dot V = v = 0, and
              !          for B field  B=(Bx,By,Bz), n_y \dot B = By = 0.
              ! See more 4.2 (p. 143) in a book by Goedbloed and Poedts.

              !! Apply reflecting for v & By
              k = 2*guard+1
              if(isFace)k=k+1
              do i = 1,guard
                 regionData(i,1:je,1:ke,ivar)= regionData(k-i,1:je,1:ke,ivar)*sign
