!!****if* source/Simulation/SimulationMain/molecular/Water1/pt_initPositionsWithDensity
!!
!! NAME
!!
...
!!
!!   This version:
!!    Simplified copy of ParticlesInitialization/WithDensity/CellMassBins/pt_initPositionsWithDensity.F90,
...
subroutine pt_initPositionsWithDensity (blockID,success)
  ...
  use Particles_data, ONLY:  pt_myPE, pt_numProcs, pt_numLocal, pt_geometry, &
...
#include "Particles.h"
  ...
  real          :: blockMass, blockVolume, ratioVolume, ratioDensity
...
! This duplicates loop below - will consolidate later - KW
...
!!  Previous version did not put any particles in small blocks due to trucnation
...
!!     pt_numLocal=0
  ...
  if (pt_myPE == MASTER_PE .OR. pt_numProcs .LE. 4) then
     print*,'pt_initPositionsWithDensity on processor', pt_myPE, &
          ': increasing pt_numLocal==',pt_numLocal,' by', numParticlesThisBlock
  end if
#endif
