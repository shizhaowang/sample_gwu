!!****if* source/Simulation/SimulationMain/LaserSlabLayers/Simulation_initBlock
!!
!! NAME
!!
...
#ifndef CHAM_SPEC
  integer :: CHAM_SPEC = 1, TARG_SPEC = 2, COAT_SPEC = 3
#endif
     ...
     do j = blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
        do i = blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
           ...
           if ( xcent(i) <= sim_targetRadius .and. &
                ycent(j) <= sim_targetHeight + sim_coatHeight + sim_vacuumHeight .and. &
                ycent(j) >= sim_vacuumHeight + sim_coatHeight ) then
              species = TARG_SPEC
           else if (xcent(i) <= sim_targetRadius .and. &
                ycent(j) <= sim_coatHeight + sim_vacuumHeight .and. &
           ...
           if(species == TARG_SPEC) then
              ...
              tion = sim_tionTarg
              trad = sim_tradTarg
           else if (species == COAT_SPEC) then
              rho = sim_rhoCoat
              tele = sim_teleCoat
              tion = sim_tionCoat
              trad = sim_tradCoat
           ...
           if (NSPECIES > 0) then
              ...
              do n = SPECIES_BEGIN,SPECIES_END
                 if (n==species) then
                    call Grid_putPointData(blockID, CENTER, n, EXTERIOR, axis, 1.0e0-10*sim_smallX)
                 else
                    call Grid_putPointData(blockID, CENTER, n, EXTERIOR, axis, 5*sim_smallX)
                 end if
              enddo
           end if
