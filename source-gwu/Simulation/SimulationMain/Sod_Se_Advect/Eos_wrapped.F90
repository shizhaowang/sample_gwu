!!****if* source/Simulation/SimulationMain/Sod_Se_Advect/Eos_wrapped
!! NAME
!!
!!  Eos_wrapped
!! 
...
#define DEBUG
  ...
  do k = range(LOW,KAXIS), range(HIGH,KAXIS)
     do j = range(LOW,JAXIS), range(HIGH,JAXIS)
        ...
        case(MODE_DENS_EI_SELE_GATHER)
           do i = 1,vecLen
              call Eos_getData(IAXIS,pos,1,solnData,dataStruct,eosData,massFraction)
!!$              if (solnData(DBGS_VAR,i,j,k) > 1.0e-3) then
