!!****if* source/Simulation/SimulationMain/Sod_Se_Advect/Simulation_initBlock
!!
!! NAME
!!
     ...
     do j = blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
        ...
        do i = blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
           ...
           elseif ((xxL < lPosn) .and. (xxR > lPosn)) then
              ...
              if (NSPECIES > 0) then
                 call Grid_putPointData(blockID, CENTER, SPECIES_BEGIN, EXTERIOR, &
                      axis, 0.5e0-(NSPECIES-2)*0.5*sim_smallX)
                 if (NSPECIES > 1) &
                 ...
                 !if there is only 1 species, this loop will not execute
                 do n = SPECIES_BEGIN+3,SPECIES_END
                    call Grid_putPointData(blockID, CENTER, n, EXTERIOR, &
                         axis, sim_smallX)
                 enddo
              end if
...
#endif
#ifdef SUMY_MSCALAR
           call Grid_putPointData(blockId, CENTER, SUMY_MSCALAR, EXTERIOR, axis, 1/12.0)
#endif
#ifdef YE_MSCALAR
