!!****if* source/Simulation/SimulationMain/ShafranovShock/Driver_evolveFlash
!!
!! NAME
!!
!!  Driver_evolveFlash
...
#include "Flash.h"
  ...
  real    :: MaxNorm ! relative, over entire simulation. ||A-S||/||A||
  real    :: eNorm   ! error norm, ||A-S||, A-Analytical, S-Simulated
  real    :: aNorm   ! norm of analytical solution, ||A||.
...
!!******************************************************************************
     ...
     do lb=1, localNumBlocks
        call Simulation_computeAnalytical(lb,  dr_simTime)
     end do
     
     call Grid_computeVarDiff(-1,    RHOA_VAR, DENS_VAR, DIFF_VAR)     
     call Grid_computeVarNorm( 0, 2, DIFF_VAR,eNorm, 0)
     call Grid_computeVarNorm( 0, 2, RHOA_VAR,aNorm, 0)
     MaxNorm = MAX(MaxNorm, eNorm/aNorm)
...
!!******************************************************************************
  ...
  if (MaxNorm  > sim_maxTol) perfect = .FALSE.
  
  write(fileUnit,*) "L2 Error Norm (Relative):", MaxNorm
  write(fileUnit,*) "L2 Error Norm           :", eNorm
  
  write(*,*) "L2 Error Norm (Relative) :", MaxNorm
  write(*,*) "L2 Error Norm            :", eNorm
  
  if (perfect) then
     write(fileUnit,'("ShafranovShock unitTest PASSED!  all results conformed with expected values.")')
     write(*,'("ShafranovShock unitTest PASSED!  all results conformed with expected values.")')
     call Logfile_stamp( "ShafranovShock unitTest PASSED!")
  else
     write(fileUnit,'("ShafranovShock unitTest FAILED!")')
     write(*,'("ShafranovShock unitTest FAILED!")')
     call Logfile_stamp( "ShafranovShock unitTest FAILED!")
  endif
