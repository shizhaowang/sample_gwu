!!****if* source/Simulation/SimulationMain/unitTest/XYZneu_3D_BiPCGSTAB/Driver_evolveFlash
!!
!! NAME
!!
!!  Driver_evolveFlash
...
!!$#else
  ...
  do eachBoundary = 1, 2*NDIM
     ...
     case (PERIODIC)
        ...
        if (dr_globalMe .eq. 0) write(*,*) eachBoundary,'GRID_PDE_BND_PERIODIC'
     case (OUTFLOW)
        bc_types(eachBoundary)  = GRID_PDE_BND_NEUMANN
        if (dr_globalMe .eq. 0) write(*,*) eachBoundary,'GRID_PDE_BND_NEUMANN'
     case (DIRICHLET)
        bc_types(eachBoundary)  = GRID_PDE_BND_DIRICHLET
        if (dr_globalMe .eq. 0) write(*,*) eachBoundary,'GRID_PDE_BND_DIRICHLET'
     case default
