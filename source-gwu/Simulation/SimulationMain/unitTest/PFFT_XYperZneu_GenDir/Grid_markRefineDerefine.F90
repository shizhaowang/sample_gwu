!!****if* source/Simulation/SimulationMain/unitTest/PFFT_XYperZneu_GenDir/Grid_markRefineDerefine
!!
!! NAME
...
!!$  end do
  ...
  specs(1) =  sim_xMin + 1./4.*(sim_xMax - sim_xMin) +.05
  specs(2) =  sim_xMin + 3./4.*(sim_xMax - sim_xMin) -.05

  !! define a range of coordinates of the rectangle in y-direction
  specs(3) =  sim_yMin + 1./4.*(sim_yMax - sim_yMin) +.05
  specs(4) =  sim_yMin + 3./4.*(sim_yMax - sim_yMin) -.05

  !! define a range of coordinates of the rectangle in z-direction
  specs(5) =  sim_zMin + 1./4.*(sim_zMax - sim_zMin) +.05
  specs(6) =  sim_zMin + 3./4.*(sim_zMax - sim_zMin) -.05
