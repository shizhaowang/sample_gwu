!!****if* source/Simulation/SimulationMain/HeatexchangeIonEle/Simulation_initBlock
!!
!! NAME
!!
...
!!  a specified block.  This version sets up constant temperature
!!  initial consitions.
!! 
!! ARGUMENTS
!!
...
!   with some offset in time toff. The gas is initially stationary.
      ...
      ! Now add this Guassian shape to the temperature background
       temp1Zone = sim_tempBackground + sim_ionTemp
       temp2Zone = sim_tempBackground + sim_eleTemp
       temp3Zone = sim_tempBackground + sim_radTemp
...
#define DO_CALL_EOS
        ...
        eosData(EOS_TEMPION) = temp1Zone
        eosData(EOS_TEMPELE) = temp2Zone
        eosData(EOS_TEMPRAD) = temp3Zone
        eosData(EOS_DENS) = rhoZone

#ifdef MODE_DENS_TEMP_ALL
        CALL Eos(MODE_DENS_TEMP_GATHER, vecLen, eosData)
#else
...
#ifdef TION_VAR
           call Grid_putPointData(blockId, CENTER, TION_VAR, EXTERIOR, axis, temp1Zone)
           call Grid_putPointData(blockId, CENTER, T0IO_VAR, EXTERIOR, axis, temp1Zone)
#endif
#ifdef TELE_VAR
           call Grid_putPointData(blockId, CENTER, TELE_VAR, EXTERIOR, axis, temp2Zone)
           call Grid_putPointData(blockId, CENTER, T0EL_VAR, EXTERIOR, axis, temp2Zone)
#endif
#ifdef TRAD_VAR
           call Grid_putPointData(blockId, CENTER, TRAD_VAR, EXTERIOR, axis, temp3Zone)
           call Grid_putPointData(blockId, CENTER, T0R_VAR,  EXTERIOR, axis, temp3Zone)
#endif
