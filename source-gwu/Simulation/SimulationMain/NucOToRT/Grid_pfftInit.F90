!!****if* source/Simulation/SimulationMain/NucOToRT/Grid_pfftInit
!!
!! NAME 
!!
!!   Grid_pfftInit
...
#include "Flash.h"
  ...
  use gr_pfftData, ONLY : pfft_inLen, pfft_globalLen, pfft_transformType, &
       ...
       pfft_commWithTopology, pfft_usableProc, pfft_myPE, pfft_numProcs, pfft_mode
  use Driver_interface, ONLY : Driver_abortFlash
  use Grid_data, ONLY : gr_oneRefLev, gr_meshMe,gr_meshNumProcs
  ...
  comm = MPI_COMM_WORLD
  pfft_comm(1:MDIM) = (/comm, MPI_COMM_NULL, MPI_COMM_NULL/)
  ...
  call Logfile_stampMessage( & 
       "[Grid_pfftInit]: Using refinement level:"//char(48+gr_oneRefLev))
  ...
  if (needMap) then
     ...
     call Logfile_stampMessage( &
          "[Grid_pfftInit]: WARNING - Maybe you should set needMap to true when not using UG!")
  end if
  ...
  !---------------------------------------------------------------------
#if(0)
  print*,'pfft_inLen=',pfft_inLen
  print*,'pfft_globalLen=',pfft_globalLen
  print*,'pfft_procGrid=',pfft_procGrid
#endif
  pfft_inLen(:) = 1
  pfft_inLen(1:NDIM) = (pfft_globalLen(1:NDIM) + pfft_procGrid(1:NDIM) - 1) / &
