!!****if* source/Simulation/SimulationMain/VParticles_DPD/gr_ptVPBC
!!
!! NAME
!!
!!  gr_ptVPBC
...
subroutine gr_ptVPBC(dataElement, propCount, leftDomain, onBoundary)
  ...
  use gr_ptData, ONLY : gr_ptBlk,gr_ptPosx,gr_ptPosy,gr_ptPosz, gr_ptSmearLen,gr_ptproc
...
!!  lostElements=0
  ...
  do i = 1,NDIM
     ...
     if(.not.left) then
        ...
        !write(*,*) i,'pos(i)',pos(i),gr_globalDomain(LOW,i),gr_globalDomain(HIGH,i)
        if(pos(i)<gr_globalDomain(LOW,i))then
           !if (int(dataElement(TAG_PART_PROP))==925) write(*,*)'11 pos =', pos(:)
           !write(*,*) i,'pos(i)',pos(i),gr_globalDomain(LOW,i),gr_globalDomain(HIGH,i)
           lostElements=0
           ...
           !write(*,*)'5 pos',pos(1:NDIM),'proc',int(dataElement(gr_ptProc))
           call gr_ptOneFaceBC(dataElement,propCount,i,LOW,blk,lostElements)
           pos(IAXIS:KAXIS)=dataElement(gr_ptPosx:gr_ptPosz)
           !write(*,*)'3 pos',i,pos(i),'proc',int(dataElement(gr_ptProc))
        ...
        if(.not.left) then
           ...
           if((pos(i)-gr_globalDomain(LOW,i))<rc)onBoundary(i)=LEFT_EDGE
        end if
        ...
        if((.not.done).and.(.not.left))then
           ...
           if(pos(i)>gr_globalDomain(HIGH,i))then
              ...
              pos(IAXIS:KAXIS)=dataElement(gr_ptPosx:gr_ptPosz)
              !write(*,*)'4 pos',i,pos(i),'proc',int(dataElement(gr_ptProc))
              left=(lostElements>0)
           ...
           if(.not.left)then
              if((gr_globalDomain(HIGH,i)-pos(i))<rc)onBoundary(i)=RIGHT_EDGE
           end if
  ...
  !if (int(dataElement(TAG_PART_PROP))==366) write(*,*)'2 pos =', pos(:)
  leftDomain=left
