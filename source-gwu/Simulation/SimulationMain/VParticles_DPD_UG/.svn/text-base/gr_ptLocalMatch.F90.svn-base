!!****if* source/Simulation/SimulationMain/VParticles_DPD/gr_ptLocalMatch
!!
!! NAME
!!
!!  gr_ptLocalMatch
...
#include "Flash.h"
  use gr_ptData, ONLY : gr_ptBlkCount, gr_ptBlkList,&
       gr_ptBlk,gr_ptPosx, gr_ptPosz, gr_ptProc, gr_ptKeepLostParticles
  ...
  use gr_interface, ONLY:  gr_findBlock
  ...
  do i = 1,numSource
     ...
     if(blockID/=UNKNOWN) then
        ...
        pos(IAXIS:KAXIS)=sourceBuf(gr_ptPosx:gr_ptPosz,i)
...
993  format(I5,' unknown on ',I2,':(',3G15.8,')',I6)
     ...
     if(blockID==NONEXISTENT) then !! if not then move it to destBuf
        ...
        !! of elements is not more than maximu
