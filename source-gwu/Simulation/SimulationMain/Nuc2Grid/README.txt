Nuc2RT is a new version of tool developed from NucOToRT. Some new and important 
features are added, such as weighted average and gaussian processes interpolation 
methods to interpolate cells which are lack of particles.

Please note that the old config and par files are not compatibale with Nuc2RT.

The following setup generates a utility for mapping the nucleosynthesis output from
...
6.   Run setup (after switching back to the appropriate directory), with some
     additional command line options.  I use something like this:
       ...
       ./setup -auto Nuc2RT -objdir=Nuc2RT -noc -debug -2d +curvilinear +pm4dev --unit=IO/IOParticles +parallelIO -defines=PT_MAX_ATTRIBUTES=NNN Convolve=True -nxb=64 -nyb=128 -parfile=Set-2D-2-noconvo.par -maxblocks=2048 -gridinterpolation=native
...
11.  If all goes well, enjoy your output file named "PhoenixInputData".
     ...
     Note: the following step is not needed in Nuc2RT
