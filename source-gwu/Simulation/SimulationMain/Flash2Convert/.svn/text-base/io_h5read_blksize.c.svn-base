...
/****if* source/IO/IOMain/hdf5/io_h5read_blksize_
 *
 * NAME
 *
 *  io_h5read_blksize_
 ...
 * SYNOPSIS
 *
 *  void io_h5read_blksize_(file_identifier, maximum_blocks, size, local_blocks, 
 *                      total_blocks, global_offset)
 *
 *  void io_h5read_blksize_(hid_t *, int *, double *, int *, int *, int *)
 *
 *
 * DESCRIPTION
 *
 *  Read in a single processors's portion of the size array from a FLASH 
 ...
 *                           allocated for.
 *
 *  size                     the size array (assumed to be 
 *                           size[maximum_blocks][mdim]) (returned)
 *
...
void FTOC(io_h5read_blksize)(hid_t* file_identifier,
                  int* maximum_blocks,
                  double* size,
                  int* local_blocks,
                  int* total_blocks,
                  int* global_offset)
{
  ...
  /* open the dataset */
  dataset = H5Dopen(*file_identifier, "block size");
  ...
  if (status < 0){
    printf("Error: Unable to select hyperslab for size dataspace\n");
    Driver_abortFlashC("Error: Unable to select hyperslab for size dataspace\n");
  }
  ...
  if (ierr < 0){
    printf("Error: Unable to select hyperslab for size memspace\n");
    Driver_abortFlashC("Error: Unable to select hyperslab for size memspace\n");
  }
  ...
  if (status < 0){
    ...
    Driver_abortFlashC("Error: Unable to read size\n");
  }
