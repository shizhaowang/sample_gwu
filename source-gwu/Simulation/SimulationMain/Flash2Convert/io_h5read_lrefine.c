...
/****if* source/IO/IOMain/hdf5/io_h5read_lrefine_
 *
 * NAME
 *
 *  io_h5read_lrefine_
 ...
 * SYNOPSIS
 *
 *  void io_h5read_lrefine_(file_identifier, lrefine, local_blocks, total_blocks,
 *                        global_offset)
 *
 *  void io_h5read_lrefine_(hid_t *, int [], int *, int *, int *)
 *
 *
 * DESCRIPTION
 *
 *  Read in a single processors's portion of the refinement level data 
 *  (lrefine) from a FLASH HDF5 checkpoint file.  Given the HDF5 file handle 
 ...
 *
 *  lrefine                  the refinement level data (returned)
 *
...
void FTOC(io_h5read_lrefine)(hid_t* file_identifier,
                  int lrefine[],
                  int* local_blocks,
                  int* total_blocks,
                  int* global_offset)
{
  ...
  /* open the dataset */
  dataset = H5Dopen(*file_identifier, "refine level"); 
  ...
  if (status < 0){
    printf("Error: Unable to select hyperslab for lrefine dataspace\n");
    Driver_abortFlashC("Error: Unable to select hyperslab for lrefine dataspace\n");
  }
  ...
  if (status < 0){
    printf("Error: Unable to read data for lrefine\n");
    Driver_abortFlashC("Error: Unable to read data for lrefine\n");
  }
