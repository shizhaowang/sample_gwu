!!****if* source/Simulation/SimulationMain/INavierStokes/2D/IB_Cyl_parallelIB/gr_sbCreateParticles
!!
!! NAME
...
Subroutine gr_sbCreateParticles()
  use gr_sbData, ONLY : gr_sbBodyInfo, gr_sbNumBodies, aelem, totalPart
  ...
  write(*,*) 'gr_sbNumBodies=',gr_sbNumBodies

  do b = 1, gr_sbNumBodies
     p = 0
     numTriangles = gr_sbBodyInfo(b)%NumTriangles  !size(gr_sbBodyInfo(b) % triangleCentroids,2)
!     allocate(gr_sbBodyInfo(b) % particles(1:NPART_PROPS,numParticles*numTriangles))
     ...
     write(*,*) ' gr_sbBodyInfo(b) % totalPart=', gr_sbBodyInfo(b) % totalPart
