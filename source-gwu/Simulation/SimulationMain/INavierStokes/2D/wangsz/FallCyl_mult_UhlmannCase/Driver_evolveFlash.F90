...
!!$  nzc = NZB + NGUARD + 1
  ...
  if (dr_globalMe == MASTER_PE) write(*,*) 'Driver evolve flash'
...
!!$     call Timers_stop("Grid_updateRefinement")
 ...
 !wsz       call outtotecplot_uv(dr_globalMe,dr_simtime,dr_dt,dr_nstep,count, &
 !wsz                         0.0,blockList,blockCount,firstfileflag)
...
!wsz        do ibd = 1,NumBodies
!wsz           if (sm_meshMe .eq. sm_BodyInfo(ibd)%BodyMaster)then
!wsz              call sm_ioWriteSnapshot(ibd,count)
              ! Here we write particles
!wsz              call sm_ioWriteParticles(ibd,count)
!wsz           endif
!wsz        end do
#endif
