...
!  ndmax = body%max_dofs_per_node
  ...
  if( link_exists ) then
    ...
    if (abs(Body%gravity_vec(IAXIS)-sm_gravX) .gt. eps) &
      write(*,*) 'Warning:', "Body X direction gravity component not equal to flash.par gravX."
    ...
    if (abs(Body%gravity_vec(JAXIS)-sm_gravY) .gt. eps) &
      write(*,*) "Warning: Body Y direction gravity component not equal to flash.par gravY."
#if NDIM == MDIM
  ...
  ! debug, Shizhao
!  Body%gravity_vec = 0.
!  write(*,*) 'Debug: the gravity is forced to be zero' 
...
!!$  enddo
  ...
  !Body%qn(body%ID(3,1)) = acos(-1.0)/4.0
