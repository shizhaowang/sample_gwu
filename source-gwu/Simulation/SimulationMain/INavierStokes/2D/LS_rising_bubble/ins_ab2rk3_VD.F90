...
#include "SolidMechanics.h"
  ...
  use IncompNS_data, ONLY : ins_isgs, ins_invRe, ins_intschm, ins_prescoeff, ins_meshMe,&
                            ...
                            ins_gravX, ins_gravY, ins_gravZ, ins_Qin1, ins_Qin2, &
                            ins_Qout1, ins_Qout2, ins_convvel1, ins_convvel2
...
!***********************************************************************************************
  ...
  call ins_computeQinout( blockCount, blockList, .true., ins_Qin, ins_Qin2) !Shizhao for 2 phase flow
  
  ! For OUTFLOW_INS condition compute convective velocity
  !call ins_convectVelout( blockCount, blockList, ins_convvel)
  call ins_convectVelout( blockCount, blockList, ins_convvel1, ins_convvel2)
  ...
  if (dr_nstep .eq. 1) then
   ...
   do ii = 1,mph_inls
     ...
     do lb = 1,blockCount
        ...
        if ( ii .eq. mph_inls .AND. lb .eq. 1 .AND. ins_meshMe .eq. 0) then
    ...
    call mph_bcLevelSet(0.0d0,0.0d0)
     ...
     if(mod(ii,1000) == 0) then
       write(*,*) 'Output DFUN:', ins_meshMe, ii, lsT, lsDT
       call outtotecplot(ins_meshMe,lsT,lsDT,ii,ii, &
...
!*************************************************************************************************
   ...
   call Timers_start('SaveVarForFSI')  ! Shizhao Wang
...
#endif 
  enddo
  call Timers_stop('SaveVarForFSI')  ! Shizhao Wang
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  call Timers_start('CopyVarForFSI')  ! Shizhao Wang
  ...
  call Timers_stop('CopyVarForFSI')  ! Shizhao Wang


!  if (ins_meshMe .eq. MASTER_PE)  write(*,*) 'Copy old variables'
...
!  if (ins_meshMe .eq. MASTER_PE)  write(*,*) 'After solid mechanics advance'
  ...
  call ins_computeQinout( blockCount, blockList, .true., ins_Qin1, ins_Qin2) !Shizhao
  call ins_computeQinout( blockCount, blockList, .false., ins_Qout1, ins_Qout2) ! Shizhao
  ...
  call ins_rescaleVelout(  blockCount, blockList, ins_Qin1, ins_Qin2, ins_Qout1, ins_Qout2)
...
!  do ii=1,1
    ...
    if(ins_meshMe == 0) write(100000,'(I, 12f20.12)') dr_nStep, timeEndAdv, vol, cx, cy, vx, vy, xh, yh, xl, yl
...
!************************************************************************************************************
   ...
   do ii = 1,mph_lsit
     ...
     do lb = 1,blockCount
        ...
        !minCellDiag = SQRT(del(DIR_X)**2.+del(DIR_Y)**2.+del(DIR_Z)**2.)
        minCellDiag = SQRT((SQRT(del(DIR_X)**2.+del(DIR_Y)**2.))**2.+del(DIR_Z)**2.)
        if ( ii .eq. mph_lsit .AND. lb .eq. 1 .AND. ins_meshMe .eq. 0) then
           print*,"Level Set Initialization Iteration # ",ii,minCellDiag,lsDT
        end if
...
iOutPress = 0
if (iOutPress == 1) then
     write(filename, '("dfun0_",i6.6,"_",i3.3)') ins_meshMe, 0
     open(123, file=trim(filename)//'.dat')
     write(123, *) 'VARIABLES = "x", "y", "var", "var2"'
  do lb = 1,blockCount
     blockID = blockList(lb)
     ...
     write(123,*) 'ZONE  I =', blkLimitsGC(HIGH,IAXIS), ', J =', blkLimitsGC(HIGH,JAXIS)
     ...
     do j=blkLimitsGC(LOW,JAXIS), blkLimitsGC(HIGH,JAXIS)
       do i=blkLimitsGC(LOW,IAXIS), blkLimitsGC(HIGH,IAXIS)
           ...
           write(123,*) xcell, ycell, solnData(DFUN_VAR,i,j,1), solnData(DFUN_VAR,i,j,1)
