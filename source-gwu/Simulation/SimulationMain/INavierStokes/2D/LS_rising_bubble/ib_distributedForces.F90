     ...
     do gridind = 1,nkij
        ...
        if (presflag .eq. CONSTANT_ONE) then         ! Pressure
           ...
           do i = 1 , ib_stencil      
              ...
              visc = solnData(VISC_VAR,ielem(i,IAXIS,presflag+1), &
                                       ielem(i,JAXIS,presflag+1), &
                                       ielem(i,KAXIS,presflag+1));   ! Shizhao 
...
!#ifdef INS_CONSTDENS
  ...
  particleData(FXVI_PART_PROP) = (nzp*nuwy-nyp*nuwz)*visc  ! Shizhao
  particleData(FYVI_PART_PROP) = (nxp*nuwz-nzp*nuwx)*visc
  particleData(FZVI_PART_PROP) = (nyp*nuwx-nxp*nuwy)*visc
...
!  dwn = dwn - (dpdxt*tz)*normt
  ...
  particleData(FXVI_PART_PROP) = nu*dun*visc 
  particleData(FYVI_PART_PROP) = nu*dvn*visc 
  particleData(FZVI_PART_PROP) = nu*dwn*visc 
  ...
  particleData(FXVI_PART_PROP) = 2.*nu*(exx*nxp+exy*nyp+exz*nzp)*visc
  particleData(FYVI_PART_PROP) = 2.*nu*(exy*nxp+eyy*nyp+eyz*nzp)*visc
  particleData(FZVI_PART_PROP) = 2.*nu*(exz*nxp+eyz*nyp+ezz*nzp)*visc
...
!  dwn = dwn - (dpdxt*tz)*normt
  ...
  !call Driver_abortFlash("Variable density particle pressure and tangent stress not defined.")
  particleData(PRES_PART_PROP) = 0.0 !+ ins_gravX*(xp-gr_imin) + &
                                    !  ins_gravY*(yp-gr_jmin) + &
                                    !  ins_gravZ*(zp-gr_kmin)
  particleData(PEX0_PART_PROP) = 0.0
  particleData(PEXT_PART_PROP) = 0.0 !+ ins_gravX*(xbe(IAXIS)-gr_imin) + &
                                       !  ins_gravY*(xbe(JAXIS)-gr_jmin) + &
                                       !  ins_gravZ*(xbe(KAXIS)-gr_kmin)
