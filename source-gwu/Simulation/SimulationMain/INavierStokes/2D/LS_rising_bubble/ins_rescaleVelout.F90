...
#include "constants.h"
  ...
  real :: Qindout, Qindout1, Qindout2
...
#ifdef TWO_PHASE_SEP
! Only works for 2D flows
! Only works for flow along IAXIS

  if(NDIM == 3) then
    write(*,*) 'ins_rescaleVelOut only works for 2D flows in this verision'
    stop
  endif

  ! Check if Qin and Qout are zero
  if ((Qin1 .lt. eps) .and. (ins_meshMe .eq. MASTER_PE))  &
     write(*,*) 'ins_rescaleVelout: Warning Qin1=',Qin1,' <',eps  
  if ((Qout1 .lt. eps) .and. (ins_meshMe .eq. MASTER_PE)) &
     write(*,*) 'ins_rescaleVelout: Warning Qout1=',Qout1,' <',eps  
  if ((Qin2 .lt. eps) .and. (ins_meshMe .eq. MASTER_PE))  &
     write(*,*) 'ins_rescaleVelout: Warning Qin2=',Qin2,' <',eps  
  if ((Qout2 .lt. eps) .and. (ins_meshMe .eq. MASTER_PE)) &
     write(*,*) 'ins_rescaleVelout: Warning Qout2=',Qout2,' <',eps  
  ...
  if(ins_meshMe .eq. MASTER_PE) write(*,*) 'Q1 in,out=',Qin1, Qout1,Qindout1
  if(ins_meshMe .eq. MASTER_PE) write(*,*) 'Q2 in,out=',Qin2, Qout2,Qindout2
        ...
        do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
           do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
             if(solnData(DFUN_VAR,NGUARD+1,j,k) > 0.0d0) then
              facexData(VELC_FACE_VAR,NGUARD+1,j,k) = facexData(VELC_FACE_VAR,NGUARD+1,j,k)*Qindout1
             else
              facexData(VELC_FACE_VAR,NGUARD+1,j,k) = facexData(VELC_FACE_VAR,NGUARD+1,j,k)*Qindout2
             endif
           enddo
        enddo
      end if
        ...
        do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
           do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
             if(solnData(DFUN_VAR,NGUARD+NXB,j,k) > 0.0d0) then
              facexData(VELC_FACE_VAR,nxc,j,k) = facexData(VELC_FACE_VAR,nxc,j,k)*Qindout1
             else
              facexData(VELC_FACE_VAR,nxc,j,k) = facexData(VELC_FACE_VAR,nxc,j,k)*Qindout2
             endif
           enddo
        enddo
      end if
