...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  real :: dist, divergence,temp,xcell,ycell,u,v,temp2
  integer :: iiii,jjjj
  real, dimension(1,4) :: pinPoint
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
!if (dr_nstep .eq. 10) then
!   !do j=1,blkLimitsGC(HIGH,JAXIS)
!   !   do i=1,blkLimitsGC(HIGH,IAXIS)
!   do j=11,11!NGUARD+1,NGUARD+NYB
!      do i=11,11!NGUARD+1,NGUARD+NXB
!
!         print*,blockID,i,j,"Fin",facexData(VELC_FACE_VAR,i,j,11),faceyData(VELC_FACE_VAR,i,j,11),&
!                  "  ",solnData(PRES_VAR,i,j,11),solnData(DFUN_VAR,i,j,11)
!
!      end do
!   end do
!end if                   
...
Dist = 0.0
     do jjjj=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)   !Without guard cells
        ...
        temp =((facexData(VELC_FACE_VAR,iiii+1,jjjj,1)-facexData(VELC_FACE_VAR,iiii,jjjj,1))/del(IAXIS)+ &
                   (faceyData(VELC_FACE_VAR,iiii,jjjj+1,1)-faceyData(VELC_FACE_VAR,iiii,jjjj,1))/del(JAXIS))
!write(*,*)"sdf",temp
        if (temp .gt. divergence) then
                divergence = temp
!write(*,*)"BINGOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO!!!!!!!!!!!!!!!!!!!!!!!!!!!"
  bsize(:) = boundBox(2,:) - boundBox(1,:)
           ...
           ycell  = centerCoord(JAXIS) - bsize(JAXIS)/2.0 +  &
                   ...
                   0.5*del(JAXIS)
           Dist = solnData(DFUN_VAR,iiii,jjjj,1)
...
enddo
     ...
     call Grid_releaseBlkPtr(blockID,faceyData,FACEY)
     call Grid_releaseBlkPtr(blockID,solnData,CENTER)
if (pinPoint(1,1) .lt. divergence) then
  ...
  call MPI_Allreduce(pinPoint(1,1), temp2, 1, FLASH_REAL, MPI_MAX, MPI_COMM_WORLD, ierr)

if (pinPoint(1,1) .eq. temp2) then
write(*,*)"Location of the cell with max divergence=",pinPoint (1,1)," X=",pinPoint (1,2)," Y=",pinPoint(1,3)," DFUN=",pinPoint(1,4)
endif
