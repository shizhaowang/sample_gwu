...
!  if (ins_meshMe .eq. MASTER_PE)  write(*,*) 'Total IB Time =',ETIB
  ...
  call ins_computeQinout( blockCount, blockList, .true., ins_Qin)
  !write(*,*) 'ins_qIn in FSI:', ins_Qin

  ! Compute DivUstar - delta_mass and print to screen, add to ins_Qin:
  call ins_UstarStats( blockCount, blockList, .true., .true.)

  !call ins_checkStats( blockCount, blockList, DUST_VAR, & 
  !             'rhs of p eq before rescale velocity')

  ! Compute outflow mass volume ratio: (computed on NEUMANN_INS, OUTFLOW_INS)
  call ins_computeQinout( blockCount, blockList, .false., ins_Qout)
  !write(*,*) 'Qout after predict=',ins_Qout
  ...
  do lb = 1,blockCount
     ...
     solnData(DELP_VAR,:,:,:) = 0.0  ! Wang Shizhao for debug
...
!  if (ins_meshMe .eq. MASTER_PE) write(*,*) 'Mean Div Ustar A=',meanPres*(dt*ins_alfa)
  ...
  !call ins_checkStats( blockCount, blockList, DUST_VAR, 'rhs of p eq')
  !call ins_checkStats( blockCount, blockList, DELP_VAR, 'before sol')
  ...
  !write(*,*) 'block limits:', blkLimits
  !write(*,*) 'block limits GC:', blkLimitsGC
  !call ins_checkStats( blockCount, blockList, DELP_VAR, 'after sol')
