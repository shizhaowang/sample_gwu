...
subroutine Driver_evolveFlash()
  ...
  use IO_data , ONLY : IO_plotFileIntervalStep, io_plotFileNumber, IO_plotFileIntervalTime
...
!-----------------------------------------------------------------------------------------
  ...
  call outtoflowvc(dr_globalMe,dr_simtime,dr_dt,dr_nstep,count, &
  ...
  print*, 'return from IncompNS timestep'
     ...
     !--------------------------------------------------------------------
     ! Output to Tecplot
     ! if (MOD(dr_nstep,IO_plotFileIntervalStep) .eq. 0) then
