...
weight_surf = zeros(1,2);
for iel=1:nel
   
   % Integrate on surface:    
   xm = 0.5*(XY(ws_IEN(iel,1),IAXIS:JAXIS)+XY(ws_IEN(iel,2),IAXIS:JAXIS)); 
   
   lm = sqrt( (XY(ws_IEN(iel,2),IAXIS)-XY(ws_IEN(iel,1),IAXIS))^2 + ...
              (XY(ws_IEN(iel,2),JAXIS)-XY(ws_IEN(iel,1),JAXIS))^2 );
   
   sm = 1/lm*(XY(ws_IEN(iel,2),IAXIS:JAXIS) - XY(ws_IEN(iel,1),IAXIS:JAXIS));
   
   nm(IAXIS) = sm(JAXIS); 
   nm(JAXIS) =-sm(IAXIS);
          
   pm = 0. + rhof*dot(grav_vec,xm);
   
   weight_surf = weight_surf + pm*nm(IAXIS:JAXIS)*lm*L;
   
end

disp(['Weight_vol =', num2str(weight_vol,'%10.8f %10.8f') ...
    ', Weight_surf=', num2str(weight_surf,'%10.8f %10.8f')])
disp(['Diff=' num2str(abs(weight_vol-weight_surf),'%10.8f %10.8f')])
...
for jcyl=1:nCyl(JAXIS)
   ...
   for icyl = 1:nCyl(IAXIS)
    ...
    %% Inertia properties:
    massDensity = 1.25; % No mass, all prescribed problem.
    volume      = pi*(D/2)^2 * L;
