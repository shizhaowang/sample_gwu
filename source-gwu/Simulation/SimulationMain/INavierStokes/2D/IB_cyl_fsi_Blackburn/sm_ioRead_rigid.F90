...
!  ndmax = body%max_dofs_per_node
  ...
  ! Dampness, Shizhao Wang, Aug 13, 2014
  allocate(Body%Damp(NDIM+1)) ! [Cx Cy Ctheta]
  Body%Damp = 0.0
  dims = (/NDIM+1,1,1/)
  ! Rigid Body Dampness in x,y,theta:
  CALL h5dopen_f (file, "body/Dampness", dset, h5err)
  call sm_io_checkHdfErr(h5err, 'body/Dampness')
  CALL h5dread_f(dset, H5T_NATIVE_DOUBLE, Body%Damp, dims, h5err)
  CALL h5dclose_f(dset , h5err)
#endif
...
!!$  
  ...
  ! Debug, Shizhao
  Body%Mass = Body%Mass*10.0
  Write(*,*) 'WARNING: DEBUG STATUS********, BODY MASS * 10 ****'
