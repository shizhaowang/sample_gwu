...
!       Load convergence error for the body:        
  ...
  ! Debug, Shizhao Wang
!  if ( time < 50.0 ) then
!    BodyInfo%qn(1:neq) = sin(time)
!    BodyInfo%qdn(1:neq) = cos(time)
!    BodyInfo%qddn(1:neq) = -sin(time)

    !BodyInfo%qn(1:neq) = 1.0 - 0.1*cos(2.0*3.14159265*time)
    !BodyInfo%qdn(1:neq) = 0.1*(2.0*3.14159265)*sin(2.0*3.14151965*time)
    !BodyInfo%qddn(1:neq) = 0.1*(2.0*3.14159265)*cos(2.0*3.14159265*time)
!    integ%pcconvflag  = SM_PCCONVERGED
!  endif

  ! Debug
  !write(*,*) 'qn, after integral:', BodyInfo%qn(1:neq)
  !write(*,*) 'qdn, after integral:', BodyInfo%qdn(1:neq)
  !write(*,*) 'qddn, after integral:', BodyInfo%qddn(1:neq)
