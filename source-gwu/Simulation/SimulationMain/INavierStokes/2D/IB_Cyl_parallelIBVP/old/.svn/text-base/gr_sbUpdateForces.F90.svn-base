!!****if* source/Simulation/SimulationMain/INavierStokes/2D/IB_Cyl_parallelIBVP/old/gr_sbUpdateForces
!!
!! NAME
!!  gr_sbUpdateForces
!!
...
#include "Flash_mpi.h"
  ...
  do b = 1, gr_sbNumBodies
     ...
     if (bodyInfo % myPE == bodyInfo % bodyMaster) then
        ...
        ! only purpose of generating virtual particles for surface marker on the
...
!!$     countf=countf+1
!!$     call int2char(countf,index_count)
!!$     call int2char(gr_meshMe,index_proc)
...
!!$     do i = 1,localCount
!!$        if (int(particles2(TAG_PART_PROP,i)) .eq. 1) then
!!$           write(113,'(2E16.8)') particles2(POSX_PART_PROP,i),particles2(POSY_PART_PROP,i)
!!$        end if
!!$     enddo
!!$     close(113)
!!$
...
!!$#endif
        ...
        do i = 1, localCount
           ...
           if (int(particles2(PROC_PART_PROP,i)) == bodyInfo % bodyMaster) then
              ...
              blkID = int(particles2(BLK_PART_PROP,i))
...
!!$#endif
           ...
           do p = 1, recvCount
              blkID = int(particles2(BLK_PART_PROP,p))
