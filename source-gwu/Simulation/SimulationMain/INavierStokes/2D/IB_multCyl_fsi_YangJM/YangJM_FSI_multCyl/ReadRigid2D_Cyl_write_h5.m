...
%% Cylinder Mesh File:
nCyl_1 = 4;   % Number of Cylinders in the x direction
nCyl_2 = 4;   % Number of Cylinders in the y direction

D  =      1; % Cylinder Diameter
L  =      1; % Unit depth.

n_sections_theta = 128; % Number of sections in circumf direction.
...
D1 = 3.0*D; % Cylinders center to center distance in x
D2 = 3.0*D; % Cylinders center to center distance in y
...
for jcyl=1:nCyl(JAXIS)
   ...
   for icyl = 1:nCyl(IAXIS)
    ...
    %% Inertia properties:
    massDensity = 10.0/(pi*D*D/4.0) %117.10286616255942/(pi*D*D/4.0)  %1.0/(pi*D*D/4.0) %1.; % No mass, all prescribed problem.
    volume      = pi*(D/2)^2 * L;
    mass        = 10.0 %117.10286616255942 %(massDensity)*volume;
    IL          = mass*(D/2)^2/2;    
    I_body      = [IL 0; 0 IL];
              
    kx          = 12.73*pi*pi*pi/25.0 %183.45089177285634 %4.0*pi*pi/25.0  %(pi)^2*mass;
    ky          = kx;
    ktheta      = 0.0 %(2*pi)^2*IL;
    stiff  = [kx ky ktheta];
    
    dampx       = 0.03*pi*pi*12.73/5.0 %0.35176685646273  %0.04*pi/5.0
    dampy       = dampx
    damptheta   = 0.0
    damp        = [dampx dampy damptheta]
    ...
    restnode      = 1;
    nrestcoords   = 2 %3 %2 %4; % All 3 + 1 dofs of node 1 are restrained.
    maxrestparams = 4;
    restnodes     = restnode*ones(1,nrestcoords);
    restdofs      = [3 4] %[1 3 4]  % [3 4] %[1 2 3 4];
    ...
    elseif (strcmp(kinemflag(ibd,:),'yang'))
        ...
        resttype(dof_to_harm)    = SM_PK_HARMONIC;
        % Properties are such that z(t) = At*sin(2*pi*f*t+phase) + fixed_coord
        Ao=0.2; f=0.195*1.2; phase = pi/2; fixed_coord=ypos;
        nparam(dof_to_harm) = 4;
        param(1:4,dof_to_harm) = [Ao f phase fixed_coord]';
    end
    ...
    hdf5write(hfile,'body/Dampness',double(damp),'WriteMode','append')  % Add dampness by Shizhao Wang
