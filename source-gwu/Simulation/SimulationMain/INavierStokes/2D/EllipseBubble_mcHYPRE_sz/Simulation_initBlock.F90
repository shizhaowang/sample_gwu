!!****if* source/Simulation/SimulationMain/INavierStokes/2D/TaylorGreenVortex/Simulation_initBlock
!!
!! NAME
!!
...
#include "Flash.h"
  ...
  !if (mph_meshMe .eq. MASTER_PE .AND. blockID .eq. 1) write(*,*) 'InitBlockTime =',dr_simTime
  ...
  do k=1,blkLimitsGC(HIGH,KAXIS)
     do j=1,blkLimitsGC(HIGH,JAXIS)
        do i=1,blkLimitsGC(HIGH,IAXIS)
          ...
          if     (xcell.ge.0.0 .AND. ycell.ge.0.0) then
             x = xcell - 0.0d0
             y = ycell - 0.0d0

          !- kpd - Initialize the Field in Quadrant II
          elseif (xcell.lt.0.0 .AND. ycell.gt.0.0) then
             x = 0.0d0 - xcell
             y = ycell - 0.0d0

          !- kpd - Initialize the Field in Quadrant III
          elseif (xcell.lt.0.0 .AND. ycell.lt.0.0) then
             x = 0.0d0 - xcell
             y = 0.0d0 - ycell 

          !- kpd - Initialize the Field in Quadrant IV
          elseif (xcell.gt.0.0 .AND. ycell.lt.0.0) then
             x = xcell - 0.0d0
             y = 0.0d0 - ycell
