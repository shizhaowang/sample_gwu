...
#include "constants.h"    
  ...
  real, allocatable, dimension(:) :: qdn_master  ! For damping, Shizhao
  ...
  allocate(qdn_master(body%max_dofs_per_node)); qdn_master=0. ! For damping
  ...
  qdn_master(ix:ew) = body%qdn(body%ID(ix:ew,imaster)) ! For damping, Shizhao

  ! Add dampness vector by Shizhao
  Qint(ix)    = - body%Stiff(IAXIS) * qn_master(ix)  & 
              & - body%Damp(IAXIS)  * qdn_master(ix) * 100
  Qint(ex)    = - body%Stiff(JAXIS) * qn_master(ex) &
              & - body%Damp(JAXIS)  * qdn_master(ex) * 100
  Qint(ia)    = 0.  ! ea = ia = 3 in 2D
  Qint(iw)    = - body%Stiff(ia) * qn_master(ia)  & 
              & - body%Damp(ia)  * qdn_master(ia)
...
!        body%Qs(idx)  = body%Qs(idx) + Qint(idx)
!     endif
!  end do
  ...
  ! debug 
!  write(*,*) 'Stiffness:', body%Stiff
!  write(*,*) 'Dampness:', body%Damp
!  write(*,*) 'Mass:', body%Mass
!  write(*,*) 'Qs(1):', body%Qs(1)
!  write(*,*) 'Qint:', Qint
!  write(*,*) 'qn_master:', qn_master
!  write(*,*) 'qdn_master:', qdn_master

  deallocate(Qint,qn_master)
  deallocate(qdn_master)
