...
#include "IncompNS.h"
  ...
  integer, parameter :: DFUN_IND  =15

  ! Total vars:
  integer, parameter :: TotVars   =15
  ...
  integer, parameter :: NXBMAX = 258
  integer, parameter :: NYBMAX = 258
  ...
  do islicez = 1 , nslicez
     ...
     do lb = 1,blockCount
        ...
        if ( (istartx+GRID_IHI) .gt. NXBMAX) then
             write(*,*) ' '
             write(*,*) 'MyPe=',gr_meshMe,', block',blockID,', max ind X=',(istartx+GRID_IHI),NXBMAX
             call Driver_abortFlash("Max index required for data greater than NXBMAX. Increase NXBMAX.")
        endif
        if ( (istarty+GRID_JHI) .gt. NYBMAX) then
             write(*,*) ' '
             write(*,*) 'MyPe=',gr_meshMe,', block',blockID,', max ind Y=',(istarty+GRID_JHI),NYBMAX
        ...
        do j = GRID_JLO , GRID_JHI
          ...
          do i = GRID_ILO , GRID_IHI
            ...
            data(ii,jj,DFUN_IND)=solnData(DFUN_VAR,i,j,k)
     ...
     if (gr_meshME .eq. MASTER_PE) then
        ...
        ! Write Multiphase constants:
        write(1)  mph_rho1,mph_rho2,mph_sten,mph_vis1,mph_vis2
