...
! --------------------------------------------------------------------------
  ...
  real :: dtrk2, alph1, alph2
  integer :: RK2_FACE_VAR, RK2_VAR
...
!  !!!!OJO
  if (firstcall) then
     do lb = 1,blockCount
        ...
        !solnData(DFUN_VAR,:,:,:) = solnData(DFUN_VAR,:,:,:) + PI/3. ! Lower Dfun by 1
        !facexData(FBAO_FACE_VAR,:,:,:) = 3.*facexData(FBAO_FACE_VAR,:,:,:)
        !faceyData(FBAO_FACE_VAR,:,:,:) = 3.*faceyData(FBAO_FACE_VAR,:,:,:)
        !facezData(FBAO_FACE_VAR,:,:,:) = 3.*facezData(FBAO_FACE_VAR,:,:,:)   
        ! Release pointers:
...
!!$  CALL SYSTEM_CLOCK(TA(1),count_rate)  
  ...
  do lb = 1,blockCount
     ...
     ! Previous steps VELS
     !facexData(VELN_FACE_VAR,:,:,:) = facexData(VELC_FACE_VAR,:,:,:)
     !faceyData(VELN_FACE_VAR,:,:,:) = faceyData(VELC_FACE_VAR,:,:,:)
     !facezData(VELN_FACE_VAR,:,:,:) = facezData(VELC_FACE_VAR,:,:,:)
     ...
     call ins_rhs3d_VD (  facexData(VELC_FACE_VAR,:,:,:),            &
                       ...
                       facezData(VELC_FACE_VAR,:,:,:),            &
                       solnData(TVIS_VAR,:,:,:),                  &
                       facexData(FBAO_FACE_VAR,:,:,:),            & ! Force distribution for
                       faceyData(FBAO_FACE_VAR,:,:,:),            & ! Linear forcing.
                       facezData(FBAO_FACE_VAR,:,:,:),            &
                       ins_invRe,                                 &
                       blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS),&
                       blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS),&
                       blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS),&
...
#ifdef TVD_RK2
 do ii=1,1 ! EULER AS KEEGAN HAD IT 2
#else /* TVD RK3 */
 do ii=1,3
#endif
    ...
    do lb = 1,blockCount
        ...
        ! Store phi for TVD RK2
#ifdef TVD_RK2
        if (ii.eq.1) then
           solnData(AAJUNK_VAR,:,:,:) = solnData(DFUN_VAR,:,:,:)
           RK2_VAR = DFUN_VAR
           RK2_FACE_VAR = VELC_FACE_VAR
           dtrk2=ins_alfa*dt
           alph1 = 0.
           alph2 = 1.
        else
        ...
        call mph_KPDadvectWENO3_3D( solnData(DFUN_VAR,:,:,:), &
                          facexData(RK2_FACE_VAR,:,:,:), &
                          faceyData(RK2_FACE_VAR,:,:,:), &
                          facezData(RK2_FACE_VAR,:,:,:), &
                          dtrk2, &
                          del(DIR_X), &
                          del(DIR_Y), &
                          del(DIR_Z), &
    ...
    do lb = 1,blockCount
       ...
       ! Point to blocks center and face vars:
       call Grid_getBlkPtr(blockID,solnData,CENTER)
       solnData(DFUN_VAR,:,:,:) = alph1*solnData(AAJUNK_VAR,:,:,:) + alph2*solnData(DFUN_VAR,:,:,:)       
       ! Release pointers:
       call Grid_releaseBlkPtr(blockID,solnData,CENTER)        
    end do
