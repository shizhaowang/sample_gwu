...
#include "constants.h"
  ...
  do i = 1,ib_stencil
     ...
     !faceData(FORC_FACE_VAR,ipos,jpos,kpos) = &
     !faceData(FORC_FACE_VAR,ipos,jpos,kpos) + phile(i,CONSTANT_ONE)*zL;
     
     faceData(FORC_FACE_VAR,ielem(i,IAXIS),ielem(i,JAXIS),ielem(i,KAXIS)) = &
     faceData(FORC_FACE_VAR,ielem(i,IAXIS),ielem(i,JAXIS),ielem(i,KAXIS)) + phile(i,CONSTANT_ONE)*zL;
