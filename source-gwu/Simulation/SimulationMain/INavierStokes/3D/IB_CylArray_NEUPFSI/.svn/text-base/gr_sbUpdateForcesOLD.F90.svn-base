...
!           bodyInfo % particles(FUL_PART_PROP,1:localPart) = 0.
!           bodyInfo % particles(FVL_PART_PROP,1:localPart) = 0.
!#if NDIM == MDIM
!           bodyInfo % particles(FWL_PART_PROP,1:localPart) = 0.
!#endif
!        endif
        ...
        do i = 1, totalPart ! In inverse GC fill mode particles are not sorted
           ...
           if (int(bodyInfo % particles(PROC_PART_PROP,i)) == bodyInfo%bodyMaster) then
              ...
              call Grid_updateSolidBodyForces(blockID, particleData)
              !call Grid_updateSolidBodyForces(blockID,b,i)
...
!              bodyInfo % particles(1:NPART_PROPS,ct) = bodyInfo % particles(1:NPART_PROPS,i)
#endif
...
!#endif           
           ...
           do p = 1, recvCount
              ...
              !call Timers_start("Grid_updateSolidBodyForces") 
              call Grid_updateSolidBodyForces(blockID, particleData)
!              call Grid_updateSolidBodyForces(blockID,b,p)
              !call Timers_stop("Grid_updateSolidBodyForces") 
...
!     write(*,*) gr_meshMe,'Body=',b,'Nparticles=',val,fy,fz
!     if (gr_meshMe .eq. MASTER_PE) write(*,*) ' ' 
...
!#endif
 ...
 if(gr_meshMe .eq. MASTER_PE) write(*,*) 'MAX Particles_Allbods=',Particles_Allbods
