!!****if* source/Simulation/SimulationMain/INavierStokes/3D/kientzler_bubbleFS_caseB/KPD_outtotecplot
!!
!! NAME
!!
!!  outtotecplot
...
#include "Flash.h"
  ...
  do lb = 1,blockcount
     ...
     ! Dfun: dfun(NXB+1,NYB+1,NZB+1)
     ! -------------------------------
     call centervals2corners(NGUARD,NXB,NYB,NZB,nxc,nyc,nzc, &
          solnData(DFUN_VAR,:,:,:),tpdfun)


     ! p: p(NXB+1,NYB+1,NZB+1)
     ! -------------------------------
     call centervals2corners(NGUARD,NXB,NYB,NZB,nxc,nyc,nzc, &
                            solnData(PRES_VAR,:,:,:),tpp)


     ! dens : dens(NXB+1,NYB+1,NZB+1)
     ! -------------------------------
     call centervals2corners(NGUARD,NXB,NYB,NZB,nxc,nyc,nzc,&
                            solnData(DENS_VAR,:,:,:),tpdens)

     ! visc : visc
     ! ----------
     call centervals2corners(NGUARD,NXB,NYB,NZB,nxc,nyc,nzc,&
                            solnData(VISC_VAR,:,:,:),tpvisc)
