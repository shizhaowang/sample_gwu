!!****if* source/Simulation/SimulationMain/INavierStokes/3D/IB_sphere_mcHYPRE_VD_wFS/ImBound_init
!!
!! NAME
!!
!!  ImBound_init
...
!#endif
     ...
     do i=1,NumVertices
        ...
        gr_sbBodyInfo(b)%yb(i) = yo(b) + gr_sbBodyInfo(b)%xbus(i)*sin(tita) + gr_sbBodyInfo(b)%ybus(i)*cos(tita) &
             + ao*sin(2.*PI*freq_t*dr_simTime) 
!#if NDIM == 3
...
!#endif
        ...
        gr_sbBodyInfo(b)%vbd(i) =  omg*gr_sbBodyInfo(b)%xbus(i)*cos(tita) - omg*gr_sbBodyInfo(b)%ybus(i)*sin(tita) &
        ...
        gr_sbBodyInfo(b)%vbdd(i)= -omg**2.*gr_sbBodyInfo(b)%xbus(i)*sin(tita) - omg**2.*gr_sbBodyInfo(b)%ybus(i)*cos(tita) &
     ...
     ! Write .dat file:
     open(unit=113,file='./IOData/sphere_coarse_altered.surf',form='formatted')
