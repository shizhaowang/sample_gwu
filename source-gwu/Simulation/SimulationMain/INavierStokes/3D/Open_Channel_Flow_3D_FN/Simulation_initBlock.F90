...
!#################################################
  ...
  do k=1,blkLimitsGC(HIGH,KAXIS)
     ...
     !           -0.02734*etha_x**2 +0.3404*etha_x -0.00006472
        ...
        faceyData(VELC_FACE_VAR,i,j,k) =-1.0*(0.5*sqrt(0.0045871/(xcell+x_blas))*(etha_y*temp-temp2))
        else
                temp =0.0
                temp2=0.0
        ...
        faceyData(VELC_FACE_VAR,i,j,k)=-1.0*(0.5*sqrt(0.0045871/(xcell+x_blas))*(8.0*temp-temp2))
       endif
        endif
        enddo
     enddo
...
!  faceyData(VELC_FACE_VAR,:,:,:) = 0.0
  ...
  facezData(VELC_FACE_VAR,:,:,:) = 0.0          ! w velocity
