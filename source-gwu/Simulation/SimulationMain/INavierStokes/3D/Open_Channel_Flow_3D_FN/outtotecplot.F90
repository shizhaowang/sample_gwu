...
!        count, mype
  ...
  i = TecIni('AMR3D'//NULLCHR,                            &
             'x y z u v w dfun p wx wy wz Q div'//NULLCHR,  &
             filename//NULLCHR,                           &
             './IOData/'//NULLCHR,                  &
             Debug,VIsdouble)
...
!########################################################################
 ...
 ! do kkkk=1,blkLimits(HIGH,KAXIS)
   !     do iiii=1,blkLimits(HIGH,IAXIS)
        !  do jjjj=1,blkLimits(HIGH,JAXIS)

       !    xxcell = coord(IAXIS) - bsize(IAXIS)/2.0 +   &
      !             real(iiii - NGUARD - 1)*del(IAXIS) !+   &
                 !  0.5*del(IAXIS)

     !      if (xxcell .le. minX) then
    !             minX = xxcell
   !        endif
