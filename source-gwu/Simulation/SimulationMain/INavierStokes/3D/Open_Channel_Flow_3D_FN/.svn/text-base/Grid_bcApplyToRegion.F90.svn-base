...
#include "IncompNS.h"
  ...
  integer :: ia,ib,ja,jb,ka,kb,counter
           ...
           case(SLIP_INS)
              ...
              if (isFace) then ! Set to zero velocities normal to boundary, up to boundary
                 ...
                 elseif((ivar==RH1F_FACE_VAR).OR.(ivar==RH2F_FACE_VAR).OR.(ivar==MGW8_FACE_VAR)) then
                   k = 2*guard+1
                   do i = 1,guard
                     regionData(i,1:je,1:ke,ivar) = regionData(guard+1,1:je,1:ke,ivar)    !1to4 2to4 3to4 
                   end do
                 !------------------------------------------------------------------------------------------------------------
                 endif
...
!                   if (je .gt. NGUARD) then
              ...
              if (isFace) then ! Set to zero velocities normal to boundary, up to boundary  *********
                 ...
                 !  if (je .gt. NGUARD) then !WW

     !   write(*,*)"je= ",je,"  NGUARD= ",NGUARD
                      do kkk=1,ke
                         do jjj=1,je
                         !  do i=1,guard+1
                              ycell  = coord(JAXIS) - bsize(JAXIS)/2.0   +  &
 ...
 regionData(NGUARD+1,jjj,kkk,ivar)=-0.0000000244*ycell**8-0.000001707*ycell**7-0.00004592*ycell**6-0.0006009*ycell**5-0.003817*ycell**4-0.009313*ycell**3-0.01206*ycell**2-0.2261*ycell-0.00006572
  ...
  ! Use guardcells to set to zero normal gradients of velocities not normal to
  ! boundary, at boundary
              elseif ((gridDataStruct==FACEY).and.(axis==IAXIS)) then   ! ***************
                 k = 2*guard+1   
                 if(ivar == VELC_FACE_VAR) then !EE
                      ...
                      do kkk=1,ke
                       ...
                       !    do i=1,guard+1
                              ycell  = coord(JAXIS) - bsize(JAXIS)/2.0   +  &
                               ...
                               yycell = ycell - 0.5*del(JAXIS)
                               ! write(*,*)'Ycell=', ycell
                              if (yycell .ge. 0.) then  !  GGG
                               V = 0
                             elseif ((yycell .le. 0) .and. (yycell .ge. -12.048)) then !GGGG
V = -1*(-0.000000002669*yycell**7 -0.0000001306*yycell**6 -0.000002456*yycell**5 -0.00002126*yycell**4 -0.00007273*yycell**3 +0.000003264*yycell**2 -0.00006112*yycell-0.000001492)
                              ...
                              elseif (yycell .le. -12.048) then ! GGGGG
                              V= -0.00272009
     ...
     !     end if                  ! $$$$$$$$$
                 !------------------------------------------------------------------------------------------
                 elseif((ivar==RH1F_FACE_VAR).OR.(ivar==RH2F_FACE_VAR).OR.(ivar==MGW8_FACE_VAR)) then !EEEEE
                   k = 2*guard+1
                   do i = 1,guard
                     regionData(i,1:je,1:ke,ivar) = regionData(guard+1,1:je,1:ke,ivar)    !1to4 2to4 3to4
                   end do
...
elseif ((gridDataStruct==FACEZ).and.(axis==IAXIS)) then !***************
                 if(ivar == VELC_FACE_VAR) then  !ZZZZ

!write(*,*)"Ke= ",ke," Je= ",je

      !               if (je .gt. NGUARD) then ! ADDED COPYING OFF ABOVE
                        do i=1,NGUARD
                                do kkk=1,ke
                 !               write(*,*)"THIS SHOULD BE 16x3=48"
                                        do jjj=1,je
                               regionData(i,jjj,kkk,ivar)=-regionData(2*NGUARD+1-i,jjj,kkk,ivar)
                        end do
                enddo
                enddo
       !                 endif ! for :  je .gt. NGUARD
                      elseif((ivar==RH1F_FACE_VAR).OR.(ivar==RH2F_FACE_VAR).OR.(ivar==MGW8_FACE_VAR)) then !ZZZZZ
                   k = 2*guard+1
                   do i = 1,guard
                     regionData(i,1:je,1:ke,ivar) =regionData(guard+1,1:je,1:ke,ivar)    !1to4 2to4 3to4
                   end do
           ...
           case(SLIP_INS)
              ...
              if (isFace) then ! Set to zero velocities normal to boundary, up to boundary
                 ...
                 elseif((ivar==RH1F_FACE_VAR).OR.(ivar==RH2F_FACE_VAR).OR.(ivar==MGW8_FACE_VAR)) then
                   k = 2*guard+1
                   do i = 1,guard
                     regionData(k-i,1:je,1:ke,ivar)= regionData(guard+1,1:je,1:ke,ivar)     !6to4 5to4 3to4
                   end do
                 !------------------------------------------------------------------------------------------------------------
                 endif
              else             ! Use guardcells to set to zero velocities not normal to boundary, at boundary
