...
#include "ImBound.h"
  ...
  use tree, only : grid_changed, lrefine, surr_blks
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  real, dimension(MDIM) :: centerCoord
  real :: dist,divergence,temp,xcell,ycell,zcell,u,v,w,temp2,velcoeff,xcell_vel,ycell_vel,zcell_vel,temp3
  integer :: iiii,jjjj,kkkk
  real, dimension(1,5) :: pinPoint
  real, dimension(1,7) :: pinPoint_VelCoeff
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!     blockID = blockList(lb)
    do lb = 1,count
     ...
     !if (ins_nstep .eq. 1)print*,"KPD ins_ab2rk3 bID=",blockID,"blockCount=",blockCount,coord(1),coord(2)
     !----------------------------------------------------------
...
!!***********************************************************************************************
!  !--------------------------------------------------------------------------
!  !- kpd - FORCE CONSISTENCY Testing Purposes ONLY... 
!  ! -------------------------------------------------------------------------
...
#if NDIM==3
     do k=GRID_KLO_GC/2+1,GRID_KHI_GC
        do j=GRID_JHI_GC,GRID_JHI_GC
           do i=GRID_ILO_GC,GRID_IHI_GC
...
!                       del(DIR_X),del(DIR_Y),del(DIR_Z),newu,newv,neww )
     ...
     if (ABS(del(DIR_X)-del(DIR_Y)) .gt. 1e-8 .OR. &
         ABS(del(DIR_X)-del(DIR_Z)) .gt. 1e-8 .OR. &
         ABS(del(DIR_Y)-del(DIR_Z)) .gt. 1e-8 ) then
     ...
     if (ABS(del(DIR_X)-del(DIR_Y)) .gt. 1e-8 ) then
...
if (ins_meshMe .eq. 0) print*,"KPD PROBLEM IN ins_ab2rk3.F90 with Refinement Update Midway?!?",grid_changed
!KPD  temp_grid_changed  = grid_changed
!KPD  if (ist .eq. 1) then
!KPD  call Timers_start("Grid_updateRefinement")
!KPD  call Grid_updateRefinement( ins_nstep, timeEndAdv ,gridChanged)
!KPD  call Grid_getListOfBlocks(LEAF,blockList,blockCount)
!KPD  call Timers_stop("Grid_updateRefinement")
!KPD  !  ! Write Ustar:
!KPD  ! call outtotecplot_uv(ins_meshMe,ins_tlevel,dt,ins_nstep,100, &
!KPD  !                      0.0,blockList,blockCount,1,1.)
!KPD  endif
!KPD  grid_changed  = temp_grid_changed
...
if (ins_meshMe .eq. 0) print*,"Did the Grid Change Going Into Grid_solvePoisson?",grid_changed
  poisfact = 1.0 
  call Grid_solvePoisson (DELP_VAR, DUST_VAR, bc_types, bc_values, poisfact) 
if (ins_meshMe .eq. 0) print*,"Did the Grid Change Coming Out Grid_solvePoisson?",grid_changed
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
! Finding the cell  with max divergence %%%%%%
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  call Grid_getDeltas(blockID,del)
  call Grid_getBlkBoundBox(blockId,boundBox)
  call Grid_getBlkCenterCoords(blockId,centerCoord)
  call Grid_getBlkPtr(blockID,facexData,FACEX)
  call Grid_getBlkPtr(blockID,faceyData,FACEY)
  call Grid_getBlkPtr(blockID,facezData,FACEZ)
  call Grid_getBlkPtr(blockID,solnData,CENTER)
...
do kkkk = blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
     do jjjj=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)   !Without guard cells

        do iiii=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
        temp =((facexData(VELC_FACE_VAR,iiii+1,jjjj,kkkk)-facexData(VELC_FACE_VAR,iiii,jjjj,kkkk))/del(IAXIS)+ &
                   (faceyData(VELC_FACE_VAR,iiii,jjjj+1,kkkk)-faceyData(VELC_FACE_VAR,iiii,jjjj,kkkk))/del(JAXIS)+ &
                       (facezData(VELC_FACE_VAR,iiii,jjjj,kkkk+1)-facezData(VELC_FACE_VAR,iiii,jjjj,kkkk))/del(KAXIS) )

        temp2 = MAX( ABS(facexData(VELC_FACE_VAR,iiii,jjjj,kkkk))/del(IAXIS), &
                   ABS(faceyData(VELC_FACE_VAR,iiii,jjjj,kkkk))/del(JAXIS), &
                   ABS(facezData(VELC_FACE_VAR,iiii,jjjj,kkkk))/del(KAXIS) )       


         if (abs(temp) .gt. abs(divergence)) then
                divergence = temp
  bsize(:) = boundBox(2,:) - boundBox(1,:)
           xcell = centerCoord(IAXIS) - bsize(IAXIS)/2.0 +   &
                   real(iiii - NGUARD - 1)*del(IAXIS) +   &
                   0.5*del(IAXIS)

           ycell  = centerCoord(JAXIS) - bsize(JAXIS)/2.0 +  &
                   real(jjjj - NGUARD - 1)*del(JAXIS)  +  &
                   0.5*del(JAXIS)

           zcell  = centerCoord(KAXIS) - bsize(KAXIS)/2.0 +  &
                   real(kkkk - NGUARD - 1)*del(KAXIS)  +  &
                   0.5*del(KAXIS)

           Dist = solnData(DFUN_VAR,iiii,jjjj,kkkk)


endif

        if (temp2 .gt. velcoeff) then
                velcoeff = temp2
        u = facexData(VELC_FACE_VAR,iiii,jjjj,kkkk)
        v = faceyData(VELC_FACE_VAR,iiii,jjjj,kkkk)
        w = facezData(VELC_FACE_VAR,iiii,jjjj,kkkk)
  bsize(:) = boundBox(2,:) - boundBox(1,:)
           xcell_vel = centerCoord(IAXIS) - bsize(IAXIS)/2.0 +   &
                   real(iiii - NGUARD - 1)*del(IAXIS) +   &
                   0.5*del(IAXIS)

           ycell_vel  = centerCoord(JAXIS) - bsize(JAXIS)/2.0 +  &
                   real(jjjj - NGUARD - 1)*del(JAXIS)  +  &
                   0.5*del(JAXIS)

           zcell_vel  = centerCoord(KAXIS) - bsize(KAXIS)/2.0 +  &
                   real(kkkk - NGUARD - 1)*del(KAXIS)  +  &
                   0.5*del(KAXIS)
...
enddo
     ...
     call Grid_releaseBlkPtr(blockID,solnData,CENTER)
if (abs(pinPoint(1,1)) .lt. abs(divergence)) then

        pinPoint (1,1) = divergence
        pinpoint (1,2) = xcell
        pinPoint (1,3) = ycell
        pinPoint (1,4) = zcell
        pinPoint (1,5) = Dist
...
if (pinPoint_VelCoeff(1,1) .lt. velcoeff) then

        pinPoint_VelCoeff (1,1) = velcoeff
        pinpoint_VelCoeff (1,2) = xcell_vel
        pinPoint_VelCoeff (1,3) = ycell_vel
        pinPoint_VelCoeff (1,4) = zcell_vel
        pinPoint_VelCoeff (1,5) = u
        pinPoint_VelCoeff (1,6) = v
        pinPoint_VelCoeff (1,7) = w
endif

  enddo

  call MPI_Allreduce(abs(pinPoint(1,1)), temp3, 1, FLASH_REAL, MPI_MAX, MPI_COMM_WORLD, ierr)
!   call MPI_MAX(abs(pinPoint(1,1)), temp3, 1, FLASH_REAL, MPI_MAX,MPI_COMM_WORLD, ierr)

if (abs(pinPoint(1,1)) .eq. abs(temp3)) then
write(*,*)"Location of the cell with max divergence=",pinPoint (1,1)," X=",pinPoint (1,2)," Y=",pinPoint(1,3)," Z=",pinPoint(1,4)," DFUN=",pinPoint(1,5)
endif
!if (abs(mxdivv) .ge. 0.0000001) then
!        write(*,*)"========================================> HOLY CRAP"
!endif


  call MPI_Allreduce(pinPoint_VelCoeff(1,1), temp3, 1, FLASH_REAL,&
                     MPI_MAX, MPI_COMM_WORLD, ierr)

if (pinPoint_VelCoeff(1,1) .eq. temp3) then
write(*,*)"Location of the cell imposing CFL condition. dt=",0.5*(0.1/pinPoint_VelCoeff(1,1)), &
" X=",pinPoint_VelCoeff (1,2)," Y=",pinPoint_VelCoeff(1,3)," Z=",pinPoint_VelCoeff (1,4),      &
" u=",pinPoint_VelCoeff (1,5)," v=",pinPoint_VelCoeff(1,6)," w=",pinPoint_VelCoeff (1,7)
end if
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
  ...
  !----------------------------------------------------------------------------------------------------
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!if (pinPoint(1,1) .eq. mxdivv) then
!write(*,*)"Location of the cell with max divergence=",pinPoint (1,1),"X=",pinPoint (1,2)," Y=",pinPoint(1,3)
!endif
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
