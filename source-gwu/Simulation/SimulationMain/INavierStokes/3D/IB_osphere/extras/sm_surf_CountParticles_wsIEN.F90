...
subroutine sm_surf_CountParticles_wsIEN(ibd, numPart)
  ...
  use SolidMechanics_data, only : sm_BodyInfo, sm_Dmin
  use Grid_data, only : gr_minCellSizes
  use Grid_interface, ONLY : Grid_getMinCellSize
  use Driver_interface, only : Driver_abortFlash
  use sm_element_interface, only: sm_el02_countParticles, sm_el10_countParticles
  implicit none
  ...
  Dmin = sm_Dmin
  write(*,*) 'In sm_surf_CountParticles_wsIEN, Dmin=',Dmin
  ...
  if (sm_bodyinfo(ibd)%map_use_dmin .eq. SM_TRUE) then
    ...
    Dmin = maxval(gr_minCellSizes(IAXIS:KAXIS))
  endif
#endif
