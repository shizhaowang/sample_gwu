...
!#include "MHD.h"
  ...
  real, parameter :: refineTime = 0.8154*PI
...
#define SPECIAL_REFINEMENT 1
  ...
  if ((dr_simTime+dr_dt/2.) .ge. refineTime) then

  !! Call for the specialized refinement
  specsSize=7
  !! Coordinate information --------------------------------------
  !! define a range of coordinates of the rectangle in x-direction
  specs(1) =  -1.0  + 0.005 ! sim_xMin + 0./4.*(sim_xMax - sim_xMin) +.005
  specs(2) =   1.0  - 0.005 !sim_xMax -.005

  !! define a range of coordinates of the rectangle in y-direction
  specs(3) =  -1.0  + 0.005 !sim_yMin + 2./4.*(sim_yMax - sim_yMin) +.005
  specs(4) =   1.0  - 0.005 !sim_yMin + 4./4.*(sim_yMax - sim_yMin) -.005

  !! define a range of coordinates of the rectangle in z-direction
  specs(5) =  -2.0  + 0.005 !sim_zMin + 1./4.*(sim_zMax - sim_zMin) +.05
  specs(6) =   2.0  - 0.005 !sim_zMin + 3./4.*(sim_zMax - sim_zMin) -.05
