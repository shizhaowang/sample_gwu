...
%WSZ ?? RB_IDENTITY and RB_QUATERNN
RB_IDENTITY = 1000;
RB_EULER321 = 1321;
RB_QUATERNN = 1111;

%WSZ The "ANN" indicates "analytical"
%WSZ ?? Do the numbers come from the h5 format
RB_ANNSPHERE = 55;
RB_ANNDISC   = 56;
RB_ANNRBC    = 57;
...
% Output File names:
basedir_out = ['/Users/mvanella/Dropbox/Documents/ADAPTIVE/CHRIS_OSCCYL/'];
...
%% Cylinder Mesh File:
cyldir = IAXIS; % Cylinder direction. Direction 1.

num_seg= 1; % Number of segments per cylinder
nCyl_1 = num_seg;
...
L_t= 0.0625*D; % amr =0.625*D, ug =0.0625
...
n_sections_L     =   1; % 8 for amr; 1 for ug % Number of points in the length
  ...
  for jcyl=1:nCyl(JAXIS)
   ...
   for iseg = 1:nCyl(IAXIS)
    ...
    elseif (strcmp(kinemflag(ibd,:),'rt_x'))
        ...
        phio = 0; phido=-5; param(1:2,IPHI) = [phio phido]; %SO OMEGA = 5 -> alpha=OMG*D/(2*Uoo) = 2.5        
