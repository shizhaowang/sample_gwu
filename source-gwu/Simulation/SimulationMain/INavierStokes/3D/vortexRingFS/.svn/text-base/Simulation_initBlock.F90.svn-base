!!****if* source/Simulation/SimulationMain/INavierStokes/3D/kientzler_bubbleFS_caseA/Simulation_initBlock
!!
!! NAME
!!
...
#include "Flash.h"
  ...
  real :: xo, yo, zo, rdx, rdy, rdz
  real :: x, y, z, a, b, c
  real :: tsolve, t, ct, st, x1, y1, z1, nx, ny, nz
  integer :: iters
  ...
  !if (mph_meshMe .eq. 0) write(*,*) 'InitBlockTime =',dr_simTime
  ...
  do k=1,blkLimitsGC(HIGH,KAXIS)
     do j=1,blkLimitsGC(HIGH,JAXIS)
        do i=1,blkLimitsGC(HIGH,IAXIS)
          ...
          !- kpd - Initialize the distance function to the spherical bubble
!          solnData(DFUN_VAR,i,j,k) = (1. - sqrt( ((xcell-xo)/rdx)**2 + &
!                                                 ((ycell-yo)/rdy)**2 + &
!                                                 ((zcell-zo)/rdz)**2 ) )

          solnData(DFUN_VAR,i,j,k) = rdx - (sqrt( (xcell-xo)**2.0 + &
                                                  (ycell-yo)**2.0 + &
                                                  (zcell-zo)**2.0 ))

!          if (ycell .GE. 0.0) then
             solnData(DFUN_VAR,i,j,k) = ycell
!          end if
!
!!         if (ycell .LT. 0.0 .AND. solnData(DFUN_VAR,i,j,k) .LT. 0.0) then
!            !solnData(DFUN_VAR,i,j,k) = MAX(ycell,(rdx - (sqrt( (xcell-xo)**2.0 + &
!            !                                     (ycell-yo)**2.0 + &
!             !                                     (zcell-zo)**2.0 ))))
!             solnData(DFUN_VAR,i,j,k) = (rdx - (sqrt( (xcell-xo)**2.0 + &
!                                                  (ycell-yo)**2.0 + &
...
!print*,"DFUN 11,11,11: ",solnData(DFUN_VAR,11,11,11)
