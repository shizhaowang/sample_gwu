...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  ! Sphere test vars:
  real :: xp,yp,zp,dist,xcell,xedge,ycell,yedge,zcell,zedge
  real :: xbod,ybod,zbod,ubod,vbod,wbod,zbodi,wbodi
  integer :: idx,upt,vpt,wpt
...
!!! Set spheres internal points (not forced) to sphere veloc:

#define INTERNALFORCE_SPHERE 1

#ifdef INTERNALFORCE_SPHERE
#if NDIM == MDIM 

  ! Spheres center Positions and velocities: 
  xbod = 0.
  ubod = 0.
  ybod = 0.
  vbod = 0.
  ...
  if (ins_meshMe .eq. sm_BodyInfo(1)%BodyMaster) then
      idx  = sm_BodyInfo(1)%ID(3,1) ! eq number for z of sphere center (point number 1)
      zbodi = sm_bodyInfo(1)%qn(idx)
      wbodi = sm_bodyInfo(1)%qdn(idx)
  endif

  ! Master Value goes to all:
  call mpi_allreduce ( zbodi, zbod, 1, MPI_DOUBLE_PRECISION, &
                       MPI_SUM, MPI_COMM_WORLD, ierr )
  call mpi_allreduce ( wbodi, wbod, 1, MPI_DOUBLE_PRECISION, &
                       MPI_SUM, MPI_COMM_WORLD, ierr )

  if (ins_meshMe .eq. MASTER_PE) then
      write(*,*) 'SPHERE CENTER POS=',xbod,ybod,zbod
      write(*,*) 'SPHERE CENTER VEL=',ubod,vbod,wbod
  endif


  ! Force Eulerian velocities inside Sphere:
  upt = 0; vpt = 0; wpt = 0;
  do lb = 1,blockCount
     blockID = blockList(lb)
     ...
     do k=blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)
        ...
        do j=blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
           ...
           do i=blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)
              ...
              ! U velocity:
              if (abs(facexData(FORC_FACE_VAR,i,j,k)) .lt. 1.e-12) then
                 xp = xedge
                 yp = ycell
                 zp = zcell
                 dist = sqrt((xp-xbod)**2. + (yp-ybod)**2. + (zp-zbod)**2.)
                 if (dist .lt. 0.5) then ! U Point inside the sphere - set uvel to ubod 
                    facexData(VELC_FACE_VAR,i,j,k) = ubod
                    upt = upt + 1
                 endif
              endif
              ! V velocity:
              if (abs(faceyData(FORC_FACE_VAR,i,j,k)) .lt. 1.e-12) then
                 xp = xcell
                 yp = yedge
                 zp = zcell
                 dist = sqrt((xp-xbod)**2. + (yp-ybod)**2. + (zp-zbod)**2.)
                 if (dist .lt. 0.5) then ! V Point inside the sphere - set vvel to vbod 
                    faceyData(VELC_FACE_VAR,i,j,k) = vbod
                    vpt = vpt + 1
                 endif
              endif
              ! W velocity:
              if (abs(facezData(FORC_FACE_VAR,i,j,k)) .lt. 1.e-12) then
                 xp = xcell
                 yp = ycell
                 zp = zedge
                 dist = sqrt((xp-xbod)**2. + (yp-ybod)**2. + (zp-zbod)**2.)
                 if (dist .lt. 0.5) then ! W Point inside the sphere - set wvel to wbod 
                    facezData(VELC_FACE_VAR,i,j,k) = wbod
                    wpt = wpt + 1
                 endif
              endif
           enddo
  ...
  ! Points forced beast approach:
  call mpi_allreduce ( upt, i, 1, MPI_INTEGER, &
                       MPI_SUM, MPI_COMM_WORLD, ierr )
  call mpi_allreduce ( vpt, j, 1, MPI_INTEGER, &
                       MPI_SUM, MPI_COMM_WORLD, ierr )
  call mpi_allreduce ( wpt, k, 1, MPI_INTEGER, &
                       MPI_SUM, MPI_COMM_WORLD, ierr )

  !write(*,*) 'MyProc,pts=',ins_meshMe,upt,vpt,wpt
  if (ins_meshMe .eq. MASTER_PE) write(*,*) 'Forced Points u,v,w=',i,j,k
