...
subroutine gr_markVarThreshold_KPDvortLS (Var, var_th, var_th2,lref)
...
if ( ( blockCenter(2) .LT. 0.0 .AND. blockCenter(1) .GT. -0.5 .AND.  &
(maxval(unk(var,NGUARD+1:NXB+NGUARD,NGUARD+1:NYB+NGUARD,NGUARD+1:NZB+NGUARD,b)) > var_th .OR. &
  minval(unk(var,NGUARD+1:NXB+NGUARD,NGUARD+1:NYB+NGUARD,NGUARD+1:NZB+NGUARD,b)) < var_th2) ) &
.OR. &
( maxval(unk(DFUN_VAR,:,:,:,b)) > -0.2 .AND. minval(unk(DFUN_VAR,:,:,:,b)) < 0.2 ) ) then
...
!-------------------------------------------------------------------------------
  ...
  return
end subroutine gr_markVarThreshold_KPDvortLS
