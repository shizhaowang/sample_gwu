...
#include "paramesh_preprocessor.fh"
      ...
      Subroutine mpi_amr_1blk_restrictFaces(mype,iopt,lcc,lfc,lec,lnc,      & 
                                       lfulltree,filling_guardcells,ilevel)
...
!-----Input/Output Arguments
      Integer, Intent(in)  :: mype,iopt,ilevel
...
!-----Begin Executable Code
      ...
      !!- kpd - Test loop to double check nodes/procs/nodetypes to
      !!           check mpi runs, and addressing for remote procs
      !do lb=1,lnblocks
      !   print*,"Nodes",gr_meshMe,lb,"nodetype",nodetype(lb)
      !   if (lb .eq. 37 .AND. gr_meshMe .eq. 0) then 
      !      print*,"ADDRESS",laddress(1,9998),laddress(1,9999)
      !   end if
      !end do
...
!-----Cycle through parents in decreasing order of refinement
      ...
      !-----------------------------------------
      !-----------------------------------------
      !- kpd - Cycle to coarser rstricted level
      !-----------------------------------------
      !- kpd - Original criterion: Do level = llrefine_max-1,llrefine_min,-1
      Do level = ilevel-1,ilevel-1
      !-----------------------------------------
      !-----------------------------------------
      !-----------------------------------------
      ...
      ledge     = .False.
      lrestrict = .True.
      !- kpd - Sets up cross block communication
      ...
      !print*,"Entering mpi_amr_1blk_restrict.F90 with iopt",iopt,lb,level, &
      !   nodetype(lb),nodetype_save(lb),lrefine(lb)

      !- kpd - lrefine(lb) is the level a block is on
      !        "leaf, child block has nodetype" = 1
      !        All level 4's: 5,6,7,8,10,...   have nodetype = -1
      !        All level 3's: 3,4,9,14,20...   have nodetype = 1
      !        All level 2's: 2,19,36,53       have nodetype = 2
      !        All level 1's: 1                have nodetype = 3
...
!------------------------------------------------------
      ...
      !---------------------------------------------------------------------------
      !- kpd - Original BAD criterion...
      !If ((nodetype(lb) == 2 .and. lrefine(lb) == level) .or.          & 
      !    (nodetype(lb) == 2 .and. filling_guardcells)) Then

      !-----------------------------------------------------------------------------------
      !- kpd- New criterion to loop through non-leaf blocks on coarse restriction level... 
      !       This may need to be updated for greater levels
      !-----------------------------------------------------------------------------------
      !   Not a leaf block              block on coarser level
      If (nodetype_save(lb) .GT. 1 .and. lrefine(lb) == level) Then 
      ...
      Do ich=1,nchild
        ...
        remote_pe     = child(2,ich,lb)   ! Child processor #
        remote_block  = child(1,ich,lb)   ! Child Block #
        ...
        If (remote_pe .ne. mype) Then
          ...
          !print*,"------------------------"
          !print*,"Entering remote proc add",lb,remote_pe,remote_block,ladd_strt(remote_pe),ladd_end(remote_pe)
          !print*,"------------------------"

          Do While(.not.lfound .and. iblk <= ladd_end(remote_pe))
            !print*,"MPI block criterion",iblk,laddress(1,iblk),laddress(2,iblk)
        ...
        !!---------------------------------------------------------
        !!- kpd - Check the stencil and procs for a Parent block...
        !!---------------------------------------------------------
        !if (lb .eq. 40 .AND. gr_meshMe .eq. 0) then
        !   print*,"Stencil0",gr_meshMe,lb,ich,remote_block,remote_pe,cnodetype,cempty
        !end if

        !- kpd - This is Where the new Proc gets DROPPPED out, it DOESNOT meet cnodetype <= 2
...
!******************************************************************************
         ...
         !!------------------------------------------------------------------------
         !!- kpd - Test loops to ensure addressing for cross processor blocks is OK
         !!------------------------------------------------------------------------
         !if (lb .eq. 37) then
         !   !print*,"Stencil3",gr_meshMe,lb,ich,remote_block,remote_pe,cnodetype,cempty
         !end if
         if (dr_nstep .eq. 115) then
            i1 = 1 + nguard
            i2 = nxb + nguard
            j1 = 1 + k2d*nguard
            j2 = nyb + k2d*nguard
            !print*,"Stencil",gr_meshMe,lb,ich,child(1,ich,lb),child(2,ich,lb),remote_block,remote_pe
            do j=j1,j2
               do i=i1,i2
                  if (gr_meshMe .eq. 0) then
                     !print*,"St2",gr_meshMe,lb,ich,i,j,facevarx1(1,i,j,1,1)
                  end if
               end do
            end do
         end if
         ...
         !- kpd - Actual restriction (interpolation) is done here...
         !        -> facevarx1 is face centered data to restrict
         !        -> tempf is restricted data
         Call amr_restrict_fc_fun(facevarx1(:,:,:,:,1),tempf,1)

         !- kpd - ivar is the # of face variables. Loops through parent blocks
...
!                   if(lb .eq. 40 .AND. ivar .eq. 2) print*,"SENDF",ivar,lb,i,j,"-->",ii,jj,sendf(ivar,ii,jj,kk)
                 End If
               End Do
             End Do
           End Do
         ...
         Do k=1,nzb+(-nzb/2)*k3d
           Do j=1,nyb+(-nyb/2)*k2d
             Do i=1,nxb-nxb/2+1
               Do ivar=1,nfacevar
                 If (int_gcell_on_fc(1,ivar)) Then
                   If (curvilinear_conserve) Then !- kpd - FALSE
...
!                if(lb .eq. 40 .AND. ivar .eq. 2) print*,"InRestriction",lb,i+nguard,j+nguard*k2d,"-->", &
!                       i+nguard0+ioff,&
!                       j+nguard0*k2d+joff,k+nguard0*k3d+koff, &
!                       facevarx(ivar,i+nguard0+ioff,j+nguard0*k2d+joff,k+nguard0*k3d+koff,lb)
         ...
         Do k=1,nzb+(-nzb/2)*k3d
           Do j=1,nyb+(-nyb/2+1)*k2d
             Do i=1,nxb-nxb/2
               Do ivar=1,nfacevar
                 If (int_gcell_on_fc(2,ivar)) Then
                 If (curvilinear_conserve) Then   !FALSE
                 ...
                 Else                             !TRUE
                   facevary(ivar,i+nguard0+ioff,j+nguard0*k2d+joff,    & 
                                 k+nguard0*k3d+koff,lb)=               & 
                   sendf(ivar,i+nguard,j+nguard*k2d,k+nguard*k3d)
!if(lb .eq. 40 .AND. ivar .eq. 2)print*,"facevary",ivar,lb,i+nguard0+ioff,j+nguard0*k2d+joff, &
!                      k+nguard0*k3d+koff,facevary(ivar,i+nguard0+ioff, &
!                      j+nguard0*k2d+joff,k+nguard0*k3d+koff,lb)
                 End If
                 End If
               End Do
             End Do
...
!***************************************************************************
      ...
      Return
      End Subroutine mpi_amr_1blk_restrictFaces
