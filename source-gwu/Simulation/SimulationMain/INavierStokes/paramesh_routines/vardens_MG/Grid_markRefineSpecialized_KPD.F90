...
subroutine Grid_markRefineSpecialized_KPD (criterion,size,specs,lref)
...
! Arguments
  ...
  select case (criterion)
     case(THRESHOLD)
        var = int(specs(1))
        !icmp = int(specs(3))
        call gr_markVarThreshold_KPD(var,specs(2),specs(3),lref)
     case(VORTICITY)
        var = int(specs(1))
        call gr_markVarThreshold_KPDvort(var,specs(2),specs(3),lref)
     case(VORTwLS)
        var = int(specs(1))
        call gr_markVarThreshold_KPDvortLS(var,specs(2),specs(3),lref)
     case(ELLIPSOID)
  ...
  return
end subroutine Grid_markRefineSpecialized_KPD
