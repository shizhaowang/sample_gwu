...
subroutine Grid_applyBCEdge(bcType,bcDir,guard,var,dataRow,face,&
  ...
  use Simulation_data, ONLY: sim_pAmbient, sim_rhoAmbient, sim_windVelx, sim_windVely,sim_gamma, &
...
# include "Flash.h"
  ...
  if(face==LOW) then
     ...
     case(USER_DEFINED)
        ...
        case(VELX_VAR)
           dataRow(1:guard)=sim_windVelx
        case(VELY_VAR)
           dataRow(1:guard)=sim_windVely
        case(VELZ_VAR)
           dataRow(1:guard)=0.0
        case(ENER_VAR)
           kine = 0.5 * (sim_windVelx**2 + sim_windVely**2)
