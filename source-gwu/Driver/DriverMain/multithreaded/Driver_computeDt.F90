!!****if* source/Driver/DriverMain/multithreaded/Driver_computeDt
!!
!! NAME
!!
!!  Driver_computeDt
...
subroutine Driver_computeDt(nbegin, nstep, &
  ...
  use Driver_data, ONLY : dr_dtMin,dr_dtMax, dr_tstepChangeFactor, &
                          ...
                          dr_threadBlockList, dr_threadWithinBlock
...
#include "Flash.h"
  ...
  real    :: dtModule(2,nUnits), dtLocal(2,nUnits), tp_dtLocal(2,nUnits)
  integer :: dtMinLoc(5), lminloc(5,nUnits), tp_lminloc(5,nUnits), ngmin, pgmin
  ...
  logical :: printToScrn
  real :: extraHydroInfo
  real, dimension(MAXBLOCKS) :: extraHydroInfoArr
  ...
  !$omp parallel if (dr_threadBlockList .or. dr_threadWithinBlock) &
  !$omp default(none) &
  !$omp shared(blockList,numLeafBlocks,gcell,dtLocal,lminloc,&
  !$omp extraHydroInfoArr,dr_useSTS,dr_dtSTS) &
  !$omp private(i,xLeft,xRight,xCenter,dx,uxgrid,yLeft,yRight,yCenter,&
  !$omp dy,uygrid,zLeft,zRight,zCenter,dz,uzgrid,isize,jsize,ksize,&
  !$omp blkLimits,blkLimitsGC,del,tp_dtLocal,tp_lminloc,solnData)
  ...
  do i = 1, numLeafBlocks
     ...
     call Hydro_computeDt ( blockList(i), &
                           ...
                           solnData,      &
                           tp_dtLocal(1,HYDRO), tp_lminloc(:,HYDRO), &
                           extraInfo=extraHydroInfoArr(i) )


     call Stir_computeDt ( blockList(i),  &
                           blkLimits,blkLimitsGC,  &
                           solnData,               &
                          tp_dtLocal(1,STIR), tp_lminloc(:,STIR) )
     ...
     call Burn_computeDt ( blockList(i),  &
                           blkLimits,blkLimitsGC,  &
                           solnData,               &
                           tp_dtLocal(1,BURN), tp_lminloc(:,BURN) )
...
!!$                           dtLocal(1,GRAV), lminloc(:,GRAV) )
     ...
     call Heat_computeDt ( blockList(i),  &
                           ...
                           blkLimits,blkLimitsGC,  &
                           solnData,      &
                           tp_dtLocal(1,HEAT), tp_lminloc(:,HEAT) )

     call Heatexchange_computeDt ( blockList(i),  &
                           blkLimits,blkLimitsGC,  &
                           solnData,      &
                           tp_dtLocal(1,HEATXCHG), tp_lminloc(:,HEATXCHG) )

     call RadTrans_computeDt(blockList(i), blkLimits,blkLimitsGC, &
          solnData, tp_dtLocal(1,RADTRANS), tp_lminloc(:,RADTRANS) )

     call Cool_computeDt ( blockList(i),  &
                           blkLimits,blkLimitsGC,  &
                           solnData,      &
                           tp_dtLocal(1,COOL), tp_lminloc(:,COOL) )

     call Particles_computeDt &
          ( blockList(i), tp_dtLocal(1,PART), tp_lminloc(:,PART))
     ...
     call Diffuse_computeDt ( blockList(i),  &
                           ...
                           blkLimits,blkLimitsGC,  &
                           solnData,      &
                           tp_dtLocal(1,DIFF), tp_lminloc(:,DIFF) )
...
!!$                           dtLocal(1,COSMO), lminloc(:,COSMO) )
      call Cosmology_computeDt(tp_dtLocal(1,COSMO))

      !! Super time step
      if (dr_useSTS) then
         tp_dtLocal(1,STS) = dr_dtSTS
      endif
  ...
  !$omp critical (Reduce_dt_info)
  do i = 1, nUnits
     if (tp_dtLocal(1,i) < dtLocal(1,i)) then
        dtLocal(1,i) = tp_dtLocal(1,i)
        lminloc(:,i) = tp_lminloc(:,i)
     end if
  end do
  !$omp end critical (Reduce_dt_info)
  !$omp end parallel

  !DEV CD: At the moment Dongwook just uses extraHydroInfo to return
  !the current CFL - this is the same for all blocks.
  if (numLeafBlocks > 0) then
     extraHydroInfo = extraHydroInfoArr(1)
  else
     extraHydroInfo = 0.0
  end if
