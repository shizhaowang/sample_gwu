!!****if* source/Particles/ParticlesMain/active/DPD/pt_dpdForces
!!
!! NAME
!!    pt_DPDupdateForces
!!
...
subroutine pt_dpdForces(pos,v,btypes,parents,parentType, &
  ...
  do i=1,np
     ...
     do j=i+1,np
        ...
        if ((l<=rc).and.(l > 0.)) then
           ...
           fvec(:,i)=fvec(:,i) + fval * lij/l;
           fvec(:,j)=fvec(:,j) - fval * lij/l;
        end if
