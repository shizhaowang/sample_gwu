!!****if* source/Grid/GridSolvers/Pfft/gr_pfftTransposeV
!!
!! NAME
!!
!!  gr_pfftTransposeV
...
!! SYNOPSIS
!!
!!  call gr_pfftTransposeV (integer(IN) :: dir,
...
subroutine gr_pfftTransposeV(dir,baseDatType,inArray,outArray,inlen,outlen,pr,comm)
  implicit none
  ...
  call gr_pfftTransposeVImplementation(dir,baseDatType,&
       inArray,size(inArray),&
       outArray,size(outArray),&
       inlen,outlen,pr,comm)

  return
end subroutine gr_pfftTransposeV
...
subroutine gr_pfftTransposeVImplementation(dir,baseDatType,&
     inArray,inArraySize,&
...
#include "Flash_mpi.h"
  ...
  integer ::  i, myRank
  integer,dimension(0:pr-1) :: sendcounts,sdispls,recvcounts,rdispls
...
!!$  allocate(sendcounts(pr))
!!$  allocate(sdispls(pr))
!!$  allocate(recvcounts(pr))
!!$  allocate(rdispls(pr))
  if(dir==PFFT_FORWARD) then
     ...
     if(pr>1) then
        ...
        if(numbCommProcs .NE. pr) then
 
           print *, "[gr_pfftTransposeV] pr:", pr, "number of processors:", numbCommProcs, &
                "- these should be the same!"
           call Driver_abortFlash("[gr_pfftTransposeV] Severe error: pr .NE. numbCommProc... exiting")           
        end if
        ...
        sendcounts(:) = nPerProc
        recvcounts(:) = nPerProc
        if (product(outLen) > nx*nx1) then
           sendcounts(pr-1) = nPerProc - (product(outLen) - nx*nx1)*factor
           call MPI_Comm_Rank(comm, myRank, ierr)
           if (myRank==pr-1) recvcounts = nPerProc - (product(outLen) - nx*nx1)*factor
        end if
        do i=0,pr-1
           sdispls(i) = i * nPerProc
           rdispls(i) = i * nPerProc
        end do
        call MPI_ALLTOALLV(outArray,sendcounts,sdispls,FLASH_REAL,&
                           inArray, recvcounts,rdispls,FLASH_REAL,&
                           comm,ierr)
...
!!$                          FLASH_REAL,comm,ierr)
  ...
  return
end subroutine gr_pfftTransposeVImplementation
