...
#include "Pfft.h"
  ...
  use gr_pfftReconfigFn, ONLY : gr_pfftSrlBufToPencil, gr_pfftSrlBufToGrid
  ...
  do while(associated(fragmentNode))
     ...
     if (direction == TO_PFFT) then
        ...
        if (fragmentNode % metadata % srcFlashBlockRefLev &
           ...
           call gr_pfftSrlBufToPencil(fragmentNode, messageNode % buf)

        else if (fragmentNode % metadata % srcFlashBlockRefLev &
             > pfft_oneRefLev) then

           !RESTRICT
           call Driver_abortFlash("[gr_pfftDataFromBuf]: "//&
        ...
        if (fragmentNode % metadata % srcFlashBlockRefLev &
           ...
           call gr_pfftSrlBufToGrid(fragmentNode, messageNode % buf)
