...
!
  ...
  do blockNr = 1,gr_mpoleBlockCount
     ...
     if (insideBlock) then
         ...
         allocate (shifts (maxEdges,3))

         guardCells = .false.

         call Grid_getCellCoords (IAXIS, blockID, FACES, guardCells, shifts (:,1), nEdgesI)
         call Grid_getCellCoords (JAXIS, blockID, FACES, guardCells, shifts (:,2), nEdgesJ)
         call Grid_getCellCoords (KAXIS, blockID, FACES, guardCells, shifts (:,3), nEdgesK)

         shifts (:,1) = shifts (:,1) - gr_mpoleXcenter
         shifts (:,2) = shifts (:,2) - gr_mpoleYcenter
         shifts (:,3) = shifts (:,3) - gr_mpoleZcenter

         locate (1:3) = minloc (abs (shifts (:,1:3)), dim = 1)
