...
#include "gr_mpole.h"
  ...
  real    :: localMsum, localMDsum, localMDYsum
  real    :: maxRcyl, maxZ
  real    :: minRcyl, minZ
  real    :: Rcyl
  real    :: z
...
!
  ...
  do blockNr = 1,gr_mpoleBlockCount
     ...
     if (insideBlock) then
         ...
         shifts (:,1) = shifts (:,1) - gr_mpoleRcenter
         shifts (:,2) = shifts (:,2) - gr_mpoleZcenter

         locate (1:2) = minloc (abs (shifts (:,1:2)), dim = 1)
