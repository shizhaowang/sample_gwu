...
subroutine multigridSUPERLU (isrc_dens, img_soln, poisfact, img_src, img_res, & 
...
!  end do
 ...
 if (gr_meshMe .eq. 0) print*,"5555 Did the Grid Change in Multigrid After  gr_mgInitSlv?",grid_changed
...
enddo
 ...
 if (gr_meshMe .eq. 0) print*,"6666 Did the Grid Change in Multigrid After  gr_mgInitSlv?",grid_changed
...
if (norm_old > mgrid_max_residual_norm * norm_rhs .OR. i.eq.0) then
   ...
   do while (.not. done)
      ...
      !---------------------------------------------------------------------------- 
      call gr_mgCycleSUPERLU (mesh_lrefmax,mesh_lrefmin, img_soln, img_src, img_res, img_corr, &
           img_temp, img_temp2, mg_solve, mg_residual, mg_residualMG, mg_relax_RBGS, &
           mg_relax_SUPERLU, img_denx,mg_relax, i+1)
