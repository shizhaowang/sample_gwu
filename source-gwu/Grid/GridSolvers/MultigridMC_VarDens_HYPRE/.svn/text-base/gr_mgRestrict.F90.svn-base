...
!               then copy to the output variable.  
!
!               However, we have to trick
...
!               Temporarily alter node type arrays.
  ...
  do lb = 1, lnblocks
     if ( ((lrefine(lb) == level-1)  .and. &
          ...
          !print*,"Restrict From",lb,work(5,5,1,lb,1),work(10,10,1,lb,1)
...
!               Copy the result into the requested output level and variable.
  ...
  do lb = 1, lnblocks
     if (lrefine(lb) == level-1) then
          ...
          !print*,"Restrict To",lb,work(5,5,1,lb,1),work(10,10,1,lb,1)
