!!****if* source/Grid/GridSolvers/MultigridMC_VarDens_HYPRE/poisson/MUMPS_poisson_mg_relax_HYPRE
!!
!! NAME
!!
!!  Grid_solvePoisson
...
#include "constants.h"
  ...
  integer :: mylevel, mypart, var, ii,jj,kk,i,j,k,m,n, ierr
  ...
  real, allocatable  :: blockX(:),blockY(:),blockZ(:),yLevel(:),blockYY(:)
  real, dimension(2,MDIM) :: boundBox
  integer :: nBlocksInXDir,nBlocksInYDir,nBlocksInZDir,iCount,iFlag, NN, NZ
  real :: rx, ry, rz, minY
...
!**************************************************************************************************
        ...
        !- kpd - Calculate the center of each block
        !-----------------------------------------------------
        do lb = 1, lnblocks
...
print*,"Blocks in X,Y,Z Dir",nBlocksInXDir,nBlocksInYDir,nBlocksInZDir,"BlockCount",blockCount
        ...
        !- kpd - Find the Y location of each Y row in the XZ plane
        !------------------------------------------------------------------------
        allocate(yLevel(nBlocksInYDir))
        iCount = 0
        do jj=1,nBlocksInYDir
           iFlag = 0
           iCount = iCount+1
           ry = MINVAL(blockYY)
           do lb = 1, lnblocks
              if (lrefine(lb) == level ) then
                 if (blockYY(lb) .eq. ry ) then
                    blockYY(lb) = 10000.0 
                    if (iFlag .eq. 0) then
                       yLevel(iCount) = ry
                       iFlag = 1
                       print*,lb,iCount,yLevel(iCount)
                    end if
                 end if
              end if
           end do
        ...
        do kk=1,nBlocksInZDir
           ...
           do j=1,blockCount 
              ...
              do lb = 1, lnblocks
                 if (lrefine(lb) == level) then
                    if (blockY(lb) .eq. yLevel(jj) .AND. iFlag .eq. 0 .AND. zPlane(lb) .eq. kk) then
                       iCount = iCount+1
                       iFlag  = 1
...
!kpd - MUMPS Test Loop 
  ...
  open(unit=13,file="HEADkpdTest.dat")
  open(unit=15,file="BkpdTest.dat")
  open(unit=17,file="AkpdTest.dat")
     ...
     do k = blkLimits(LOW, KAXIS), blkLimits(HIGH, KAXIS)
        do j = blkLimits(LOW, JAXIS), blkLimits(HIGH, JAXIS)
           do i = blkLimits(LOW, IAXIS), blkLimits(HIGH, IAXIS)
              ...
              kpdA(1) = -1.0*(kpdA(2)+kpdA(3)+kpdA(4)+kpdA(5)+kpdA(6)+kpdA(7))
              NZ = NZ+1
              ...
              !***********************************************************************************************
                    ...
                    if ( m .eq. blockCount .AND. i .eq. blkLimits(HIGH, IAXIS) .AND. &
                       ...
                       if (k .eq. NGUARD+1 .AND. abs(kpdA(6)) .gt. 0.00001) then
                          NZ = NZ+1
                          write(17,*) ii,ii-(NXB*NXB*NZB*(nBlocksInXDir*nBlocksInYDir-1)+NXB*NXB),kpdA(6),"        :values"
                       else if (abs(kpdA(6)) .gt. 0.00001) then
                          write(17,*) ii,ii-(NXB*NXB),kpdA(6),"        :values"
                          NZ = NZ+1
                       end if
                       ...
                       if (k .eq. NGUARD+1 .AND. abs(kpdA(6)) .gt. 0.00001) then
                          NZ = NZ+1
                          write(17,*) ii,ii-(NXB*NXB*NZB*(nBlocksInXDir*nBlocksInYDir-1)+NXB*NXB),kpdA(6)
                       else if (abs(kpdA(6)) .gt. 0.00001) then
                          write(17,*) ii,ii-(NXB*NXB),kpdA(6)
                          NZ = NZ+1
                       end if

                    end if

                    if (k .eq. NXB+NGUARD .AND. abs(kpdA(7)) .gt. 0.00001) then
                       NZ = NZ+1
                       write(17,*) ii,ii+(NXB*NXB*NZB*(nBlocksInXDir*nBlocksInYDir-1)+NXB*NXB),kpdA(7)
                    else if (abs(kpdA(7)) .gt. 0.00001) then
                       write(17,*) ii,ii+(NXB*NXB),kpdA(7)
                       NZ = NZ+1
                    end if
  ...
  !call Driver_abortFlash("KPD HYPRE stop")
