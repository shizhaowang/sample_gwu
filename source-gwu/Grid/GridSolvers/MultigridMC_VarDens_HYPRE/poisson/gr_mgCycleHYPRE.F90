...
!  end if
  ...
  ! Shizhao, debug
  !  do lb = 1, lnblocks
  !      write(2000000+gr_meshMe*1000+i,*) lb, lnblocks, lrefine(lb), nodetype(lb)
  !  enddo
 end do

  !call gr_mgAssignFden (levelmax, img_denx, 0)  ! Shizhao turn on 
...
!!  end do
      ...
      if (levelmax > mgrid_solveLevelKPD) then
        ...
        do i = levelmax, mgrid_solveLevelKPD+1,  -1
           ...
           if (gr_meshMe .eq. 0) print*,"MG At Level: ",i
...
!      if (gr_meshMe .eq. 0) print*,"Using RBGS at bottom of multigrid with steps = 2000"
!      call mg_relax_RBGS   (mgrid_solveLevelKPD, img_res, img_corr,2000,img_denx,mgrid_solveLevelKPD)

      call cpu_time(t_start)
      call mg_restore_nodetypes (mgrid_solveLevelKPD)  ! Shizhao. 
