...
subroutine multigridHYPRE_PC (isrc_dens, img_soln, poisfact, img_src, img_res, & 
...
if (norm_old > mgrid_max_residual_norm * norm_rhs) then
   ...
   do while (.not. done)
      ...
      !---------------------------------------------------------------------------- 
      call gr_mgCycleHYPRE_PC (mesh_lrefmax,mesh_lrefmin, img_soln, img_src, img_res, img_corr, &
