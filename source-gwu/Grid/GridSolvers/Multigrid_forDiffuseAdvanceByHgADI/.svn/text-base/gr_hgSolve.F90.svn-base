!!****if* source/Grid/GridSolvers/Multigrid_forDiffuseAdvanceByHgADI/gr_hgSolve
!!
!! NAME
!!  gr_hgSolve
!!
...
#include "Multigrid.h"
  ...
  integer             :: n, m, ierr, extrap,i
  real                :: norm_lhs, norm_rhs, norm_tmp, theta = 0.5 !Explicit Time Integration (SKG) 
  ...
  !SKG: Removed call to gr_hgInitSource, as restrict of source is really what we need.
  call Timers_start("gr_restrictTree")
  ! Note indices are different but call is the same
  do i = gr_hgMeshRefineMax-1, 1, -1
     call gr_hgRestrict(i+1, gr_iSource, gr_iSource)
  enddo
  ...
  do m = 1, gr_hgMeshRefineMax
     if (m==1) call gr_hgSetMaxLevel(m)    ! This is a new FLASH3 routine, why if loop ?  call gr_hgSetMaxLevel(m)
     call gr_hgNorm(0, 1, gr_iSource, norm_tmp, MG_NODES_LEAF_ONLY)
     !call gr_hgSetZeroBoundary(m, gr_iSoln)  !oK
     call gr_hgSolveLevel(m, gr_iSource, gr_iSoln, SolveBlock, MG_NODES_ALL_NODES, dt, chi, theta)
     call gr_hgResidual(m, gr_iSource, gr_iSoln, gr_iSls,dt,chi,theta) !oK
     call gr_hgProlongToGuardCells(m, gr_iSoln, gr_iSoln, 0)  ! LBR doesn't wanna know.  But we looked at it...
  enddo 
  ...
  do n = 0, gr_hgMaxCorrections
     ...
     call gr_hgSolveLevel(1, gr_iSls, gr_iCorr, SolveBlock, MG_NODES_ALL_NODES, dt, chi,theta)
     call gr_hgLevelAdd(1, gr_iSoln, gr_iCorr, MG_NODES_LEAF_ONLY) !oK
     call gr_hgProlongToGuardCells(1, gr_iCorr, gr_iCorr, 0)  !infamous
     do m = 2, gr_hgMeshRefineMax
!!$        call gr_hgSetMaxLevel(m)  ! this is now advanced in gr_hgProlongToGuardCells
        !call gr_hgSetZeroBoundary(m, gr_iCorr)
        call gr_hgSolveLevel(m, gr_iSls, gr_iCorr, SolveBlock, MG_NODES_ALL_NODES, dt, chi, theta)
        call gr_hgResidual(m, gr_iSls, gr_iCorr, gr_iSls,dt,chi,theta)  ! not called on top level
        call gr_hgLevelAdd(m, gr_iSoln, gr_iCorr, MG_NODES_LEAF_ONLY)
        call gr_hgProlongToGuardCells(m, gr_iCorr, gr_iCorr, 0)
     enddo
