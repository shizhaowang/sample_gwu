!!****if* source/Grid/GridSolvers/Multigrid_forDiffuseAdvanceByHgADI/Grid_advanceDiffusion
!!
!! NAME
...
!!                   Currently this also determines the meaning of the rhs variable
!!                   given by iSrc.
!!                   If solnIsDelta==.TRUE.  then iSrc contains  Laplace T  (del old temp) multiplied with some factors.
!!                   If solnIsDelta==.FALSE. then iSrc contains       T     (old temp), plus an additional term that
!!                   contains  Laplace T  but which disappears in the fully implicit (theta=0) case.
!!
!! RESULT
!!  
...
subroutine Grid_advanceDiffusion (iSoln, iSrc, iCond, iConstFact, bcTypes, bcValues, dt, chi, scaleFact, &
     solnIsDelta,pass)
...
#include "constants.h"
  ...
  if (.NOT. solnIsDelta) then
     call Logfile_stamp(&
          'This implementation only supports solnIsDelta=.TRUE.',&
          '[Grid_advanceDiffusion]')
     call Driver_abortFlash('[Grid_advanceDiffusion] solnIsDelta must be TRUE!')
  end if
