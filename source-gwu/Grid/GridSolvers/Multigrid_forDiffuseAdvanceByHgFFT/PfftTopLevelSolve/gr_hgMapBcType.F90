!!****if* source/Grid/GridSolvers/Multigrid_forDiffuseAdvanceByHgFFT/PfftTopLevelSolve/gr_hgMapBcType
!!
!! NAME
!!  gr_hgMapBcType
!!
...
#include "Multigrid.h"
  ...
  use gr_hgData, ONLY: gr_hgNowActive, gr_hgCurrentGcReq_extrap, gr_hgBndTypes
  ...
  if (gr_hgNowActive) then
     if (gridDataStruct == WORK) then
        select case (bcTypeFromGrid)         ! ALSO TAKE INTO ACCOUNT SLIP-NON SLIP Boundary conditions !!
        case(OUTFLOW,DIODE,REFLECTING)
!!$        bcTypeToApply = GRIDBC_GIMME_WORK
!!$        return

!           write(*,*) 'gr_bc_types',gr_hgBndTypes(2*axis-1),MG_BND_NEUMANN

           select case (gr_hgBndTypes(2*axis-1))
           ...
           case default
              if (gr_meshMe .EQ. MASTER_PE) print*,  &
                   "gr_hgMapBcType: An unexpected gr_hgBndTypes was requested for the WORK array:(", &
                 gr_hgBndTypes,')', bcTypeFromGrid, idest
