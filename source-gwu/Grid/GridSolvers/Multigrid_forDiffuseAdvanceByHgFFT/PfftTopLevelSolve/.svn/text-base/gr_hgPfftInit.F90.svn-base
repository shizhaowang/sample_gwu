!!****if* source/Grid/GridSolvers/Multigrid_forDiffuseAdvanceByHgFFT/PfftTopLevelSolve/gr_hgPfftInit
!!
!! NAME
!!
!!  gr_hgPfftInit
...
subroutine gr_hgPfftInit()
  ...
  use gr_hgPfftData, ONLY : gr_hgPfftSolveFlag, gr_hgPfftLastMappedLevel, &
       gr_hgPfftMaxDirectSolveLevel, gr_hgbcTypes
...
#include "Multigrid.h"
  ...
  do eachBoundary = 1, 2*NDIM
     ...
     call RuntimeParameters_mapStrToInt(bcTypeStr(eachBoundary), gr_hgbcTypes(eachBoundary))

     select case(gr_hgbcTypes(eachBoundary))
     case (PERIODIC) 
...
!         gr_hgBndConditions = MG_BND_PERIODIC          ! Defined to do mean substractions in hg Multigrid  
     ...
     if( gr_hgbcTypes(1) .eq. MG_BND_PERIODIC  .and.  &   ! in x periodic
         gr_hgbcTypes(3) .eq. MG_BND_NEUMANN   .and.  &   ! in y Neuman
         gr_hgbcTypes(5) .eq. MG_BND_NEUMANN  ) then      ! in z Neuman

         solveflag = 1   
!         gr_hgBndConditions = MG_BND_NEUMANN


     elseif(gr_hgbcTypes(1) .eq. MG_BND_PERIODIC .and. &  ! in x periodic
