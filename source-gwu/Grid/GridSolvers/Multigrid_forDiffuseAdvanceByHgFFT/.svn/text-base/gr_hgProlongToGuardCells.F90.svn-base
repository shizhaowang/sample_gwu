!!****if* source/Grid/GridSolvers/Multigrid_forDiffuseAdvanceByHgFFT/gr_hgProlongToGuardCells
!!
!! NAME
...
!==============================================================================
  ...
  do b=1,lnblocks
     ...
     ! the owning processor (non-blocking).
          ...
          if ((ichild == 0) .or. (ichild == c)) then
               if (child(2,c,b) == hg_myPE) then  ! local child
                ...
                call mpi_issend(send_prolong_data(1,1,1,c,h), nmax1*nmax2*nfaces, &
                     ...
                     MPI_COMM_WORLD, send_prolong_req(nsent), ierr)
             endif
          endif
       enddo
       ...
       if (any_sent) h = h + 1
       if (h > nbbuf_prolong) &
            call Driver_abortFlash("Buffer space exceeded in gr_hgProlongToGuardCells")
     end if
  enddo
