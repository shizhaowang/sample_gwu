!!****if* source/Grid/GridSolvers/BiPCGStab/varcoeffpoisson/Grid_solvePrecond
!!
!! NAME
!!
!!  Grid_solvePoisson
...
! No preconditioner
  do lb = 1, blockCount
     ...
     solnVec(iSoln,ili:iui,jli:jui,kli:kui)=solnVec(iSrc,ili:iui,jli:jui,kli:kui)
     ...
     do k = blkLimits(LOW, KAXIS), blkLimits(HIGH, KAXIS)      
        do j = blkLimits(LOW, JAXIS), blkLimits(HIGH, JAXIS)
           do i = blkLimits(LOW, IAXIS), blkLimits(HIGH, IAXIS)
              ...
              RHSVal(ii) = -1.0*cellVolumes(i,j,k)*solnVec(iSrc,i,j,k)
