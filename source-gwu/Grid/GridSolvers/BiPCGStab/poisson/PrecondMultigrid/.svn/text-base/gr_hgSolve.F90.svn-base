!!****if* source/Grid/GridSolvers/BiPCGStab/poisson/PrecondMultigrid/gr_hgSolve
!!
!! NAME
!!  gr_hgSolve
!!
...
subroutine gr_hgSolve(gr_iSource, gr_iSoln, gr_iSls, gr_iCorr, SolveBlock, bndTypes, src_fact, dt, chi)
  ...
  use gr_hgInterface, ONLY: gr_hgBndry, gr_hgInitSource, gr_hgLevelAdd, gr_hgLevelAddScalar, &
       ...
       gr_hgSolveLevel, gr_hgPfftInitGrid, gr_hgPfftSolveLevel,gr_hgGuardcell
  ...
  use bicg_common, ONLY : precond_init
  implicit none
  ...
  integer             :: n, m, ierr, extrap, eachBoundary
  real                :: norm_lhs, norm_rhs, norm_tmp

  character*70        :: internalFile
  integer,save        :: comm, globalTopLevel,solveLevel
  logical,save        :: requestMap
  logical,save        :: suppressPfft
  integer,save        :: gridChanged
  integer,save        :: prevSolveLevel = -1

#include "Flash_mpi.h"

  if (precond_init) then !precond_init
...
!!$        gr_hgBndTypes(eachBoundary) = bndTypes(eachBoundary)
     if (bndTypes(eachBoundary) == gr_hgbcTypes(eachBoundary) .OR. suppressPfft .OR. &
        ...
        if (gr_meshMe .eq. 0) then
           ...
           write(*,*) 'gr_hgSolve Error: Check you have set multigrid BCs properly on flash.par.'
...
!!$  call gr_hgNorm(0, 1, gr_iSource, norm_rhs, MG_NODES_LEAF_ONLY) !oK
!!$  
!!$  if (norm_rhs .eq. 0.0)   norm_rhs = 1.
  ...
  ! --------------------------------------------------------------------------------
  ! We only need to prolong data from globalTopLevel.  In the original Multigrid 
  ! implementation we had to perform a solve from level 1 to gr_hgMeshRefineMax.  
  ! PFFT saves us the solve from levels 1 to globalTopLevel-1 because it operates 
  ! on refinement level globalTopLevel.
  ! --------------------------------------------------------------------------------
...
!!$     call gr_hgResidual(m, gr_iSource, gr_iSoln, gr_iSls) !oK
  ...
  do n = 0, gr_hgMaxCorrections
     ...
     do m = gr_hgMeshRefineMax-1,  solveLevel, -1 !globalTopLevel
...
!!$        endif 
     ...
     ! *** Again we ignore the levels below globalTopLevel ***
     do m = solveLevel+1, gr_hgMeshRefineMax !globalTopLevel+1
