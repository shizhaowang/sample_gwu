!!****if* source/Grid/GridSolvers/Multigrid_experimental/gr_hgSolveLevel
!!
!! NAME
!!  gr_hgSolveLevel
!!
...
!!
!!  Some implementations of this routine do two Jacobi sweeps on the
!!  first two interior cells for each block, in a second phase in
!!  order to dampen inter-block discontinuities.  These sweeps are
...
!! NOTES
!!
!!  The Jacobi sweeps are commented out in this implementation.
!!  The caller is expected to follow a call to this routine with a
!!  call to a routine that does relaxation sweeps if necessary.
!!
!!***
...
#include "Flash_mpi.h"
  ...
  do b = 1, lnblocks
     ...
     if (SolveThisBlock) then
        ...
        ! Use standard way of finding delta x / delta y / delta z
...
!  call timer_start("relaxation")
  ...
  do n = 1, 2
     ...
     do b = 1, lnblocks
        ...
        !   SolveThisBlock = (SolveThisBlock .and. (nodetype(b) /= 1))
        !endif
        SolveThisBlock = (nodetype(b) == LEAF)
