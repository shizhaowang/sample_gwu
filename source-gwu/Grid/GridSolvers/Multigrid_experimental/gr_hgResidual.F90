!!****if* source/Grid/GridSolvers/Multigrid_experimental/gr_hgResidual
!!
!! NAME
!!  gr_hgResidual
!!
...
!!  dt, chi, theta - Not required in this scenario (specific to MG implicit) 
!! RESULT
!!
...
#include "constants.h"
  ...
  do b = 1, lnblocks
     if (lrefine(b) == level .OR. &
          (level == 0 .and. gr_hgSaveNodeType(b) == LEAF)) then
  ...
  if ( ALL(gr_hgBndTypes(1:2*NDIM) == MG_BND_PERIODIC .or. gr_hgBndTypes(1:2*NDIM) == MG_BND_NEUMANN) ) then
     ...
     do b = 1, lnblocks
        if ((lrefine(b) == level) .or. &
             ((gr_hgSaveNodetype(b) == LEAF) .and. (lrefine(b) < level .or. level == 0))) then
           bvol = bsize(1,b)
