!!****if* source/Grid/GridSolvers/Multigrid_experimental/PfftTopLevelSolve/gr_hgSolve
!!
!! NAME
!!  gr_hgSolve
!!
...
!!$        gr_hgBndTypes(eachBoundary) = bndTypes(eachBoundary)
  ...
  do m = solveLevel, gr_hgMeshRefineMax
     ...
     if (m == solveLevel .AND. .NOT. suppressPfft) then
        ...
        call gr_hgSolveLevel(m, gr_iSource, gr_iSoln, SolveBlock, MG_NODES_ALL_NODES)
     end if

     call gr_hgGSPrecondition(gr_iSource, gr_iSoln, 2, 1)
     call gr_hgProlongBndries(m, gr_iSoln, gr_iSoln, 0)
  enddo
  call gr_hgResidual(0, gr_iSource, gr_iSoln, gr_iSls)
  ...
  do n = 0, gr_hgMaxCorrections
     ...
     !   call gr_hgGSPrecondition(gr_iSls, gr_iCorr, 2, 1) ! Why not? - KW
     call gr_hgLevelAdd(solveLevel, gr_iSoln, gr_iCorr, MG_NODES_LEAF_ONLY)
     call gr_hgProlongBndries(solveLevel, gr_iCorr, gr_iCorr, 0)
