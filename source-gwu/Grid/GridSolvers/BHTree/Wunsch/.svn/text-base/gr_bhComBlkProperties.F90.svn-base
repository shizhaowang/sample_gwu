...
!! DESCRIPTION
!!
!!   Communicate global block properties: nodetype, lrefine, child,
...
subroutine gr_bhComBlkProperties()
  ...
  use gr_bhData, ONLY : gr_bhTreeNodetype, gr_bhTreeLrefine, &
    ...
    gr_bhIlist, gr_bhTreeNFirstLev, gr_bhTreeBS, gr_bhTreeMyPE
...
#include "Flash_mpi.h"
  ...
  call MPI_ALLREDUCE (lnblocks,gr_bhTreeMaxlnblocks,1,MPI_INTEGER, & 
  ...
  ! allocate messages
  if (gr_bhIlist .eq. 0) then
    int_mes_size = (3+2*MCHILD)*gr_bhTreeMaxlnblocks+1
  else
    int_mes_size = (3+2*MCHILD+2*MFACES)*gr_bhTreeMaxlnblocks+1
  endif
  ...
  do i = 1,gr_bhTreeMaxlnblocks ! copy nodetype and lrefine into sent_int_mes
    if (sent_int_mes(i) .ne. -1) then
      ...
      if (gr_bhIlist .ne. 0) then
        do j = 1, mfaces
          sent_int_mes((3+2*mchild)*gr_bhTreeMaxlnblocks + (i-1)*2*mfaces + 2*(j-1) + 1) = neigh(1,j,sent_int_mes(i))
          sent_int_mes((3+2*mchild)*gr_bhTreeMaxlnblocks + (i-1)*2*mfaces + 2*(j-1) + 2) = neigh(2,j,sent_int_mes(i))
        enddo
      endif
    endif
  enddo
  ...
  do i = 0,gr_bhTreeNumProcs-1
    if (i /= gr_bhTreeMyPE) then
      call MPI_Irecv(recv_int_mes(1,i), int_mes_size, MPI_INTEGER, i, 1, gr_bhComm, reqs(j), ierr)
      call MPI_Isend(sent_int_mes(1), int_mes_size, MPI_INTEGER, i, 1, gr_bhComm, reqs(2*(gr_bhTreeNumProcs-1)+j), ierr)
      j = j + 1
  ...
  ! copy data back from messages to global tr_* structures
  do l = 0,gr_bhTreeNumProcs-1
    do i = 1,gr_bhTreeMaxlnblocks
      if (recv_int_mes(i,l) .ne. -1) then
        ...
        if (gr_bhIlist .ne. 0) then
          do j = 1, mfaces
            gr_bhTreeNeigh(1,j,recv_int_mes(i,l),l) = &
                                         recv_int_mes((3+2*mchild)*gr_bhTreeMaxlnblocks + (i-1)*2*mfaces + 2*(j-1) + 1,l)
            gr_bhTreeNeigh(2,j,recv_int_mes(i,l),l) = &
                                         recv_int_mes((3+2*mchild)*gr_bhTreeMaxlnblocks + (i-1)*2*mfaces + 2*(j-1) + 2,l)
          enddo
        endif
      endif
    enddo
