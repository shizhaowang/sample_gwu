!!****if* source/Grid/GridSolvers/HYPRE_KPDa/paramesh/gr_hypreCreateMatrix
!!
!!  NAME 
!!
                    ...
                    do eachNegh = 1, numNegh                                                            
                       ...
                       if (numNegh > 1) then !! we are on coarse cell, use the average computed from fine cell (2 of them)
                          ...
                          if (NDIM > 1) then
                             ...
                             !- kpd - faceAreas = dx on the fine grid...
                             condimh = 0.5*(solnVec(iFactorB,i-1,j,k)+ solnVec(iFactorB,i,j,k))*faceAreas(IAXIS,i,j,k)
!print*,solnVec(iFactorB,i,j,k),faceAreas(IAXIS,i,j,k)
                          else
                             condimh = xflux(eachNegh,i,j,k)
                          end if
                       end if
...
!!print*,"AA",condimh,theta,dt,delmh(IAXIS)
...
!print*,"BB",condimh,theta,dt,delmh(IAXIS)
