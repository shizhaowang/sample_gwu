!!****if* source/Grid/GridStructures/unitTest/Grid_solidBodyUnitTest
!!
!! NAME
!!
...
subroutine Grid_solidBodyUnitTest(fileUnit,perfect)
  ...
  use gr_sbData, ONLY : gr_sbBodyInfo, gr_sbNumBodies, &
       gr_sbPtNumX, gr_sbPtNumY, gr_sbPtNumZ
  ...
  integer :: p, numParticles, count, b, blkID, proc, i, ierr, myPE, numProcs
  ...
  do b = 1, gr_sbNumBodies
     if (gr_sbBodyInfo(b) % comm /= MPI_COMM_NULL) then
        if (gr_sbBodyInfo(b) % myPE == gr_sbBodyInfo(b) % bodyMaster) then
           ...
           call MPI_Comm_rank(gr_sbBodyInfo(b) % comm, myPE, ierr)
           call MPI_Comm_size(gr_sbBodyInfo(b) % comm, numProcs, ierr)
