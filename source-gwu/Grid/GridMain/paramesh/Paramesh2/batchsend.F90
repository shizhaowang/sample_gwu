...
!#define ERRROR_CHECK
       ...
       subroutine b_int_sendrcv(maxbatchlen,msglen, & 
     &                          ns,sproc,stag,sval, & 
     &                          nr,rproc,rtag,rval)
       ...
       integer :: sval(ns*msglen),  rval(nr*msglen)
       ...
       integer, allocatable, dimension(:,:)  ::  msgs, rmsgs
...
!
       ...
       do i=1,ns
          ...
          msgs(omsgsin(p)*(msglen+1) + 2,p) = stag(i)
          do j=1,msglen 
...
!           call MPI_WAITALL(nmsgsrecv,rreqs,stat,istat)
!       do n=1,nmsgsrecv 
       do m=1,nmsgsrecv 
...
#ifdef DEBUG_BATCH
         print *,' [',rtag(j),(rval(msglen*(j-1)+k),k=1,msglen,n),']'
#endif
           enddo
...
!   subroutine b_logical_sendrcv(maxbatchlen,msglen,ns,sproc,stag,sval,
!                                            nr,rproc,rtag,rval)
!  <TYPE> == logical
!
!  This does the same as the integer case.  The logicals are turned
!  into +1 or 0, then passed to b_int_sendrcv().
!
       subroutine b_logical_sendrcv(maxbatchlen,msglen, & 
     &                          ns,sproc,stag,sval, & 
     &                          nr,rproc,rtag,rval)
       IMPLICIT NONE
       integer :: maxbatchlen, msglen
       integer :: ns, nr
       integer :: sproc(ns), rproc(nr), sproc_(1), rproc_(1)
       integer :: stag(ns),  rtag(nr), stag_(1), rtag_(1)
       logical :: sval(ns*msglen),  rval(nr*msglen)
       integer, allocatable :: sival(:),  rival(:)
       integer :: istat
       integer :: nbits
       integer :: imsglen
       integer :: intno, bno, curint
       integer :: sz1, sz2
       integer :: i,j,k,p,n,m
!
       nbits = bit_size(-1)

       call intdivrndup(msglen, nbits, imsglen)


       sz1 = ns*imsglen          !sizes for sival, rival should never be zero
       if (sz1 .eq. 0) sz1 = 1

       sz2 = nr*imsglen
       if (sz2 .eq. 0) sz2 = 1

       allocate(sival(sz1),rival(sz2),stat = istat)
       if (istat .ne. 0)call Driver_abortFlash('batchsend: alloc rival')
 
       sival(:) = 0
       do i=1,ns
#ifdef DEBUG_BATCH
          print *,'Want to send to ',sproc(i),':',stag(i), & 
     &                              (sval((i-1)*msglen+j),j=1,msglen)
#endif
          ...
          bno = 0
          do j=1,msglen
             if (sval((i-1)*msglen + j)) then
                curint = ibset(curint,bno)
             else
                curint = ibclr(curint,bno)
             endif 
             bno = bno + 1
             if (bno .eq. nbits) then
                sival(imsglen*(i-1)+intno+1) = curint
                intno = intno + 1
                curint = 0
                bno = 0
             endif
       ...
       if ( nr /= 0 .and. ns /= 0 ) then

          call b_int_sendrcv(maxbatchlen,imsglen,   &
                             ns,sproc,stag,sival,   &
                             nr,rproc,rtag,rival)

       else if ( ns == 0 .and. nr == 0 ) then

          call b_int_sendrcv(maxbatchlen,imsglen,   &
                             ns,sproc_,stag_,sival, &
                             nr,rproc_,rtag_,rival)
       ...
       do i=1,nr
          bno = 0
          intno = 0
          curint = rival(imsglen*(i-1)+intno+1)
          do j=1,msglen
             rval(msglen*(i-1) + j) = btest(curint,bno)
#ifdef DEBUG_BATCH
              print *,' Got [',rtag(i),':',rval(msglen*(i-1) + j), & 
     &                '] from ', rproc(i), ' was ',curint
#endif
             bno = bno + 1
             if (bno .eq. nbits) then
                intno = intno + 1
