        ...
        do k = 1+k3d,nzb+(k3d*((2*nguard)-1))
           do j = 1+k2d,nyb+(k2d*((2*nguard)-1))
              do i = 2,nxb+(2*nguard)-1
                 ...
                 delu(2,i,j,k) = delu(2,i,j,k)*dely/XCOORD(i)

                 delua(2,i,j,k) = abs(unk(iref,i,j+1,k,lb)) +  & 
                      abs(unk(iref,i,j-1,k,lb))
                 delua(2,i,j,k) = delua(2,i,j,k)*dely/XCOORD(i)
#endif
...
! d/dydx
                 ...
                 delu2(2) = delu2(2)*dely/XCOORD(i)

                 delu3(2) = abs(delu(1,i,j+1,k)) + abs(delu(1,i,j-1,k))
                 delu3(2) = delu3(2)*dely/XCOORD(i)

                 delu4(2) = delua(1,i,j+1,k) + delua(1,i,j-1,k)
                 delu4(2) = delu4(2)*dely/XCOORD(i)
...
! d/dydy
                 ...
                 delu2(4) = delu2(4)*dely/XCOORD(i)

                 delu3(4) = abs(delu(2,i,j+1,k)) + abs(delu(2,i,j-1,k))
                 delu3(4) = delu3(4)*dely/XCOORD(i)

                 delu4(4) = delua(2,i,j+1,k) + delua(2,i,j-1,k)
                 delu4(4) = delu4(4)*dely/XCOORD(i)
#endif
...
! d/dydz
                 ...
                 delu2(8) = delu2(8)*dely/XCOORD(i)

                 delu3(8) = abs(delu(3,i,j+1,k)) + abs(delu(3,i,j-1,k))
                 delu3(8) = delu3(8)*dely/XCOORD(i)

                 delu4(8) = delua(3,i,j+1,k) + delua(3,i,j-1,k)
                 delu4(8) = delu4(8)*dely/XCOORD(i)
...
! compute the error
                 ...
                 do kk = 1, ndim2
                    num = num + delu2(kk)**2
                    denom = denom + (delu3(kk) + (epsil*delu4(kk)+1.E-99))**2
                 end do
