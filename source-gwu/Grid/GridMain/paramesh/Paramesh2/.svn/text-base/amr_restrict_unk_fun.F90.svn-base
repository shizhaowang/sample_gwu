!!****if* source/Grid/GridMain/paramesh/Paramesh2/amr_restrict_unk_fun
!!
!! NAME
!!
...
!! SYNOPSIS
!!
!!  amr_restrict_unk_fun(datain, dataout, block_parent, ioff, joff, koff)
!!
!!  amr_restrict_unk_fun(real(), real(), integer, integer, integer, integer)
!!
!!
!! DESCRIPTION
!!
...
!! NOTES
!!
!!  For now, we support cartesian, 2-d cylindrical, spherical, and 2-d polar
!!  geometries only.
!!
!!  The interface for this routine has been modified from the standard 
!!  paramesh version, and the corresponding modifications were made to the
...
subroutine amr_restrict_unk_fun(datain,dataout, block_parent, ioff, joff, koff)

  use physicaldata, ONLY :nxb, nyb, nzb, k2d, k3d, nguard, nvar,ndim
  use Driver_interface, ONLY : Driver_abortFlash
  use Grid_interface, ONLY : Grid_getCellCoords
  use Grid_data, ONLY :  gr_iloGc, gr_ihiGc, gr_jloGc, gr_jhiGc, &
       gr_kloGc, gr_khiGc
...
#include "constants.h"
  ...
  real datain(nvar,gr_iloGc:gr_ihiGc,gr_jloGc:gr_jhiGc,gr_kloGc:gr_khiGc)
  real dataout(nvar,gr_iloGc:gr_ihiGc,gr_jloGc:gr_jhiGc,gr_kloGc:gr_khiGc)
...
! (including guardcells)
 ...
 !DEV: kda this should go in physicalConstants!
...
#ifdef DEBUG_AMR
  print *, 'restrict unk fun geom: ', gr_geometry, CARTESIAN, CYLINDRICAL, SPHERICAL, POLAR
#endif
  if (.NOT. validGeom) then
     call Driver_abortFlash("[amr_restrict_unk_fun] ERROR: invalid geometry.")
  endif
...
#ifdef DEBUG_AMR
     print*,'just becore calling coords',csize
#endif
           ...
           case (CARTESIAN)
              ...
              do ivar=1,nvar
                 ...
                 dataout(ivar,i,j,k) = ( & 
                      ...
                      datain(ivar, i+1, j+k2d, k+k3d))*.125
...
! do the volume weighted averaging
              do ivar=1,nvar
                 ...
                 dataout(ivar,i,j,k) = ( & 
                      cvol(1)*datain(ivar, i,   j,     k) + & 
                      cvol(2)*datain(ivar, i+1, j,     k) + & 
                      cvol(3)*datain(ivar, i,   j+k2d, k) + & 
                      cvol(4)*datain(ivar, i+1, j+k2d, k) )*pvol_i
...
! do the volume weighted averaging
              do ivar=1,nvar
                 ...
                 dataout(ivar,i,j,k) = ( & 
                      cvol(1)*datain(ivar, i,   j, k) + & 
                      cvol(2)*datain(ivar, i+1, j, k) )*pvol_i
...
! do the volume weighted averaging
              do ivar=1,nvar
                 ...
                 dataout(ivar,i,j,k) = ( & 
                      ...
                      cvol(2)*datain(ivar, i+1, j,     k    ) + & 
                      cvol(3)*datain(ivar, i,   j+k2d, k    ) + & 
                      cvol(4)*datain(ivar, i+1, j+k2d, k    ) + &
                      cvol(5)*datain(ivar, i,   j,     k+k3d) + & 
                      cvol(6)*datain(ivar, i+1, j,     k+k3d) + & 
                      cvol(7)*datain(ivar, i,   j+k2d, k+k3d) + & 
                      cvol(8)*datain(ivar, i+1, j+k2d, k+k3d))*pvol_i
