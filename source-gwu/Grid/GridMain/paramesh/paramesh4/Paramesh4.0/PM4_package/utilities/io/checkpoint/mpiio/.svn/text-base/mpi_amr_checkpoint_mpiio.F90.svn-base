...
!!****f* utilities/io/checkpoint/mpiio/amr_checkpoint_wr_mpiio
!! NAME
!!
...
!!
!!   call amr_checkpoint_wr_mpiio(integer, optional logical, optional char*80,
...
!! CALLS
!!
!!   write_blocks_mpiio_r8 or write_blocks_mpiio_r4
!!
!! RETURNS
!!
...
!! DESCRIPTION
!! 
!!  Subroutine to checkpoint PARAMESH runs in parallel using the MPIIO library.
!!  It writes out the tree data structure and data stored in PARAMESH blocks.
!!
!!  The files produced will have names of the form 
!!  'paramesh_chk_######.mpiio'. where '######' is the file_num argument passed into
!!  this routine.
!!
!!  The routine gets called directly by the driver routine 'amr_checkpoint_wr' if
!!  the user selects MPIIO format as the means of outputting the file.
!!
!!  The routine is not, by default, included in the PARAMESH default setup.  In order to
!!  use it an install script in utilities/io/checkpoint/mpiio must first be run and
...
!#define DEBUG
      ...
      subroutine amr_checkpoint_wr_mpiio (file_num,  & 
     &                                   l_with_guardcells, & 
     &                                   user_attr_1, & 
     &                                   user_attr_2, & 
     &                                   user_attr_3, & 
...
! handle user attributes
      ...
      call write_blocks_mpiio_r8(filename, & 
     &                          tot_blocks, & 
     &                          tot_blocks_wr, & 
     &                          max_lnblocks, & 
     &                          lnblocks_wr, & 
     ...
     &                          iorder)
      ...
      call write_blocks_mpiio_r4(filename, & 
     &                          tot_blocks, & 
     &                          tot_blocks_wr, & 
     &                          max_lnblocks, & 
     &                          lnblocks_wr, & 
...
!!****f* utilities/io/checkpoint/mpiio/amr_checkpoint_re_mpiio
!! NAME
!!
...
!! SYNOPSIS
!!
!!   call amr_checkpoint_re_mpiio(file_num)
!!   call amr_checkpoint_re_mpiio(file_num, l_with_guardcells,  
!!                               user_attr1, user_attr2, user_attr3, 
!!                               user_attr4, user_attr5)
!!
!!   call amr_checkpoint_re_mpiio(integer, optional logical, optional char*80,
!!                               optional real, optional real, optional real, 
!!                               optional real, optional real)
!!
!! ARGUMENTS
...
!! CALLS
!!
!!   read_blocks_mpiio_r8 or read_blocks_mpiio_r4
!!
!! RETURNS
!!
...
!!  produced using the routine amr_checkpoint_wr_mpiio.  It reads in the tree data 
...
!!  the user selects MPIIO format as the data formet for the file.
!!
!!  The routine is not, by default, included in the PARAMESH default setup.  In order to
!!  use it an install script in utilities/io/checkpoint/mpiio must first be run and
!!  PARAMESH must be recompiled.
...
!!***
      ...
      subroutine amr_checkpoint_re_mpiio (file_num, & 
     &                                   l_with_guardcells, & 
     &                                   user_attr_1, & 
     &                                   user_attr_2, & 
     &                                   user_attr_3, & 
...
! set limits on data arrays
      ...
      call read_blocks_mpiio_r8(filename, & 
     &                         tot_blocks, & 
     &                         lnblocks, & 
     &                         mdim, & 
     &                         ndim, & 
     ...
     &                         iorder)
      ...
      call read_blocks_mpiio_r4(filename, & 
     &                         tot_blocks, & 
     &                         lnblocks, & 
     &                         mdim, & 
     &                         ndim, & 
...
! COMPUTE TREE DATA FROM gid
      ...
      itemp = n_to_left(mype)
      call MPI_Allgather(itemp, 1,MPI_INTEGER, & 
