...
void get_new_type_r4(int nxb, int nyb, int nzb, int nvar, int iorder[],
...
void get_udim_mpiio_r4(int nvar, int nx, int ny, int nz, 
...
#ifdef UNDERSCORE
void write_blocks_mpiio_r4_ (char* file_name_in, 
#endif
#ifdef DOUBLE_UNDERSCORE
void write_blocks_mpiio_r4__ (char* file_name_in, 
#endif
#ifndef UNDERSCORE 
#ifndef DOUBLE_UNDERSCORE
void write_blocks_mpiio_r4 (char* file_name_in, 
#endif
#endif
			   int*  tot_blocks,
			   int*  tot_blocks_wr,
			   ...
			   int iorder[])
{
  ...
  */
  ...
  get_new_type_r4(iend-istart, jend-jstart, kend-kstart, *nvar,
	       iorder,
	       *nx, *ny, *nz, &unk_type);
  MPI_Type_size(unk_type, &unk_size);

  get_new_type_r4(iend-istart+1, jend-jstart, kend-kstart, *nbndvar,
	       iorder,
	       *nx+1, *ny, *nz, &facevarx_type);
  MPI_Type_size(facevarx_type, &facevarx_size);

  get_new_type_r4(iend-istart, jend-jstart+k2d, kend-kstart, *nbndvar,
	       iorder,
	       *nx, *ny+k2d, *nz, &facevary_type);
  MPI_Type_size(facevary_type, &facevary_size);

  get_new_type_r4(iend-istart, jend-jstart, kend-kstart+k3d, *nbndvar,
	       iorder,
	       *nx, *ny, *nz+k3d, &facevarz_type);
  MPI_Type_size(facevarz_type, &facevarz_size);

  get_new_type_r4(iend-istart, jend-jstart+k2d, kend-kstart+k3d, *nbndvare,
	       iorder,
	       *nx, *ny+k2d, *nz+k3d, &unk_e_x_type);
  MPI_Type_size(unk_e_x_type, &unk_e_x_size);

  get_new_type_r4(iend-istart+1, jend-jstart, kend-kstart+k3d, *nbndvare,
	       iorder,
	       *nx+1, *ny, *nz+k3d, &unk_e_y_type);
  MPI_Type_size(unk_e_y_type, &unk_e_y_size);

  get_new_type_r4(iend-istart+1, jend-jstart+k2d, kend-kstart, *nbndvare,
	       iorder,
	       *nx+1, *ny+k2d, *nz, &unk_e_z_type);
  MPI_Type_size(unk_e_z_type, &unk_e_z_size);

  get_new_type_r4(iend-istart+1, jend-jstart+k2d, kend-kstart+k3d, *nbndvarc,
	       iorder,
  ...
  for (lb = 0; lb < *lnblocks; lb++) {
	...
	get_udim_mpiio_r4(*nvar, *nx, *ny, *nz, ivar, istart, jstart, kstart, iorder, udim, it);
	...
	get_udim_mpiio_r4(*nbndvar, *nx+1, *ny, *nz, ivar, istart, jstart, kstart, iorder, udim, it);
	...
	get_udim_mpiio_r4(*nbndvar, *nx, *ny+k2d, *nz, ivar, istart, jstart, kstart, iorder, udim, it);
	...
	get_udim_mpiio_r4(*nbndvar, *nx, *ny, *nz+k3d, ivar, istart, jstart, kstart, iorder, udim, it);
	...
	get_udim_mpiio_r4(*nbndvare, *nx, *ny+k2d, *nz+k3d, ivar, istart, jstart, kstart, iorder, udim, it);
	...
	get_udim_mpiio_r4(*nbndvare, *nx+1, *ny, *nz+k3d, ivar, istart, jstart, kstart, iorder, udim, it);
	...
	get_udim_mpiio_r4(*nbndvare, *nx+1, *ny+k2d, *nz, ivar, istart, jstart, kstart, iorder, udim, it);
    ...
    if (*nvar_chk_nc > 0) {
      ...
      get_udim_mpiio_r4(*nbndvarc, *nx+1, *ny+k2d, *nz+k3d, ivar, istart, jstart, kstart, iorder, udim, it);
