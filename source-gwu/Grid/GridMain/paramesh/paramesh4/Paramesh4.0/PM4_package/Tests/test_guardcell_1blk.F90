...
! refine grid and apply morton reordering to grid blocks if necessary
      ...
      write(*,*) 'main : pe ',mype,' neigh ',neigh(:,:,:lnblocks)
      ...
      if(mype.eq.ii) then
      do l=1,lnblocks
         write(*,228) ii,l,neigh(:,1:2*ndim,l)
228      format('proc ',i2,' lb ',i2,' neigh ',8(i3,1x))
      enddo
      endif
      Call MPI_BARRIER(amr_mpi_meshComm, ierr)
      enddo
