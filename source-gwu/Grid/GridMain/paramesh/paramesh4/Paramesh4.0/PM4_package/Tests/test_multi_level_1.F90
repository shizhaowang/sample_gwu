...
#include "paramesh_preprocessor.fh"
      ...
      program test_multi_level_1

! This test strings 12 blocks along the x axis between -6 and +6,
! to test the package for multiple level 1 blocks in the tree.
...
! set up a 12 block in 3D covering the domain -6 < x < 6,
! 0 < y < 1.0, 0 < z < 1.0.
      ...
      if(mype.eq.0.) then
                ...
                coord(1,lb) = g_xmin+.5+real(lb-1)
                coord(2:3,lb) = .5
                bnd_box(1,1,lb) = g_xmin+real(lb-1)
                bnd_box(2,1,lb) = g_xmin+real(lb)
                bnd_box(1,2:3,lb) = 0.
                bnd_box(2,2:3,lb) = 1.
                nodetype(lb) = 1
                lrefine(lb) = 1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &                'Test of multi_level_1 Successful ', & 
