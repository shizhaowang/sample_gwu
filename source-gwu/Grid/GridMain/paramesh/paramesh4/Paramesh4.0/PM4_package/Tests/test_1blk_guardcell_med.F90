...
!# define DEBUG
      ...
      program test_1blk_guardcell_med
...
! set up a single block covering the whole cubic domain
      ...
      if(mype.eq.0.) then
                ...
                coord(:,1) = 0.5*(bnd_box(2,:,1) + bnd_box(1,:,1))
                nodetype(1) = 1
                lrefine(1) = 1
...
! This section of code was causing me problems on thunderhead
!      if(mype.eq.0) then
!        open(unit=20,status='unknown',
!     .       file=trim(output_dir)//'loop_count_max.dat')
!        read(20,*) loop_count_max
!        close(unit=20)
!      endif
!      Call MPI_BARRIER(amr_mpi_meshComm, ierr)

      if(nprocs.gt.0) call mpi_bcast(loop_count_max,1,MPI_INTEGER,0, & 
...
!     icoord = 0
     ...
     &         az*(zk**interp_mask_unk(ivar))
       ...
       if(abs(v0-unk1(ivar,i,j,k,1))>accuracy) then
         ...
         write(*,*) 'a xyz ',ax,ay,az,' xi j k ',xi,yj,zk, & 
     &           lnblocks
...
998      format('u:error proc ',i3,' block l= ',5(2x,i3),2x,1pe13.6,2x, & 
     &       1pe13.6)
