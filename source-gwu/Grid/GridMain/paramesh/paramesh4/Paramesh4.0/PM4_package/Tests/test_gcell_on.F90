...
#include "paramesh_preprocessor.fh"
      ...
      program test_gcell_on



! This test is identical to test_edges_1blk except that gcell_on_cc is
! used to limit the variables which have guardcells filled.
...
#ifdef TEMP_TEST
!-----
      if (.not.advance_all_levels)t hen
...
! If NO_PERMANENT_GUARDCELLS is undefined, then these selections
! of variables to be switched on, will be active for the entire
! test. If NO_PERMANENT_GUARDCELLS is defined, you can modify the
! selection before each test.
      if(nvar.eq.2) then
        do i = 1,nvar
        if (i == 1) then
           gcell_on_cc(i) = .false.
...
! end temporary testing section
      ...
      if (.not.no_permanent_guardcells) then
!
! If permanent guardcell storage is available, set guardcell values
! to an incorrect value so that tests will fail unless guardcell filling
! is working correctly.
      do l=1,lnblocks

        unk(:,1:nguard0,:,:,l) = -1.
        unk(:,nxb+nguard0+1:nxb+2*nguard0,:,:,l) = -1.
        if(ndim.ge.2) then
        unk(:,:,1:nguard0*k2d,:,l) = -1.
        unk(:,:,nyb+(nguard0+1)*k2d:nyb+2*nguard0*k2d,:,l) = -1.
        endif
        if(ndim.eq.3) then
        unk(:,:,:,1:nguard0*k3d,l) = -1.
        unk(:,:,:,nzb+(nguard0+1)*k3d:nzb+2*nguard0*k3d,l) = -1.
        endif

        facevarx(:,1:nguard0,:,:,l) = -1.
        facevarx(:,nxb+nguard0+2:nxb+2*nguard0+1,:,:,l) = -1.
        if(ndim.ge.2) then
        facevarx(:,:,1:nguard0*k2d,:,l) = -1.
        facevarx(:,:,nyb+(nguard0+1)*k2d:nyb+2*nguard0*k2d,:,l) = -1.
        endif
        if(ndim.eq.3) then
        facevarx(:,:,:,1:nguard0*k3d,l) = -1.
        facevarx(:,:,:,nzb+(nguard0+1)*k3d:nzb+2*nguard0*k3d,l) = -1.
        endif

        facevary(:,1:nguard0,:,:,l) = -1.
        facevary(:,nxb+nguard0+1:nxb+2*nguard0,:,:,l) = -1.
        if(ndim.ge.2) then
        facevary(:,:,1:nguard0*k2d,:,l) = -1.
        facevary(:,:,nyb+(nguard0+2)*k2d:nyb+2*nguard0*k2d+k2d,:,l) = -1.
        endif
        if(ndim.eq.3) then
        facevary(:,:,:,1:nguard0*k3d,l) = -1.
        facevary(:,:,:,nzb+(nguard0+1)*k3d:nzb+2*nguard0*k3d,l) = -1.
        endif

        facevarz(:,1:nguard0,:,:,l) = -1.
        facevarz(:,nxb+nguard0+1:nxb+2*nguard0,:,:,l) = -1.
        if(ndim.ge.2) then
        facevarz(:,:,1:nguard0*k2d,:,l) = -1.
        facevarz(:,:,nyb+(nguard0+1)*k2d:nyb+2*nguard0*k2d,:,l) = -1.
        endif
        if(ndim.eq.3) then
        facevarz(:,:,:,1:nguard0*k3d,l) = -1.
        facevarz(:,:,:,nzb+(nguard0+2)*k3d:nzb+2*nguard0*k3d+k3d,l) = -1.
        endif
...
! test of unk communications
     ...
     &       coord(1,l)-(.5*bsize(1,l)+dx)
       ...
       elseif (.not.no_permanent_guardcells) then
! check to make sure that variables not filled have a deliberately
! incorrect answer. Note, this check does not works if there is
! no permanent guardcell storage, because then it is not possible
! to initialize the guardcells to a chosen incorrect value,
! before calling the guardcell operation.
       if(abs(v0-unk1(ivar,i,j,k,1)).le.accuracy) then
         if(i.lt.1+nguard.or.i.gt.nxb+nguard) then
           write(*,696) l,i,j,k,unk1(ivar,i,j,k,1),v0
           ierror_sum = ierror_sum + 1
         endif
         if(ndim.gt.1.and.(j.lt.1+nguard.or.j.gt.nyb+nguard)) then
           write(*,697) l,i,j,k,unk1(ivar,i,j,k,1),v0
           ierror_sum = ierror_sum + 1
         endif
         if(ndim.gt.2.and.(k.lt.1+nguard.or.k.gt.nzb+nguard)) then
           write(*,698) l,i,j,k,unk1(ivar,i,j,k,1),v0
           ierror_sum = ierror_sum + 1
         endif
       endif
...
! if we have permanent guardcell storage, we need to call amr_guardcell again
! because the message buffers will still have data relevant to unk here.
      Call MPI_BARRIER(amr_mpi_meshComm, ierr)
      iopt = 2
      nlayers = nguard_work
      call amr_guardcell(mype,iopt,nlayers)
...
! if we have permanent guardcell storage, we need to call amr_guardcell again
! because the message buffers will still have data relevant to work here.
      Call MPI_BARRIER(amr_mpi_meshComm, ierr)
      iopt = 1
      nlayers = nguard
      call amr_guardcell(mype,iopt,nlayers)
...
696      format('x gcell masking is not working properly!',4(2x,i3), & 
     &       2x,f9.6,2x,f9.6)
697      format('y gcell masking is not working properly!',4(2x,i3), & 
     &       2x,f9.6,2x,f9.6)
698      format('z gcell masking is not working properly!',4(2x,i3), & 
     &       2x,f9.6,2x,f9.6)
