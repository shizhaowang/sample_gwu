...
#include "test_defs.fh"
      ...
      double precision :: time_start, time_wr_hd, time_re_hd
      double precision :: time_wr_de, time_re_de
...
! write checkpoint file using hdf5
        iunit1 = 50
     ...
     &                          user_attr_4 = 4., & 
     &                          user_attr_5 = 5.)
        time_wr_hd = mpi_wtime() - time_start
        else                      ! no_permanent_guardcells
        l_with_guardcells = .false.
        time_start = mpi_wtime()
...
! write a second file just for fun
     ...
     &                          user_attr_4 = 4., & 
     &                          user_attr_5 = 5.)
        time_wr_hd = time_wr_hd + (mpi_wtime() - time_start)
        else                      ! no_permanent_guardcells
        l_with_guardcells = .false.
        time_start = mpi_wtime()
     ...
     &                          user_attr_5 = 5.)
        ...
        if(mype.eq.0) then 
           write(*,*) 'Checkpoint file written'
        end if
        call amr_flush(101)
        Call MPI_BARRIER(amr_mpi_meshComm, ierr)

! write checkpoint file using default
        iunit1 = 50
        if(mype.eq.0) write(*,*) 'Calling checkpoint_wr'
        time_wr_de = 0.
        if (no_permanent_guardcells) then
     ...
     &                          user_attr_4 = 4., & 
     &                          user_attr_5 = 5.)
        time_wr_de = mpi_wtime() - time_start
        else                      ! no_permanent_guardcells
        l_with_guardcells = .false.
        time_start = mpi_wtime()
        call amr_checkpoint_wr (iunit1,l_with_guardcells, & 
...
! write a second file just for fun
     ...
     &                          user_attr_4 = 4., & 
     &                          user_attr_5 = 5.)
        time_wr_de = time_wr_de + (mpi_wtime() - time_start)
        else                      ! no_permanent_guardcells
        l_with_guardcells = .false.
        time_start = mpi_wtime()
        call amr_checkpoint_wr (iunit2,l_with_guardcells, & 
...
! read checkpoint file back in
     ...
     &                          user_attr_4 = user4, & 
     &                          user_attr_5 = user5)
        time_re_hd = mpi_wtime() - time_start
        else                      ! no_permanent_guardcells
        l_with_guardcells = .false.
        time_start = mpi_wtime()
...
! read 2nd checkpoint file back in
     ...
     &                          user_attr_4 = user4, & 
     &                          user_attr_5 = user5)
        time_re_hd = time_re_hd + (mpi_wtime() - time_start)
        else                      ! no_permanent_guardcells
        l_with_guardcells = .false.
        time_start = mpi_wtime()
     ...
     &                          user_attr_5 = user5)
        ...
        Call MPI_BARRIER(amr_mpi_meshComm, ierr)
        if(mype.eq.0) then
           write(*,*) 'Checkpoint file read back in'
        end if
        call amr_flush(101)
        Call MPI_BARRIER(amr_mpi_meshComm, ierr)
...
! read checkpoint file back in
     ...
     &                          user_attr_4 = user4, & 
     &                          user_attr_5 = user5)
        time_re_de = mpi_wtime() - time_start
        else                      ! no_permanent_guardcells
        l_with_guardcells = .false.
        time_start = mpi_wtime()
        call amr_checkpoint_re (iunit1,l_with_guardcells, & 
...
! read 2nd checkpoint file back in
     ...
     &                          user_attr_4 = user4, & 
     &                          user_attr_5 = user5)
        time_re_de = time_re_de + (mpi_wtime() - time_start)
        else                      ! no_permanent_guardcells
        l_with_guardcells = .false.
        time_start = mpi_wtime()
        call amr_checkpoint_re (iunit2,l_with_guardcells, & 
...
! Store a copy of the current solution in gt_unk
     ...
     &                        lrestrict,lfulltree, & 
     &                        iopt,lcc,lfc,lec,lnc,tag_offset)
      write(*,*) 'pe ',mype,' exiting 1blk_guardcell_setup in main'
...
! Delayed copying work from unk until unk had good restricted data on parent blocks
! set up data in work
      noff = (nguard_work - nguard)*npgs
      if(noff.ge.0) then
        work(il_bnd+noff:iu_bnd+noff,jl_bnd+noff*k2d:ju_bnd+noff*k2d, & 
...
! first test facevarx
     ...
     &                             facevarx1(ivar,i,j,k,1),v0
            print *,' error = ',abs(facevarx1(ivar,i,j,k,1)-v0)
                      ierror_sum = ierror_sum + 1
                      endif
            enddo
          enddo
...
!      call guardcell_test(mype)
      ...
      Call MPI_BARRIER(amr_mpi_meshComm, ierr)
      if(mype.eq.0) then
         write(*,*) 'Mesh check done.'
         write(*,*) ' TIME TO WRITE HDF5 = ',time_wr_hd
         write(*,*) ' TIME TO READ  HDF5 = ',time_re_hd
         write(*,*) ' '
         write(*,*) ' TIME TO WRITE DEFAULT = ',time_wr_de
         write(*,*) ' TIME TO READ  DEFAULT = ',time_re_de
      end if
      Call MPI_BARRIER(amr_mpi_meshComm, ierr)
...
997      format('w:error proc ',i3,' block l= ',4(2x,i3),2x,f7.4,2x, & 
     &       f7.4)
996      format('fx:error proc ',i3,' block l= ',5(2x,i3),2x,e10.4,2x, & 
     &       e10.4)
