...
! It selectively refines a block next to the north pole at the inner boundary, and
! checks to make sure that the grid refinement is uniform across the pole.
! It checks the prolonged values of the solution which is assumed to be the theta
! coordinate. After each refinement, except the last, the solution on the prolonged
! grid is reset to the correct theta value. The final prolongation does not get
! corrected. This leads to an error in the cells closest to the poles (ie j=nguard+1
! and nyb+nguard-1 in the case of unk) in the finest blocks, since linear interpolation
! is being used. This 'error' is allowed for in the automated testing. If there are
! no permanent guardcells then a guardcell call is made prior to testing, and this
! causes the boundary condition on the inner radial boundary for the cell closest to
! the pole to overwrite the value of facevarx1. Therefore for that data value is
! then tested against the correct analytic solution.

! NOTE : This tests the prolongation operation but is only a partial test of the 
! guardcell filling!

! BE WARNED. This test was a bitch to set up so tinker with it at your
! own risk!!!

! This program checks both the grid and the prolonged values.
...
!#define CHECK_GRID
      ...
      program test_singular_coord_sph_prol
...
! at the poles in the theta direction.
     ...
     &                (bnd_box(2,:,1:lnblocks)-bnd_box(1,:,1:lnblocks))
      ...
      if (spherical_pm) then
                neigh(:,1,1:2) = -40        ! radial boundary condition
                neigh(:,2,1:2) = -41        ! radial boundary condition
...
! Now cycle over blocks adjusting refinement of initial setup as required
      ...
      if (spherical_pm) then
          ...
          elseif(loop_count.eq.1) then
            do l=1,lnblocks
              test1 = abs(coord(1,l)-(1.+5.*.25))
              test2 = abs(coord(2,l)-pi/4.)
              test3 = abs(coord(3,l)-pi/4.)
              if( test1.lt.accuracy.and.test2.lt.accuracy & 
     &                             .and.test3.lt.accuracy) & 
     &            refine(l)=.true.
     ...
     &          ' coord ',coord(:,l),1.+5.*.25,pi/4.,pi/4.
          ...
          elseif(loop_count.eq.2) then
            do l=1,lnblocks
              test1 = abs(coord(1,l)-(1.+5.*.125))
              test2 = abs(coord(2,l)-pi/8.)
              test3 = abs(coord(3,l)-pi/8.)
              if( test1.lt.accuracy.and.test2.lt.accuracy & 
     &                             .and.test3.lt.accuracy) & 
     &            refine(l)=.true.
              if(refine(l)) write(*,*) 'blk ',l,' refine ',refine(l), & 
     &          ' coord ',coord(:,l),1.+5.*.125,pi/8.,pi/8.
            enddo
          elseif(loop_count.eq.3) then
            do l=1,lnblocks
              test1 = abs(coord(1,l)-(1.+5.*.0625))
              test2 = abs(coord(2,l)-pi/16.)
              test3 = abs(coord(3,l)-pi/16.)
              if( test1.lt.accuracy.and.test2.lt.accuracy & 
     &                             .and.test3.lt.accuracy) & 
     &            refine(l)=.true.
            enddo
          elseif(loop_count.eq.4) then
            do l=1,lnblocks
              test1 = abs(coord(1,l)-(1.+5.*.125))
              test2 = abs(coord(2,l)-(pi-pi/8.))
              test3 = abs(coord(3,l)-pi/8.)
              if( test1.lt.accuracy.and.test2.lt.accuracy & 
     &                             .and.test3.lt.accuracy) & 
     &            refine(l)=.true.
            enddo
          elseif(loop_count.eq.5) then
            do l=1,lnblocks
              test1 = abs(coord(1,l)-(1.+5.*.0625))
              test2 = abs(coord(2,l)-(pi-pi/16.))
              test3 = abs(coord(3,l)-pi/16.)
              if( test1.lt.accuracy.and.test2.lt.accuracy & 
     &                             .and.test3.lt.accuracy) & 
     &            refine(l)=.true.
...
#ifdef DEBUG
       call amr_flush(6)
       call mpi_barrier (amr_mpi_meshComm, errcode)
...
!------------------------------------------------
! insert for testing purposes. The automatic coordinate
! based testing process will fail at the poles because 
! prolongation will average between two equal values rather
! than interpolating with a zero or pi value at the pole.
! This section simply resets the value filled by prolongation
! to the correct value in order to pass this test. It only
! fixes those points which need fixing. It will also reset
! guardcell values.
! 

       if(loop_count.lt.loop_count_max-1) then
!start test
! set the solution array to be the grid points x,y or z coordinates
       do l=1,lnblocks
       ...
       if(abs(bnd_box(1,2,l)).lt.eps   .or. & 
     &    abs(bnd_box(2,2,l)-pi).lt.eps) then

       if(abs(bnd_box(1,2,l)).lt.eps) then      ! north pole
       ...
       elseif(abs(bnd_box(2,2,l)-pi).lt.eps) then ! south pole
         if(no_permanent_guardcells) then
           jl=nyb
           ju=nyb
         else
     ...
     &       coord(3,l)-.5*(bsize(3,l)+dz)+dz*real(k-nguard0)
       ...
       do j=jl,ju
       do i=1+nguard0,nxb+nguard0+1
       x0 = coord(1,l)-.5*bsize(1,l) + dx*real(i-nguard0-1)
       y0 = coord(2,l)-.5*(bsize(2,l)+dy)+dy*real(j-nguard0)
       if(ndim.eq.3) z0 = & 
     &       coord(3,l)-.5*(bsize(3,l)+dz)+dz*real(k-nguard0)
...
! This fix is not required for facevary since this always has
! coarse data at theta=0 and pi.
       ...
       y0 = coord(2,l)-.5*(bsize(2,l)+dy)+dy*real(j-nguard0)
       if(ndim.eq.3) z0 = & 
     &       coord(3,l)-.5*bsize(3,l)+dz*real(k-nguard0-1)
       call map_theta_phi(y0,z0)
       value = ax*x0 + ay*y0 + az*z0
       do ivar=1,nfacevar
       facevarz(ivar,i,j,k,l) = value*real(ivar)
       ...
       endif                      ! polar block if test
       endif                      ! nodetype end if
...
!      if(nodetype(l).eq.1 .or. advance_all_levels) then
     ...
     &       coord(1,l)-(.5*bsize(1,l)+dx)
       ...
       call map_theta_phi(yj,zk)

! if j = nguard or nguard+1 at the north pole, or nguard+nyb or
! nguard+nyb+1 at the south pole, and there is a coarser block across
! a radial face, then the interpolation of yj will operate on two equal
! values of the yj coordinate. Therefore in this case set polarfact = 2.
! for the y multiplier in value to enable this test to ignore this issue.
!north pole
         polarfact = 1.
         bound = 0.
         ...
         lpolar=.true.
#ifdef NOTNOW
             if(neigh(1,1,l).eq.-1) then
               if( i.le.nguard. and. & 
     &           (j.le.nguard+1) )  & 
     &            unk1(ivar,i,j,k,1)= ax*xi+ay*yj*polarfact+az*zk
             endif
             if(neigh(1,2,l).eq.-1) then
               if( i.gt.nxb+nguard. and. & 
     &           (j.le.nguard+1) )  & 
     &            unk1(ivar,i,j,k,1)= ax*xi+ay*yj*polarfact+az*zk
             endif
#endif /* NOTNOW */
             if(lrefine(l).eq.maxrefine) then
             if( j.eq.nguard+1) then
               polarfact = 2.
               bound = 0.
             endif
             endif
...
#endif /* NOTNOW */
             if(lrefine(l).eq.maxrefine) then
             if( j.eq.nyb+nguard) then
               polarfact = 2.
               bound = pi
             endif
             endif
         endif
       value = ax*xi+ay*(yj*polarfact-bound)+az*zk
       do ivar=1,nvar
       v0 = value*real(ivar)
...
! if j = nguard_work or nguard_work+1 at the north pole, or nguard_work+nyb or
! nguard_work+nyb+1 at the south pole, and there is a coarser block across
! a radial face, then the interpolation of yj will operate on two equal
! values of the yj coordinate. Therefore in this case set polarfact = 2.
! for the y multiplier in value to enable this test to ignore this issue.
!north pole
...
#ifdef NOTNOW
             if(neigh(1,1,l).eq.-1) then
               if( i.le.nguard_work. and. & 
     &           (j.ge.nguard_work.and.j.le.nguard_work+1) )  & 
     &                                                     polarfact=2.
             endif
             if(neigh(1,2,l).eq.-1) then
               if( i.gt.nxb+nguard_work. and. & 
     &           (j.ge.nguard_work.and.j.le.nguard_work+1) ) & 
     &                                                     polarfact=2.
             endif
#endif /* NOTNOW */
             if(lrefine(l).eq.maxrefine) then
             if( j.eq.nguard_work+1) then
               polarfact = 2.
               bound = 0.
             endif
             endif
!south pole
         ...
         lpolar=.true.
             ...
             if(neigh(1,1,l).eq.-1) then
               if( i.le.nguard_work. and. & 
     &           (j.ge.nyb+nguard_work.and.j.le.nyb+nguard_work+1) )  & 
     &                                                    polarfact=2.
             endif
             if(neigh(1,2,l).eq.-1) then
               if( i.gt.nxb+nguard_work. and. & 
     ...
     &                                                    polarfact=2.
       ...
       if(abs(work1(i,j,k,1)-value) > accuracy) then
         ...
         print *,' neighs = ',neigh(1,:,l)
         print *,' coord+ = ',coord(2:3,l)+(bsize(2:3,l)/2.)
         print *,' coord- = ',coord(2:3,l)-(bsize(2:3,l)/2.)
         ierror_sum = ierror_sum + 1
       endif
       enddo
...
!north pole
         ...
         lpolar=.true.
             ...
             if( j.eq.nguard+1) then
               polarfact = 2.
               bound = 0.
               if( ((neigh(1,1,l).eq.-40 .and. i.eq.nguard+1) .or. & 
     &             (neigh(1,2,l).eq.-41 .and. i.eq.nxb+nguard+1)) & 
     &              .and.(no_permanent_guardcells) & 
     &              .and. loop_count_max.gt.1 )  then
                                 polarfact = 1.         ! this line deals with the special case
                                                        ! where the boundary condition at the inner
                                                        ! radial boundary resets the boundary face
                                                        ! value next to the pole to the correct
                                                        ! analytic value. 
                                                        ! Note, if loop_count_max              ???
                                                        ! is = 1 then this fix is not needed   ???
                                  bound = 0.
               endif
             endif
             endif
!south pole
     ...
     &                             facevarx1(ivar,i,j,k,1),v0
                      write(*,*) 'polar ',polarfact,' bound ',bound
                      ierror_sum = ierror_sum + 1
                      endif
            enddo
          enddo
...
!north pole
         ...
         lpolar=.true.
             if(neigh(1,1,l).eq.-1) then
               if( i.le.nguard. and. & 
     &           (j.ge.nguard.and.j.le.nguard+1) ) polarfact=2.
             endif
             if(neigh(1,2,l).eq.-1) then
               if( i.gt.nxb+nguard. and. & 
     &           (j.ge.nguard.and.j.le.nguard+1) ) polarfact=2.
...
!south pole
         ...
         lpolar=.true.
             if(neigh(1,1,l).eq.-1) then
               if( i.le.nguard. and. & 
     &           (j.ge.nyb+nguard.and.j.le.nyb+nguard+1) ) polarfact=2.
             endif
             if(neigh(1,2,l).eq.-1) then
               if( i.gt.nxb+nguard. and. & 
     &           (j.ge.nyb+nguard.and.j.le.nyb+nguard+1) ) polarfact=2.
...
! Sum number of errors 
 ...
 222  if(mype.eq.0) then
     ...
     &                'Test of singular_coord_sph_prol Successful ', & 
     ...
     &                'Test of singular_coord_sph_prol failed ', & 
     &                ': nprocs ',nprocs,' ndim ',ndim, & 
     &                ' noperm ',lnperm,' advanceall ',ladvanceall, & 
     &                ' mpi ',lmpi
        endif
...
!#endif /* CHECK_GRID */
     ...
     &            'this test work past the mpi_morton_bnd_fluxcon call.'
      ...
      end program test_singular_coord_sph_prol
