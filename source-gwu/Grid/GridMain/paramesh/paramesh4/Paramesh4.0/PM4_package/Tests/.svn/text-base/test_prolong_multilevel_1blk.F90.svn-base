...
#define TEMP_TEST
      ...
      program test_prolong_multilevel




! This test was added specifically to test whether when two
! neighboring leaf blocks at different refinement levels are
! asked to refine, the finer block prolongs correctly.
!
...
!        do while(loop_count.lt.2)
          ...
          elseif(loop_count.eq.1) then
            ...
            if(ndim.eq.3) then
              do l=1,lnblocks
                if( coord(1,l).eq..25.and.coord(2,l).eq..25.and. & 
     &          coord(3,l).eq..25) refine(l)=.true.
              enddo
            elseif(ndim.eq.2) then
              do l=1,lnblocks
                if( coord(1,l).eq..25.and.coord(2,l).eq..25) & 
     &                 refine(l)=.true.
              enddo
            endif
...
!#ifdef DEBUG
     ...
     &    ' loop_count = ',loop_count,' lnblocks=',lnblocks
       call amr_flush(6)
       call mpi_barrier (amr_mpi_meshComm, errcode)
!#endif /* DEBUG */
...
! prolongation is applied to unk and facevar's
     ...
     &    ' loop_count = ',loop_count
      ...
      iopt = 1
      nlayers = nguard
      write(*,*) 'calling amr_prolong loop_count = ',loop_count
      call amr_prolong(mype,iopt,nlayers)
      write(*,*) 'exiting amr_prolong loop_count = ',loop_count, & 
     &        ' iopt ',iopt
...
! test of unk communications
     ...
     &       coord(1,l)-(.5*bsize(1,l)+dx)
       ...
       do ivar=1,nvar
       v0 = value*real(ivar)
       if(abs(v0-unk1(ivar,i,j,k,1)).gt.accuracy) then
...
! test of work communications
     ...
     &       coord(1,l)-(.5*bsize(1,l)+dx)
       ...
       value = ax*xi+ay*yj+az*zk
       if(abs(value-work1(i,j,k,1)).gt.accuracy) then
      ...
      do k=klbnd,kubnd
        ...
        do j=jlbnd,jubnd
          ...
          do i=ilbnd,iubnd+ione
            ...
            do ivar=1,nfacevar
              v0 = value*real(ivar)
              if(abs(v0-facevarx1(ivar,i,j,k,1)) & 
     &                          .gt.accuracy) then
      ...
      do k=klbnd,kubnd
        ...
        do i=ilbnd,iubnd
          ...
          do j=jlbnd,jubnd+ione
            ...
            do ivar=1,nfacevar
              v0 = value*real(ivar)
              if(abs(v0-facevary1(ivar,i,j,k,1)) & 
     &                          .gt.accuracy) then
      ...
      do j=jlbnd,jubnd
        ...
        do i=ilbnd,iubnd
          ...
          do k=klbnd,kubnd+ione
            ...
            do ivar=1,nfacevar
              v0 = value*real(ivar)
              if(abs(v0-facevarz1(ivar,i,j,k,1)) & 
     &                          .gt.accuracy) then
      ...
      do k=klbnd,kubnd+ione
        ...
        do j=jlbnd,jubnd+ione
          ...
          do i=ilbnd,iubnd
            ...
            do ivar=1,nvaredge
              v0 = value*real(ivar)
              if(abs(v0-unk_e_x1(ivar,i,j,k,1)) & 
     &                          .gt.accuracy) then
      ...
      do k=klbnd,kubnd+ione
        ...
        do i=ilbnd,iubnd+ione
          ...
          do j=jlbnd,jubnd
            ...
            do ivar=1,nvaredge
              v0 = value*real(ivar)
              if(abs(v0-unk_e_y1(ivar,i,j,k,1)) & 
     &                          .gt.accuracy) then
      ...
      do j=jlbnd,jubnd+ione
        ...
        do i=ilbnd,iubnd+ione
          ...
          do k=klbnd,kubnd
            ...
            do ivar=1,nvaredge
              v0 = value*real(ivar)
              if(abs(v0-unk_e_z1(ivar,i,j,k,1)) & 
     &                          .gt.accuracy) then
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &                'Test of prolong_multilevel_1blk Successful ', & 
     ...
     &                'Test of prolong_multilevel_1blk failed ', & 
     &                ': nprocs ',nprocs,' ndim ',ndim, & 
     &                ' noperm ',lnperm,' advanceall ',ladvanceall, & 
     &                ' mpi ',lmpi
        endif
