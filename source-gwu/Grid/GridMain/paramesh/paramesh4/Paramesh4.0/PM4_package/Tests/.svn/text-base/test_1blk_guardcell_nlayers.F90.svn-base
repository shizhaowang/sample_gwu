...
!# define DEBUG
      ...
      program test_1blk_guardcell_nlayers
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &                maxval(interp_mask_nc(:))
      ...
      icoord = iloop
      if (mype == 0) write(*,*) 'starting loop : icoord = ',icoord

      if (icoord == 1) then
         nlayersx = nguard
         nlayersy = interp_mask_unk(1)-2
         nlayersz = interp_mask_unk(1)-2
      elseif (icoord == 2) then
         nlayersx = interp_mask_unk(1)-2
         nlayersy = nguard
         nlayersz = interp_mask_unk(1)-2
      ...
      icoord = 0
      if (nlayersx <= 0) nlayersx = 1
      if (nlayersy <= 0) nlayersy = 1
      if (nlayersz <= 0) nlayersz = 1
...
! set the workspace array layer to be tested
      ...
      if(ioptw-1.gt.nvar_work) then
          write(*,*) 'ERROR: Too few work arrays ',nvar_work
...
! prolongation is applied to unk and facevar's
     ...
     &    ' loop_count = ',loop_count
       ...
       if (.not.advance_all_levels) then
          ...
          ! As a test, set the upper levels to zero
          do lb = 1,lnblocks
             if (nodetype(lb) .ne. 1) then
                if (nvar > 0) unk(:,:,:,:,lb) = 0.
                if (nfacevar > 0) then
                   facevarx(:,:,:,:,lb) = 0.
                   facevary(:,:,:,:,lb) = 0.
          ...
          iopt = 1
          call amr_restrict (mype, iopt, 0, .true.) ! true. for filling_guardcells
          iopt = ioptw
          call amr_restrict (mype, iopt, 0, .true.) ! true. for filling_guardcells
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &                'Test of 1blk_guardcell_nlayers Successful ', & 
     ...
     &                'Test of blk_guardcell_nlayers failed ', & 
     &                ': nprocs ',nprocs,' ndim ',ndim, & 
     &                ' noperm ',lnperm,' advanceall ',ladvanceall, & 
     &                ' mpi ',lmpi
        endif
...
!      call guardcell_test(mype)
     ...
     &            ierror_tot, & 
     &            lrelevant, & 
     &            'blk_guardcell_nlayers         ')

 2    call amr_close()

         
 998  format('u:error proc ',i3,' block l= ',5(2x,i3),2x,e15.10,2x, & 
     &     e15.10)
