...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &           'for this test!'
      ...
      if(mype.eq.0) print *,' nprocs = ',nprocs,mype
...
! set the solution array to be the grid points x,y or z coordinates
     ...
     &       coord(3,l)-.5*(bsize(3,l)+dz)+dz*real(k-nguard_work0)
       ...
       if(nvarcorn.gt.0.and.ndim.gt.1) then
...
! set up data in unk_e_x[y][z] 
      ...
      if(nvaredge.gt.0.and.ndim.gt.0) then
...
! Now cycle over blocks adjusting refinement of initial setup as required
        ...
        loop_c_range = 3
!       loop_c_range = 1
        if(ndim.eq.1) loop_c_range = 4

        do while(loop_count.lt.loop_c_range)
     ...
     &                 refine(l)=.true.
        ...
        elseif(loop_count.eq.3) then
          ...
          if(ndim.eq.1) then
            do l=1,lnblocks
              if( coord(1,l).eq..3125) refine(l)=.true.
              if( coord(1,l).eq..6125) refine(l)=.true.
            enddo
          endif
...
! zero out fluxes on those interior faces bordering finer blocks
     ...
     &             remote_pe.eq.laddress(2,iblk) ) then
      ...
      if(cnodetype.eq.2)  & 
     &     write(*,*) 'zeroing flux on face 1 of ',l,mype & 
     &    ,' due to neigh ',remote_block,remote_pe,' of type ', & 
     &      cnodetype & 
     &    ,' surr_blks ',surr_blks(:,1,1+k2d,1+k3d,l)
      endif
! 
     ...
     &             remote_pe.eq.laddress(2,iblk) ) then
      ...
      if(cnodetype.eq.2)  & 
     &     write(*,*) 'zeroing flux on face 2 of ',l,mype & 
     &    ,' due to neigh ',remote_block,remote_pe,' of type ', & 
     &      cnodetype & 
     &    ,' surr_blks ',surr_blks(:,3,1+k2d,1+k3d,l)
      endif
! 
      if(ndim.ge.2) then
     ...
     &             remote_pe.eq.laddress(2,iblk) ) then
      ...
      if(cnodetype.eq.2)  & 
     &     write(*,*) 'zeroing flux on face 3 of ',l,mype & 
     &    ,' due to neigh ',remote_block,remote_pe,' of type ', & 
     &      cnodetype & 
     &    ,' surr_blks ',surr_blks(:,2,1,1+k3d,l)
      endif
! 
     ...
     &             remote_pe.eq.laddress(2,iblk) ) then
      ...
      if(cnodetype.eq.2)  & 
     &     write(*,*) 'zeroing flux on face 4 of ',l,mype & 
     &    ,' due to neigh ',remote_block,remote_pe,' of type ', & 
     &      cnodetype & 
     &    ,' surr_blks ',surr_blks(:,2,1+2*k2d,1+k3d,l)
      endif
! 
      endif
      if(ndim.eq.3) then
     ...
     &             remote_pe.eq.laddress(2,iblk) ) then
      ...
      if(cnodetype.eq.2)  & 
     &     write(*,*) 'zeroing flux on face 5 of ',l,mype & 
     &    ,' due to neigh ',remote_block,remote_pe,' of type ', & 
     &      cnodetype & 
     &    ,' surr_blks ',surr_blks(:,2,2,1,l)
      endif
! 
     ...
     &             remote_pe.eq.laddress(2,iblk) ) then
      ...
      if(cnodetype.eq.2)  & 
     &     write(*,*) 'zeroing flux on face 6 of ',l,mype & 
     &    ,' due to neigh ',remote_block,remote_pe,' of type ', & 
     &      cnodetype & 
     &    ,' surr_blks ',surr_blks(:,2,2,1+2*k3d,l)
      endif
...
! ensure flux conservation
      nsub=1
      call amr_flux_conserve(mype,nsub,flux_dir = icoord)
...
! finally test flux_z
     ...
     &                    flux_z(ivar,i,j,k,l),v0
      ...
      if(mype.eq.0) write(*,*) 'icoord = ',icoord
      Call MPI_BARRIER(amr_mpi_meshComm, ierr)
      call amr_1blk_guardcell_reset
      Call MPI_BARRIER(amr_mpi_meshComm, ierr)
      ...
      if(mype.eq.0) write(*,*) 'Not appropriate with odd number ', & 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &                'Test of flux_conserve_1blk Successful ', & 
     ...
     &                'Test of flux_conserve_1blk failed ', & 
     &                ': nprocs ',nprocs,' ndim ',ndim, & 
     &                ' noperm ',lnperm,' advanceall ',ladvanceall, & 
     &                ' mpi ',lmpi
        endif
