...
! local variables
      ...
      character*40,save :: c_routine = 'mpi_morton_bnd'
...
!     integer :: neigh_morts(6,3,npts_neigh),indx(npts_neigh)
      ...
      integer          :: i_timer = 1        ! mpi_morton_bnd

      integer          :: npts_neigh1,npts_neigh2

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#ifdef DEBUG_FLOW_TRACE
      write(*,*) 'entered mpi_morton_bnd : pe ',mype
...
! Associated changes would also be needed in mpi_morton_bnd and mpi_amr_1blk_restrict
      ...
      if(nodetype(lb).le.2 .or. advance_all_levels) then
...
!      pmort_neigh = -1
     ...
     &                      bnd_box(1,1,lb))
      ...
      if (timing_mpi) then
      timer_mpi_morton_bnd3(i_timer,1) =   & 
     &                            timer_mpi_morton_bnd3(i_timer,1) & 
     &                          + mpi_wtime() - time4
      time4 = mpi_wtime()
      endif
...
#endif /* DEBUG */
            if(istack.gt.npts_neigh1) call expand_neigh_morts
...
#ifdef DEBUG
      write(*,*) 'parent pmort_neigh(2,2,2) ',pmort_neigh(6,2,2,2), & 
...
! in the formula for neigh_morts(6,3,istack)            
     ...
     &             ' added neigh_morts ', & 
     &              neigh_morts(6,:,istack),' istack ',istack
#endif /* DEBUG */
          else
#ifdef DEBUG 
...
! in the formula for neigh_morts(6,3,istack)            
            if(lsingular_line) then
            if(abs(pbndbox(1,2)).lt.eps.and.jj.eq.1) then
             j00 = 3
            elseif(abs(pbndbox(2,2)-pi).lt.eps.and.jj.eq.3) then
             j00 = 1
            endif
            endif
        endif

            neigh_morts(6,3,istack) = (4-ii)+((4-j00)-1)*3+((4-kk)-1)*9
            if(nguarda.gt.nmax_lays) neigh_morts(6,3,istack) = 14
#ifdef DEBUG 
...
! in the formula for neigh_morts(6,3,istack)            
            if(lsingular_line) then
            cbndbox(1,1) = coord(1)+bsize(1)*.5*real(i-3)
            cbndbox(2,1) = coord(1)+bsize(1)*.5*real(i-2)
            cbndbox(1,2) = coord(2)+bsize(2)*.5*real(j-3)
            cbndbox(2,2) = coord(2)+bsize(2)*.5*real(j-2)
            if(ndim.eq.3) then
              cbndbox(1,3) = coord(3)+bsize(3)*.5*real(k-3)
              cbndbox(2,3) = coord(3)+bsize(3)*.5*real(k-2)
            else
              cbndbox(:,3) = bnd_box(:,3,lb)
            endif
            if(abs(cbndbox(1,2)).lt.eps.and.j.eq.1) then
             j00 = 3
            elseif(abs(cbndbox(2,2)-pi).lt.eps.and.j.eq.3) then
             j00 = 1
            endif
            endif
        endif

            neigh_morts(6,3,istack) = (4-i0)+((4-j00)-1)*3+((4-k0)-1)*9  & 
     &                                + 27
...
! provide the complete COMMATRIX to all processors
     ...
     &             commatrix_recv(1:nprocs)
      ...
      if(mype.eq.0) then
         ...
         write(*,'(" ")')
         write(*,'(" ",8i3)') commatrix_recv(1:nprocs)
         write(*,'(" ")')
      endif
...
! Exchange morton information between processors.
     ...
     &          no_of_mortonbnds_received
      write(*,*) 'pe ',mype,' r_mortonbnd(:,1:max_no_of_blocks,1) ', & 
     &          r_mortonbnd(6,:,1:max_no_of_blocks,1)
#endif /* DEBUG  */
...
#ifdef DEBUG 
          write(*,*) 'pe ',mype,' stack entry ',neigh_morts(6,:,i), & 
...
! Store communication info for future use
     ...
     &                          + mpi_wtime() - time1
      ...
      contains
        ...
        end subroutine expand_neigh_morts
...
!--------------------------------------------------
      ...
      subroutine compress_list(neigh_morts,istack,no_of_remote_neighs, & 
     &                         mype,nprocs,l_on_pe)
      ...
      integer, dimension(6,istack) :: t1neigh_morts, t2neigh_morts
      integer, dimension(istack)   :: indx

      integer :: i, itemp, istart, iend, level, j_pe
      integer :: jstack, j
      integer :: morton(6), ierrorcode, ierr
...
#ifdef DEBUG
       write(*,*) 'pe ',mype,' in compress_list ', & 
     &         ' istack = ',istack,' shape(neigh_morts) ', & 
     &          shape(neigh_morts)
#endif /* DEBUG  */
...
#ifdef DEBUG
       write(*,*) 'pe ',mype,' before sorting neigh_morts ', & 
     &         ' istack = ',istack,' neigh_morts ', & 
     &          neigh_morts(6,1,1:istack)
#endif /* DEBUG  */
...
! sort the neigh_morts list according to their morton numbers
      ...
      call morton_sort(t1neigh_morts(:,1:istack),indx(1:istack), & 
     &                    istack)
...
! refinement level
       ...
       do i = 1,istack
          ...
          if (i < istack) then
           if(.not. &
               morton_equal(neigh_morts(1:6,1,istart),  &
                            neigh_morts(1:6,1,i+1))     &
              ) then
...
! unless l_on_pe is true (for flux conservation), skip the local
! processor while searching since any morton nos. found
! on the local proc are not required in the list of remote blocks to be
! fetched.
            if( (j_pe.ne.mype+1)  & 
     &               .or. & 
     &          (j_pe.eq.mype+1 .and. l_on_pe) ) then

            if( & 
     &    ( (morton_greater_than(neigh_morts(1:6,1,i), & 
     &                           morton_limits(1:6,1,1,j_pe))) & 
     &                               .or. & 
     &      ( (morton_equal(neigh_morts(1:6,1,i), & 
     &                      morton_limits(1:6,1,1,j_pe))).and. & 
     &        (neigh_morts(6,2,i).ge.morton_limits(6,2,1,j_pe))    ) & 
     &    ) .and. & 
     &    ( (morton_less_than(neigh_morts(1:6,1,i), & 
     &                        morton_limits(1:6,1,2,j_pe))) & 
     &                               .or. & 
     &      ( (morton_equal(neigh_morts(1:6,1,i), & 
     &                      morton_limits(1:6,1,2,j_pe))).and. & 
     &        neigh_morts(6,2,i).le.morton_limits(6,2,2,j_pe)    ) & 
     &    ) & 
     &        ) lfound = .true.
