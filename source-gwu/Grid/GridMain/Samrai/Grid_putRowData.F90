!!****if* source/Grid/GridMain/Samrai/Grid_putRowData
!!
!! NAME
!!  Grid_putRowData
!!
...
!!                  real(IN)    :: datablock(dataSize(1)),
!!                  integer(IN) :: dataSize)
!!  
!! DESCRIPTION 
!!  
!!  Puts a row of simulation data for a specified variable and cells.
!!  
...
!!
!!  varID : integer value that specifies which variable to put into storage.
!!             ex: DENS_VAR, PRES_VAR as defined in Flash.h 
!!             ALLVARS (defined in constants.h) puts all variables.  
!!  
!!
...
!!
!!
!!  datablock : a real 1 dimensional array containing the data
...
!!          
!!          size(1) holds the number of cells in the row to return
!!          
!!
!! EXAMPLE  
!!  
...
!!  
!!             call Grid_putRowData(blockID, DENS_VAR, IAXIS, EXTERIOR, &
...
!!
!!  
!!    In this 2d block example we will put part of a row for all variables 
...
!!          do blockID = 1, localNumBlocks
!!  
!!             call Grid_putRowData(blockID, ALLVARS, IAXIS, INTERIOR, &
!!                               startingPos, dataBlock, dataSize)
!!  
!!          end do
!!
...
subroutine Grid_putRowData(blockid, cellType, variable, beginCount, &
...
#include "Flash.h"
  ...
  integer, intent(in) :: blockid, cellType, variable, beginCount, row
  ...
  call samrai_put_row_data(patchNum, levelNum, varIDSamrai, row, posSamrai, datablock, datasize)
