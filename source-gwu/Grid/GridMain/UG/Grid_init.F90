...
! have been set based on the corresponding runtime parameters.
  ...
  allocate(gr_globalNeghProc(gr_meshNumProcs,NDIM*CONSTANT_TWO))
...
!!  end do
  ...
  ! H.E. 
  ! This will be executed on everyprocessor
  
  do i = 1,NDIM
     !! determine the left negh along i, if -1 set it to the last processor along this axis
     lnegh=gr_axisMe(i)-1     
     if(lnegh<0) lnegh=gr_axisNumProcs(i)-1    
     gr_globalNeghProc(gr_globalMe+1,HIGH*i-LOW)=lnegh
     rnegh=gr_axisMe(i)+1    !! do the same for right neighbor 
     if(rnegh==gr_axisNumProcs(i)) rnegh=0
     gr_globalNeghProc(gr_globalMe+1,i*HIGH)=rnegh
  end do
