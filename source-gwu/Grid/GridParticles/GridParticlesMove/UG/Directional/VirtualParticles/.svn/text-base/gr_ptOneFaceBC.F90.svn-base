...
#include "Flash.h"
  ...
  logical, optional, intent(IN) :: leaving
  logical, optional, intent(INOUT) :: VP
  integer, optional, intent(OUT) :: pos
  real, optional, intent(OUT) :: newPos

!!  logical, intent(INOUT) :: moved
  real :: dist
  integer :: vel,posPred,velPred
  real,dimension(LOW:HIGH) :: corner
  logical :: predictor, singlePeriodicBlock
  
  
  if(leaving.or. ((.not.leaving).and.(gr_domainBC(face,axis)==PERIODIC)) )then
     ...
     elseif(gr_domainBC(face,axis)==PERIODIC) then
        ...
        newPos=dist+particle(pos)
        if(predictor.and.leaving)particle(posPred)=dist+particle(posPred)
