...
#else
  use Grid_data, ONLY : gr_meshMe, gr_axisNumProcs
...
!print *, "Processor", gr_meshMe, "in Grid_mapParticlesToMesh"
  ...
  bufferSize=minval(blkSizeGC)
  bufferSize=(blkSizeGC(IAXIS)*blkSizeGC(JAXIS)*blkSizeGC(KAXIS))/bufferSize
  bufferSize=bufferSize*2
  allocate(recvBuf(bufferSize))
  allocate(sendBuf(bufferSize))
  ...
  do axis = IAXIS,NDIM
     ...
     if (gr_axisNumProcs(axis) > 1) then
        call gr_ptExchangePartialMap(blkLimits,blkLimitsGC,bufferSize,axis,LOW,sendBuf,recvBuf)
        call gr_ptExchangePartialMap(blkLimits,blkLimitsGC,bufferSize,axis,HIGH,sendBuf,recvBuf)
     end if
