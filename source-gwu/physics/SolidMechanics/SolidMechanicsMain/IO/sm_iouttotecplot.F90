!!****if* source/physics/SolidMechanics/SolidMechanicsMain/IO/sm_iouttotecplot.F90
!!
!! NAME
!! 
!!
...
subroutine sm_iouttotecplot(nstep,time,dt,count)
  ...
  real :: id, x, y, z, nx, ny, nz, tx, ty, tz, u, v, w, us, vs, ws, un, vn, wn, dv, ddv, dddv, p
  
  real :: LFx, LFy, dh, area
...
#ifdef WRITE_PART
  write(index_name,"(I6.6)") count
     open(unit=110,file='./IOData/parts3d.'//index_name,form='formatted',&
          status='replace')
     close(110)
#endif
!-----------------------------------------------------------------------
...
!           nmlx=gr_sbBodyInfo(ibd)%particles(FULO_PART_PROP,i)
!           nmly=gr_sbBodyInfo(ibd)%particles(FVLO_PART_PROP,i)
!           nmlz=gr_sbBodyInfo(ibd)%particles(FWLO_PART_PROP,i)
           pres=gr_sbBodyInfo(ibd)%particles(PRES_PART_PROP,i)
!           pres=gr_sbBodyInfo(ibd)%particles(GLOB_PART_PROP,i)
           ...
           write(110,'(13g19.10)') xi,yi,zi        
        enddo

#ifdef TEST_PART_OUTPUT
        ! Shizhao added this part for debuging flow information on the 3D particle surface
        write(110,'(A)') '# ID, x, y, z, nx, ny, nz, tx, ty, tz, u, v, w, us, vs, ws, un, vn, wn, dv, ddv, dddv' 
        do i = 1,gr_sbBodyInfo(ibd)%totalpart
           x = gr_sbBodyInfo(ibd)%particles(POSX_PART_PROP,i) 
           y = gr_sbBodyInfo(ibd)%particles(POSY_PART_PROP,i)
           z = gr_sbBodyInfo(ibd)%particles(POSZ_PART_PROP,i)
 
           nx = gr_sbBodyInfo(ibd)%particles(NMLX_PART_PROP,i) 
           ny = gr_sbBodyInfo(ibd)%particles(NMLY_PART_PROP,i)
           nz = gr_sbBodyInfo(ibd)%particles(NMLZ_PART_PROP,i)
 
           tx = gr_sbBodyInfo(ibd)%particles(VORX_PART_PROP,i) 
           ty = gr_sbBodyInfo(ibd)%particles(VORY_PART_PROP,i)
           tz = gr_sbBodyInfo(ibd)%particles(VORZ_PART_PROP,i)
 
           u = gr_sbBodyInfo(ibd)%particles(VELX_PART_PROP,i) 
           v = gr_sbBodyInfo(ibd)%particles(VELY_PART_PROP,i)
           w = gr_sbBodyInfo(ibd)%particles(VELZ_PART_PROP,i)
 
           us = gr_sbBodyInfo(ibd)%particles(FUL_PART_PROP,i) 
           vs = gr_sbBodyInfo(ibd)%particles(FVL_PART_PROP,i)
           ws = gr_sbBodyInfo(ibd)%particles(FWL_PART_PROP,i)
 
           un = gr_sbBodyInfo(ibd)%particles(FULO_PART_PROP,i) 
           vn = gr_sbBodyInfo(ibd)%particles(FVLO_PART_PROP,i)
           wn = gr_sbBodyInfo(ibd)%particles(FWLO_PART_PROP,i)
 
           dv   = gr_sbBodyInfo(ibd)%particles(ACCX_PART_PROP,i) 
           ddv  = gr_sbBodyInfo(ibd)%particles(ACCY_PART_PROP,i)
           dddv = gr_sbBodyInfo(ibd)%particles(ACCZ_PART_PROP,i)
 
           p  = gr_sbBodyInfo(ibd)%particles(PRES_PART_PROP,i)
           !p  = gr_sbBodyInfo(ibd)%particles(PEXT_PART_PROP,i)
           id = gr_sbBodyInfo(ibd)%particles(GLOB_PART_PROP,i)

           write(110,'(30g20.12)') id, x, y, z, nx, ny, nz, tx, ty, tz, u, v, w, us, vs, ws, un, vn, wn, dv, ddv, dddv, p
...
#else
        ! Write Particles 2D:
        write(110,'(A)') 'VARIABLES = "X", "Y", "nx", "ny", "p", "fvx", "fvy", "wz", "Lx", "Ly" "a" "dh" "u" "v"'
        write(110,*)'ZONE I=',gr_sbBodyInfo(ibd)%totalpart,', T="Parts'//index_ibd//'"',', F=POINT'
        do i = 1,gr_sbBodyInfo(ibd)%totalpart
           xi  =gr_sbBodyInfo(ibd)%particles(POSX_PART_PROP,i)
           yi  =gr_sbBodyInfo(ibd)%particles(POSY_PART_PROP,i)
           ...
           fvy =gr_sbBodyInfo(ibd)%particles(FYVI_PART_PROP,i)
           wz  =gr_sbBodyInfo(ibd)%particles(VORZ_PART_PROP,i)
           LFx  =gr_sbBodyInfo(ibd)%particles(FUL_PART_PROP,i)
           LFy  =gr_sbBodyInfo(ibd)%particles(FVL_PART_PROP,i)
           area =gr_sbBodyInfo(ibd)%particles(AREA_PART_PROP,i)
           dh   =gr_sbBodyInfo(ibd)%particles(HL_PART_PROP,i)
           !u = gr_sbBodyInfo(ibd)%particles(VELX_PART_PROP,i) 
           !v = gr_sbBodyInfo(ibd)%particles(VELY_PART_PROP,i)
           u = gr_sbBodyInfo(ibd)%particles(UITP_PART_PROP,i) 
           v = gr_sbBodyInfo(ibd)%particles(VITP_PART_PROP,i)
           write(110,'(20g19.10)') xi,yi,nmlx,nmly,pres,fvx,fvy,wz,LFx,LFy,area,dh,u,v
        enddo
#endif  /* NDIM */
        close(110)      

#else  /* ifndef WRITE_BINARY_GEO */
#ifdef WRITE_PART
        ! Shizhao for debug
        ! Write Particles
        open(unit=110,file='./IOData/parts3d.'//index_name,form='formatted',&
...
!           pres=gr_sbBodyInfo(ibd)%particles(GLOB_PART_PROP,i)
           ...
           wy  =gr_sbBodyInfo(ibd)%particles(VORY_PART_PROP,i)
           wz  =gr_sbBodyInfo(ibd)%particles(VORZ_PART_PROP,i) 
           write(110,'(30g19.10)') xi,yi,zi,nmlx,nmly,nmlz,pres,fvx,fvy,fvz,wx,wy,wz        
        enddo
#else
        ...
        if (any(sm_BodyInfo(ibd)%OnBoundary(:,:) .eqv. .TRUE.) .and. .true.) then
           ...
           elseif(x_sgn .eq. -1) then 
              !nwx = int((minval(vx(:)-eps)-gr_globalDomain(LOW,IAXIS))/Lx) ! The minval(...) should be abs(minval(...)), Shizhao
              nwx = int(abs(minval(vx(:)-eps)-gr_globalDomain(LOW,IAXIS))/Lx)  ! Shizhao
           endif
           ...
           elseif(y_sgn .eq. -1) then 
              !nwy = int((minval(vy(:)-eps)-gr_globalDomain(LOW,JAXIS))/Ly) ! The minval(...) should be abs(minval(...)), Shizhao 
              nwy = int(abs(minval(vy(:)-eps)-gr_globalDomain(LOW,JAXIS))/Ly) ! Shizhao
           endif
           ...
           elseif(z_sgn .eq. -1) then 
              !nwz = int((minval(vz(:)-eps)-gr_globalDomain(LOW,KAXIS))/Lz)  ! The minval(...) should be abs(minval(...)), Shizhao
              nwz = int(abs(minval(vz(:)-eps)-gr_globalDomain(LOW,KAXIS))/Lz) ! Shizhao
           endif
#endif           
           plot_nel = 0
