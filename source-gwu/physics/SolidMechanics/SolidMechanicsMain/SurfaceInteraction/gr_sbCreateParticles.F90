...
! Turn off to reduce the time in printing on screens. Shizhao Wang, Jan 07, 2015
           if (b .eq. 1) print*,gr_meshMe,"Total # of Particles on the 1st body: ",gr_sbBodyInfo(b)%totalPart
...
!        do ip = 1, 3862
!          write(30000,*) sm_bodyInfo(b)%x(ip), sm_bodyInfo(b)%y(ip), sm_bodyInfo(b)%z(ip)
!        enddo
        ...
        ! debug
!        write(31000,*) gr_sbBodyInfo(b) % totalPart 
!        do ip = 1, gr_sbBodyInfo(b) % totalPart
!          write(31000,*) gr_sbBodyInfo(b) % particles(POSX_PART_PROP,ip), &
!          gr_sbBodyInfo(b) % particles(POSY_PART_PROP,ip), &
!          gr_sbBodyInfo(b) % particles(POSZ_PART_PROP,ip)
!        enddo
!        write(31000,*) '================' 
