...
!  write(5100,*)  sm_bodyInfo(ibd)%ndofs
!  write(5100,*)  sm_bodyInfo(ibd)%ng
!  write(5100,*)  sm_bodyInfo(ibd)%neq
!  write(5100,*)  sm_bodyInfo(ibd)%qn
  ...
  do e=1,nel
     ...
     select case( sm_bodyInfo(ibd)%ws_eltype(e) )
        ...
        case( FOUR_NODE_QUADRILATERAL )
           call sm_el03_mapParticles(sm_bodyInfo(ibd), e, ptelem, &
                                     xpos(1:ptelem),ypos(1:ptelem),zpos(1:ptelem), &
                                     xvel(1:ptelem),yvel(1:ptelem),zvel(1:ptelem), &
                                     xacc(1:ptelem),yacc(1:ptelem),zacc(1:ptelem), &
                                     xnrm(1:ptelem),ynrm(1:ptelem),znrm(1:ptelem), &
