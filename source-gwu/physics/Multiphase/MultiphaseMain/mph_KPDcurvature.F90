...
#include "Flash.h"
        ...
        do j = jy1,jy2
           do i = ix1,ix2    
              ...
              !----------------------------------------------------
              !if(abs(crv(i,j,k)) > eps) write(*,*) 'crv', crv(i,j,k), rPhiXE,rMagE,rPhiXW,rMagW,rPhiYN,rMagN, rPhiYS,rMagS
           end do
        end do
#ifdef RIAZ_CURV
...
! Shizhao Wang smoothes the viscosity, to reduce the unphysical
! oscillations at high Reynolds numbers (Re>1000)
#ifdef SMOOTH_VISC_COR
       pi = acos(-1.0d0)
       k=1
       do j = jy1-1,jy2+1
          do i = ix1-1,ix2+1
              !ws = 4.0d0*sqrt(dx*dx+dy*dy)
              ws = 2.0d0*min(dx,dy)

              if( abs(s(i,j,k)) <= -ws .and. (s(i,j,k)*s(i-1,j,k)<0.0d0 .or. &
                                              s(i,j,k)*s(i+1,j,k)<0.0d0 .or. &
                                              s(i,j,k)*s(i,j-1,k)<0.0d0 .or. &
                                              s(i,j,k)*s(i,j+1,k)<0.0d0 )) then
                 ! H = 0.5d0*(1-s(i,j,k)/ws-sin(pi*s(i,j,k)/ws)/pi)
                 hs = 0.5d0*(1-s(i,j,k)/ws-sin(pi*s(i,j,k)/ws)/pi)
                 visc(i,j,k) = (1.0d0-hs)*vis1/vis2 + hs*vis2/vis2
                 if(visc(i,j,k) > 1.0d0) then
                    write(*,*) 'Waring: reset visc', visc(i,j,k), i,j,k
                    visc(i,j,k) = 1.0d0
                 endif
                 if(visc(i,j,k) < vis1/vis2) then
                    write(*,*) 'Waring: reset visc', visc(i,j,k), i,j,k
                    visc(i,j,k) = vis1/vis2
                 endif
              end if
           end do
        end do
...
! oscillations at high Reynolds numbers (Re>1000)
        ...
        do j = jy1-1,jy2+1
           do i = ix1-1,ix2+1
              ...
              if(s(i,j,k).ge. ws) then
                 ! H = 0
                 visc(i,j,k) = vis1/vis2
              elseif( s(i,j,k) >= -ws ) then
                 ! H = 0.5d0*(1-s(i,j,k)/ws-sin(pi*s(i,j,k)/ws)/pi)
                 hs = 0.5d0*(1-s(i,j,k)/ws-sin(pi*s(i,j,k)/ws)/pi)
                 visc(i,j,k) = (1.0d0-hs)*vis1/vis2 + hs*vis2/vis2
                 if(visc(i,j,k) > 1.0d0) then
