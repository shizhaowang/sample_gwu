...
! --------------------------------------------------------------------------
  ...
  call Timers_start('InitLocVarAndCheckBDs') !Shizhao Wang
  newu = 0.
  newv = 0.
  neww = 0.
  flxint_u = 0.
...
#ifdef FLASH_GRID_PARAMESH
  ! Shizhao Wang added these lines for restarting the simulation
  ! The morton process is needed to be initialized for restarting
  if(firstcall) then
    grid_changed = 1
    call amr_mg_init()
    firstcall = .false.
...
!!$  endif
  ...
  call Timers_stop('InitLocVarAndCheckBDs') !Shizhao Wang

  ! Timestep Loop:
  do ist = 1,itmx

  call Timers_start('PreSetRHS')  !Shizhao Wang
  ...
  if (ins_isgs .NE. 0) then
     do lb = 1,blockCount
        ...
        call ins_vt_damp(ins_isgs,NGUARD,nxc,nyc,nzc,                   &
                    ins_invRe,del(DIR_X),del(DIR_Y),del(DIR_Z),    &
                    coord,bsize,                                   &
                    facexData,&
                    faceyData,&
...
! Shizhao add these lines to fix fluxes
#ifdef FLASH_GRID_PARAMESH
...
!!$  CALL SYSTEM_CLOCK(TA(1),count_rate)
  ...
  call Timers_stop('PreSetRHS')  !Shizhao Wang
...
! Shizhao add these lines to fix fluxes after updating the IB forces
#ifdef FLASH_GRID_PARAMESH
  ! Fix fluxes at block boundaries
  call ins_fluxfix(NGUARD,nxc,nyc,nzc,nxc-1,nyc-1,nzc-1,&
                   blockCount,blockList)
...
!  if (ins_meshMe .eq. MASTER_PE)  write(*,*) 'Total IB Time =',ETIB
  ...
  call Timers_start('PosSourceTerm')  ! Shizhao Wang
  ! Compute inflow volume ratio: (Not computed on NOT_BOUNDARY, NEUMANN_INS, OUTFLOW_INS)
  ...
  call Timers_stop('PosSourceTerm')  ! Shizhao Wang
...
!!$#endif
  ...
  call Timers_start('CheckDiv') ! Shizhao Wang
...
!  write(1000000+ins_meshMe,*) mxdivv, mndivv
  ...
  !----------------------------------------------------------------------------------------------------
  call Timers_stop('CheckDiv') ! Shizhao Wang
