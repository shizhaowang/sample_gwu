...
!print*,"Vt Blocks",blockID
!print*,"Vt Blocks Before",blockID,facexData(7,8,8,8),faceyData(7,8,8,8),facezData(7,8,8,8)

do k=1,NZB+NGUARD*2
do j=1,NZB+NGUARD*2
do i=1,NZB+NGUARD*2

facexData(7,i,j,k) = REAL(blockID)*SIN(REAL(i))*COS(REAL(j))*TAN(REAL(k))
faceyData(7,i,j,k) = REAL(blockID)*SIN(REAL(j))*SIN(REAL(k))*TAN(REAL(i)) 
facezData(7,i,j,k) = REAL(blockID)*SIN(REAL(k))*TAN(REAL(i))*TAN(REAL(j))

end do
end do
end do

!print*,"Vt Blocks After ",blockID,facexData(7,8,8,8),faceyData(7,8,8,8),facezData(7,8,8,8)
     ...
     do k=GRID_KLO_GC,GRID_KHI_GC
        do j=GRID_JLO_GC,GRID_JHI_GC
           do i=GRID_ILO_GC,GRID_IHI_GC
              ...
              if     (solnData(VISC_VAR,i,j,k) .lt. 0.0) then
                 solnData(VISC_VAR,i,j,k) = 0.0
              elseif (solnData(VISC_VAR,i,j,k) .gt. 1.0) then
                 solnData(VISC_VAR,i,j,k) = 1.0
              end if
...
!*********************************************************************************************
  ...
  !KPD - Initialize Vorticity Variables
  tpdudxc = 0.
  tpdudyc = 0.
  tpdudzc = 0.
  tpdvdxc = 0.
  ...
  ! KPD - Velocity derivatives @ cell center:
  call ins_velgradtensor(NGUARD,facexData,faceyData,facezData, &
             del(DIR_X),del(DIR_Y),del(DIR_Z),tpdudxc,tpdudyc,tpdudzc,&
             tpdvdxc,tpdvdyc,tpdvdzc,tpdwdxc,tpdwdyc,tpdwdzc)

  ! KPD - Corner values of vorticity:
  solnData(OMGX_VAR,:,:,:) = tpdwdyc - tpdvdzc
  solnData(OMGY_VAR,:,:,:) = tpdudzc - tpdwdxc
  solnData(OMGZ_VAR,:,:,:) = tpdvdxc - tpdudyc
