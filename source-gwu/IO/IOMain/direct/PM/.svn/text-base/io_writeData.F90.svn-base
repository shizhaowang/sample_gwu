!!****if* source/IO/IOMain/direct/PM/io_writeData
!!
!! NAME
!!
!!  io_writeData
...
#include "Flash.h"
  ...
  write(ioLun)localNumBlocks
  
    
  !! write gid. gr_gid is filled in Grid_sendOutputData
  !! gid contains neighbor data
  write(ioLun)gr_gid(:, 1:localNumBlocks)


  !! Write the bndbox
  write(ioLun)bnd_box(:,:,1:localNumBlocks)
  


  !store the coordinates
  write(ioLun)coord(:,1:localNumBlocks)


  ! store the block size
  write(ioLun)bsize(:,1:localNumBlocks)
  ...
  do var = UNK_VARS_BEGIN,UNK_VARS_END
     ...
     if(io_doublePrecision) then
        ...
        write(ioLun)unk(var,io_ilo:io_ihi, io_jlo:io_jhi, io_klo:io_khi,1:localNumBlocks) 
        ...
        if (isPlotVar) then
           ...
           unkt(1,1:NXB,1:NYB,1:NZB,1:localNumBlocks) = real( &
                (unk(var,io_ilo:io_ihi,io_jlo:io_jhi,io_klo:io_khi, 1:localNumBlocks)), kind = single) 
  ...
  do i = SCRATCH_GRID_VARS_BEGIN,SCRATCH_GRID_VARS_END
     ...
     if(isPlotVar) then
        ...
        if(io_doublePrecision) then
           ...
           write(ioLun)scratch(i,io_ilo:io_ihi, io_jlo:io_jhi, io_klo:io_khi,1:localNumBlocks) 

        else
           
           unkt(1,1:NXB,1:NYB,1:NZB,1:localNumBlocks) = real( &
                scratch(i,io_ilo:io_ihi, io_jlo:io_jhi, io_klo:io_khi,1:localNumBlocks), kind = single) 
