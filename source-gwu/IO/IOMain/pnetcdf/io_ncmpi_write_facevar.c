...
void FTOC(io_ncmpi_write_facevar)(int* file_identifier,
				  int* varid,
				  int* dir,  /*directionality of facevar being output*/
				  ...
				  int* global_offset)
{
  ...
  *(record_label_new + 4) = '\0';
  ...
  if(status < 0){
    Driver_abortFlashC("Error: unable to get dim in io_ncmpi_write_facevar");
  }
  ...
  switch(*dir){
    case IAXIS: /*x direction*/
	status = ncmpi_inq_dimid(ncid, "dim_nxb_long", &dim_nxb);
	if(status < 0){
	    Driver_abortFlashC("Error: unable to get dim_nxb_long in io_ncmpi_write_facevar");
	}
	break;
    case JAXIS:
	status = ncmpi_inq_dimid(ncid, "dim_nyb_long", &dim_nyb);
	if(status < 0){
	    Driver_abortFlashC("Error: unable to get dim_nyb_long in io_ncmpi_write_facevar");
	}
	break;
    case KAXIS:
    	status = ncmpi_inq_dimid(ncid, "dim_nzb_long", &dim_nzb);
	if(status < 0){
	    Driver_abortFlashC("Error: unable to get dim_nzb_long in io_ncmpi_write_facevar");
	}
	break;
    default:
	Driver_abortFlashC("Error: bad direction provided to io_ncmpi_write_facevar");
  }
  ...
  if (status < 0){
    printf("Error: Unable to define var in io_ncmpi_write_facevar\n");
    Driver_abortFlashC("Error: Unable to define var io_ncmpi_write_facevar\n");
  }  
  ...
  if (status < 0){
    printf("Error: ncmpi_put_vara_double_all, favevar\n");
    Driver_abortFlashC("Error: ncmpi_put_vara_double_all, facevar\n");
