...
/****if* source/IO/IOMain/pnetcdf/io_ncmpi_read_generic_rarr_
 *
 * NAME
 *
 *  io_ncmpi_read_generic_rarr_
 *
 *
 * SYNOPSIS
 *
 *  void io_ncmpi_read_generic_rarr_(file_identifier, generic_arr, local_size, total_size,
 *                        global_offset, dataset_name, dataset_name_len)
 *
 *  void io_ncmpi_read_generic_rarr_(hid_t*, int*, int*, int*, int*, char*, int*)
 *
 *
 * DESCRIPTION
 *
...
void FTOC(io_ncmpi_read_generic_rarr)(int* file_identifier,
				      int generic_arr[],
				      int* local_size,
				      int* total_size,
				      int* global_offset,
				      ...
				      int* dataset_name_len)
{
  ...
  *(dataset_name_new + *dataset_name_len) = '\0';
  ...
  status = ncmpi_get_vara_double_all(ncid, varid, &start_1d, &count_1d, generic_arr);
