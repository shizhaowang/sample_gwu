...
/****if* source/IO/IOMain/pnetcdf/io_ncmpi_write_generic_rarr_
 *
 * NAME
 *
 *  io_ncmpi_write_generic_rarr_
 *
 *
 * SYNOPSIS
 *
 *  void io_ncmpi_write_generic_rarr_(file_identifier, generic_arr, local_size, total_size,
 *                        global_offset, dataset_name, dataset_name_len, dim_name, dim_name_len)
 *
 *  void io_ncmpi_write_generic_rarr_(hid_t*, int*, int*, int*, int*, char*, int*, char*, int*)
 *
 *
 * DESCRIPTION
 *
...
void FTOC(io_ncmpi_write_generic_rarr)(int* file_identifier,
				       int generic_arr[],
				       int* local_size,
				       int* total_size,
				       int* global_offset,
				       ...
				       int* dim_name_len)
{
  ...
  *(dim_name_new + *dim_name_len) = '\0';
  ...
  /* check to see if the dimension already exists.  If so, use it other wise define it */

     
  status = ncmpi_inq_dimid(ncid, dim_name_new, &dim_generic); 

  /*status will be less than 0 if dimension does not already exist */
  if(status < 0) {
  ...
  status = ncmpi_def_var (ncid, dataset_name_new, NC_DOUBLE, rank, &dim_generic, &varid);
  ...
  if (status < 0){
    printf("Error io_ncmpi_write_generic_rarr: can not enddef\n");
    Driver_abortFlashC("Error io_ncmpi_write_generic_rarr: enddef\n");
  }
  ...
  */
  ...
  status = ncmpi_put_vara_double_all(ncid, varid, &start_1d, &count_1d, generic_arr);
  ...
  if (status < 0){
    printf("Error: ncmpi_put_vara_double_all, generic_arr\n");
    Driver_abortFlashC("Error: ncmpi_put_vara_double_all, generic_arr\n");
